...
- [Capture + check are model-bounded](feedback_capture_and_check_are_model_bounded.md) — self-check + document-as-you-go run WITHIN your model of a change's footprint → systematically MISS out-of-model sites (the coupled index/pointer/sibling/elsewhere-ledger); only EXTERNAL enumeration catches them — independent reviewer (different model) / structural full-set guard / a MUTATING subagent's returned change-manifest; recurrence of a miss-class = build the guard, not more manual rechecks; the deepest M7 why
