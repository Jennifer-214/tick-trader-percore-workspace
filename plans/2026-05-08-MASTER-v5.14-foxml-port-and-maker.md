...
### Phase 3 — Maker MVP (~3-4 days; off by default) — **DEFERRED 2026-05-09**

**v5.14.7 — Maker order MVP (POST_ONLY + cancel-and-replace)** [3-4 days] — **DEFERRED to v6.0+ master plan**

**Deferral rationale (2026-05-09):** Operator has no order book data captured yet (DepthRecorder not run; no historical depth CSVs for backtest replay). Without depth data, MVP can't be paper-tested in backtest. Operator chose comprehensive v6.0 master plan once depth data exists, vs ship-MVP-foundation-now-iterate-later. Total effort same (~30-45h either path); sequencing differs. Tracked as TECH_DEBT-008.

**Trigger to revisit:** When operator captures order book data (DepthRecorder runs OR external depth-tape feed). At that point either reopen v5.14.7 plan as foundation OR draft v6.0 maker master plan covering full scope (queue-position simulation, depth-aware offset, fill-rate feedback, race reconciliation, fee rebate handling, multi-level depth).

**Phase 4 succession:** v5.14.8+ no longer block on Phase 3; predecessors re-anchor to v5.14.6 close.
...
v5.14.6     — /bug-check skill                             [SHIPPED 2026-05-09]
              (registry-driven; see commits 1b4563d + workspace 4785905)
...
v5.14.8     — Stamp body lineage + stale gating            [PENDING]
  ...
  v5.14.8.C — environment metadata + tests
              ...
              (Predecessor re-anchored 2026-05-09: v5.14.6 close;
               was v5.14.7 but v5.14.7 deferred — see TECH_DEBT-008)

v5.14.7     — Maker order MVP (off by default)             [DEFERRED 2026-05-09]
              Deferred to v6.0+ master plan pending order book data.
              See TECH_DEBT-008 + Phase 3 deferral note above.
              Original sub-tags preserved in plan file for reference:
