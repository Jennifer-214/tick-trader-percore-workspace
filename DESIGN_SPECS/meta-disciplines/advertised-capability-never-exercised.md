...
applications:
  ...
  - 'REGISTRY-ROW / WIRE-KEY surface (D-426, 2026-08-17) — the THIRD unit of the same shape, after tooling and functions. 18 of 46 rows in FOREACH_STAMP_BOUND_MODEL_CONST have NO production producer. Ten are live defects, and the severity is carried INDIRECTLY: nothing reads the 9 inference_cfg_* rows VALUES, so a value-oriented trace returns "no consumer" and stops — but their absence means MASK_inference_cfg is never set, and that bit is the sole gate for the entire train->serve cfg-derived drift layer (~30 rows + 4 more, 3 of them REFUSE_STRICT). PARITY-042. The lesson for detection: at this unit the consumer is not the row, it is the PRESENCE BIT the row would set'
---
