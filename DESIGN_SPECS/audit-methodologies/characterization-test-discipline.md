...
- **Slippage book-price (paper/backtest-mode only).** The fill books at `tick ± slip` (entry `+`, exit `−`), NOT at `live_tp`: the production SSoT is the OMS submit chokepoint **`CoreFrameworks/OrderManager.hpp:1008-1012`** (`a9_fill = (type==ORDER_MARKET_SELL) ? event_price−slip : event_price+slip`), inside the `!LIVE_TRADING` synthetic-fill branch — so **live books the raw executionReport price** (slippage 0; the live adapter path never reaches this block). `slippage_pct` binds at `Order_BindPreResolved` (`Order.hpp:364` ← `core_cfg.slippage_pct`; non-zero `0.05%` default per D-203). ⚠️ **The old `EventLoop_OnEvent` slip block (`ControllerEventLoop.hpp:1873-1890`) + the `ExitBuffer_PendingProceeds` exit-slip (`Portfolio.hpp:201-203`, single_core/backtest estimator only) were SUPERSEDED/deleted by the A9 fix — do NOT cite them** (re-grounded 2026-06-14 by the F-059 I+A fan-out; the exact drift this spec warns about). `live_tp` is the exit *trigger* (the SG compare `ExecutionCore.hpp:429-431`), never a booked price — conflating them mis-models both paths.
...
- **Cloning a fixture — verify its STATE-DRIVER matches YOUR path (else false-green / no-op).** A characterization test cloned from a sibling inherits the sibling's fill-DRIVER, which may not drive YOUR path. Two real traps caught at F-059's `oms-ts-1e` design (2026-06-14): (a) cloning oms-ts-1's **manual `HandleFill`+`Order_SetState(FILLED)`+slot-free loop** makes a `OrderManager_Tick`-driven fill a dedup **no-op** (`Order_GetState==FILLED → return 0`) → the slip-via-`Submit→Tick` arm never fires → the assertion passes for the wrong reason; (b) the clone's **`core_cfg=nullptr`** leaves `slippage_pct=0` → the slip is a silent no-op → the slip test degenerates into a no-slip test. Rule: when a test's value depends on a path the clone-source did NOT exercise, DRIVE that path (don't hand-advance state), pass the REAL cfg the path reads, and add a **witness assert that the path actually ran** (e.g. `ORDER_FILLED` reached *via Tick*; booked price ≠ unslipped). Sister to the `tsa-live-2` hand-seed anti-pattern (Lens-3 false-positive surface). **Bridge note:** the `Money_ToDouble`→`money_from_double_payload` double-bridge a paper synth crosses is bit-EXACT for `|v| ≤ 2^53` (≈ $90M; proven IEEE-754, ~400× margin at $60k) → a `Money_Eq` golden across it is safe for sane price/balance magnitudes.
