...
When two options both compile + both run, the gradient resolves the choice. These are NOT absolute (Hard Invariants H1-H22 are absolute) — they're how to think about borderline trade-offs.
...
path (40-400ns p99); DECIMAL money (`Money` = `FixedPoint<10,8>`, 16B, exact at venue 8dp — Ship B) + branchless binary fixed-point features (`FPN_Binary<F=64>` = 16B);
...
| `FixedPoint/` | `Money` = `FixedPoint<10,8>` (decimal money, Ship-B) + `FPN_Binary<F=64>` (16B binary features) + certified shared kernels (`umul_128x128_256`/`udiv256_qr`/`divmul_pow10`) + `is_fp_binary_v`/`is_fp_decimal_v` traits + `EngineMoneyT`/`MONEY_ENCODING_EPOCH` |
...
| H4 | **`Money` (decimal `FixedPoint<10,8>`) for ALL money/accounting math** (prices, qtys, fees, balances, money-pct thresholds — exact at venue 8dp; ops saturate + half-even per `Money_*` family); `FPN_Binary<F=64>` for FEATURE math (rolling/regime/ema/signals); money↔feature crossings ONLY at named `Money_ToBinary`/`Money_FromBinary` seams (D-122/D-170); NEVER `float`/`double` on either path (display-only OK) |
...
| **H15** | **Every X-macro registry in the codebase MUST have a row in `FOREACH_REGISTRY` meta-registry** at `CoreFrameworks/MetaRegistry.hpp` (codified `.F.4d` 2026-05-16). Adding a new registry without enrollment fails CI (`tools/check_meta_registry.py` — runs in the doc sweep + at the `/readiness`//ship gates; D-304 name-fix, was cited as a nonexistent `test_meta_registry_coverage`). Closes meta-Class-18 (added registry but forgot to document). See `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`. |
| H16 | ⚠️ **UNENFORCED — an INTENT, not a live guard (2026-08-16).** Both its registry (`FOREACH_DERIVED_FILTER`) and its cited CI check were measured NONEXISTENT at HEAD. Do not rely on it; do not cite it as enforced. Full facts + disposition options → **TECH_DEBT-284**. Codified `.F.4d` 2026-05-16. |
...
| **H19** | **Every `FOREACH_REGISTRY` row with LEVEL > 0 (any non-root registry) MUST declare a valid PARENT** (the registry it sits under). Topology discipline; enforced by `tools/check_meta_registry.py` **Check 3** (name corrected 2026-08-16 — the previously cited `test_meta_registry_topology` never existed, 0 hits tree-wide; same D-304 name-drift class as H15's) (per the enforced `MetaRegistry.hpp` numbering: Level 0 = the codebase-wide root `FOREACH_REGISTRY` / `ROOT_NONE`; Level 1 = a direct registry / PARENT `FOREACH_REGISTRY`; Level 2 = a child of a Level-1 meta-registry; numbering corrected to match code `.E.0.10` 2026-06-11). Codified `.F.4d` 2026-05-16. See `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`. |
...
| **H22** | **Scale-invariance / shard independence — every design works INDEPENDENTLY + scales horizontally across N nodes and N clusters.** A node's (and a cluster's) trading state + logic is a PURE FUNCTION of its OWN local inputs (its resolved cfg incl. `core_N_*` overrides, its positions, its strategy/model, its cluster's shared resources) — NO coupling to other shards or to flattened-global state on the sharded path. **Operational test: adding the (N+1)th node/cluster requires ZERO change to existing per-shard logic.** The premise the whole sharded architecture (+ `.E.1` multi-exchange) rests on. Class 26 (cross-node iteration) / Class 27 (scalar cfg-mirror) / the torn-read class / A1 (warm-restart read GLOBAL `take_profit_pct`, not the node's per-strategy override) are instances of its violation. CI-check candidate: flag a per-shard read of a global cfg field that HAS a `core_N_*` override. Codified 2026-06-11. See `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`. |

**H15-H19 codified at v5.15.5.F.4d ship close 2026-05-16** (per `plans/v5.15-live-readiness/subplans/2026-05-16-v5.15.5.F.4d-merged-framework-bandit-thompson.md` Charter 11 closure). Placement decision: H15/H18/H19 in this CLAUDE.md table (broad framework-discipline visibility); H16/H17 in `DOCS/STRATEGY_AND_CODING_RULES.md` private overlay (structural enforcement detail). See `DOCS/DESIGN_PHILOSOPHY.md` § 2 for the family-grouped narrative discussion of the invariants (the H20–H22 narratives are pending fold into § 2).
...
- Hot-path math: `Money` for money compares/ops, `FPN_Binary<F>` for features; no floats (both 16B `__int128`-backed)
...
| Concern | Skills |
|---|---|
| **Pre-coding plan verification** | `/decision-check` + `/precoding-audit-gate` + `/readiness` + `/second-opinion` |
