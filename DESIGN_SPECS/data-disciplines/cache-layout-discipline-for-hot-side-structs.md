...
load-bearing concern. Per DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md
...
- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md` (uses
  all 3 rules)
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md` (v5.14.10
  precedent; snapshot-side variant of this discipline)
- `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md` (Rule 5 caveat
  for byte-comparison contexts)
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md` (tight-pack arrays
  are AVX-512-friendly)
- `DESIGN_SPECS/data-disciplines/hot-side-array-element-alignment-for-sparse-access.md` (v5.15.5.C.5; **complementary**: Rule 2 here covers TIGHT-PACK for 1-line elements; sister spec covers MULTI-LINE structs where sparse hot-path access reads first-line subset. Same cache-discipline philosophy, different element-size regime.)
