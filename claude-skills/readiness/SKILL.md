...
| 41 | Multi-surface deletion ordering (leaves-first sequencing) | v5.15.5.F.4d.1.B.4 v1.7.5+ (M4 Pillar B14 NEW; sister memory `feedback_multi_surface_deletion_ordering_discipline.md`; B-Plus v0.4 `--gen-deletion-cohort` mechanizes) | sidecar deferred (categorical trigger in CLAUDE.local.md + sister memory + DESIGN_SPECS B14 pillar sufficient at first canonical; sidecar created at 2nd canonical application per file-size-split-discipline.md) |
| 42 | Unconditionalization latent assumption shift | v5.15.5.F.4d.1.B.4 v1.7.5+ (M4 Pillar B15 NEW; sister memory `feedback_unconditionalization_latent_assumption_audit.md`; 1st instance only; Stage 3 deferred per 2-instance threshold) | sidecar deferred (see Check 41 rationale) |
| 43 | Operator-facing doc cohort enumeration when cfg field deletion detected | v5.15.5.F.4d.1.B.4 v1.7.5+ (NEW going-forward rule; sister memory `feedback_operator_facing_doc_cohort_at_cfg_deletion.md`; B-Plus v0.4 generator includes `operator-facing-doc` kind classification) | sidecar deferred (see Check 41 rationale) |
| 44 | Cfg field categorization at registry-add time (4-category decision tree) | v5.15.5.F.4d.1.B.4 v1.7.6+ (sister memories `feedback_cfg_field_categorization_at_registry_add_time` + `feedback_categorize_by_consumer_pattern_not_field_name`; CI Check 8 sister) | `checks/check-44-cfg-field-categorization.md` |
| 45 | Tests-changed section enumeration (modified / broken-replaced / NEW) when coding sequence touches `tests/` | v5.15.5.F.4d.1.D.1+ (NEW; mechanical via `tools/check_plan_body_tests_section.py`; sister memory `feedback_test_change_enumeration_per_plan_body`; M7 9th canonical) | `checks/check-45-tests-changed-section-enumeration.md` |
| 46 | Identifier-retirement + dead-code: plan touching persisted VERSIONs / persisted enum CODES / persisted bits / cfg-field name keys, OR any deletion → run `python3 tools/check_identifier_retirement.py` (tombstone retired slots; never renumber/reuse) + prove-then-remove dead code (`/dead-code-trace`; inline deadness is NOT compiler-caught) | v5.15.5.F.4d.1.E #11 2026-06-02 (NEW; H21 + Class 40; mechanical via `tools/check_identifier_retirement.py` pre-commit Check H; spec `dead-code-and-identifier-retirement-discipline.md`) | sidecar deferred (CI guard + DESIGN_SPEC + H21 sufficient at first canonical) |
| 47 | **Acceptance-oracle totality before delegation (M10)** — for every task this plan DELEGATES to a subagent/fork/parallel lane: name the acceptance oracle and classify it **TOTAL** (fails on ANY deviation — byte-identity vs a reference, an output golden, parity of two independent implementations, a round-trip identity) or **PARTIAL** (tests / green build / lint sweep / "it works"). TOTAL → accept on green. PARTIAL → the plan MUST budget a context-carrying hand-review of the diff before commit; re-running the delegate's own commands does NOT count. Assume PARTIAL until demonstrated otherwise. | `E.1.2.B` `0.3`+ (NEW; spec `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`; sister memory `feedback_delegate_on_total_oracle_handreview_on_partial`; toolchain invariant T12; D-385). Judgment check — deliberately NOT mechanized (oracle totality is a property of what a check covers, not a parseable fact) |
...
- **Deletion-class disciplines (Checks 41-43, NEW v5.15.5.F.4d.1.B.4 v1.7.5+):** M4 implementation-layer blind-spot taxonomy extended pillars (multi-surface deletion ordering B14, unconditionalization latent assumption B15) + sister operator-facing-doc cohort enumeration discipline. Trigger: plan body proposes deletion-class scope (cfg field removal / API surface removal / cohort wrapper deletion / centralized-arch deprecation / etc.); deletion-scope classification per `feedback_multi_surface_deletion_ordering_discipline` drives Check selection. B-Plus v0.4 `--gen-deletion-cohort PATTERN` mechanizes cohort enumeration; `/blindspot-scan B14`/`B15` audits at pre-coding gate enforce; `feedback_operator_facing_doc_cohort_at_cfg_deletion` + `feedback_archived_changelog_preservation_discipline` sister disciplines load at every session.
