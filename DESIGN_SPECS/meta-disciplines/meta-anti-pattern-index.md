...
|---|---|---|---|---|---|
| **AR-1** | Categorical risk-dismissal over an UN-enumerated set ("the rest are exact/safe/identical/unaffected") | `feedback_enumerate_set_before_categorical_claim` (M8-candidate) | reflection: "did I bound scope / dismiss a risk via a property over a set I didn't enumerate + verify member-by-member?" | gate verification pass + harvest | `.E.0.1` R1 — `FromDouble` assumed exact-integer; it wasn't |
