...
3. **Hand cold agents the context — INCLUDING the navigation-infra slice.** Subagents start cold (DESIGN_SPECS/decisions/postmortems do NOT auto-load). Give each: the bug class being hunted, the surface + file paths, the domain rules (e.g. `Money`=decimal vs `FPN_Binary`=features), a structured output shape, and *"cite file:line; READ the actual code; do not assume."* **PLUS — for BOTH I- and A-class — a dependency / blast-radius trace of the surface: what touching it hits DOWNSTREAM + where its data flows IN (the `/dependency-chain-trace` + `/trace-deps` lens; § Nav-infra item 2 below). STANDARD arming, not optional — a surface mapped without its dependency edges is judged in a vacuum. (Operator-directed 2026-06-14: dep-traces are part of how I+A agents are *initialized*. The cohort SKILLS already inherit this via the shared Stage 0 nav-infra consult [enforced by `check_navinfra_cohort_reference.py`]; spelling it into this step makes AD-HOC `Agent`-tool fan-outs carry it too — the gap that prompted the directive.)**
   **PLUS — when the surface has a MATCHING domain-audit SKILL, SPAWN that skill (or an agent that READs + APPLIes its `SKILL.md`), do NOT approximate it with a general agent armed only with the invariant list.** `/dod-audit` for cache-line / bit-packing / false-sharing / registry-fit; `/hft-audit` for hot-path-purity / lock-free sync-point / branchless; `/accounting-audit` for money; `/ml-audit` for the ML pipeline; `/dependency-chain-trace` for blast-radius (already standard arming, above). The skill is a codified catalog walk; an armed prompt is a hint that catches the obvious and misses the specific. (Operator-directed 2026-06-15: arming general agents with H4/H7/H8 got *reasoning about* the invariants, not the DOD cache-line/bit-packing/false-sharing + HFT sync-point catalogs — the dedicated passes surfaced the `FPN_Binary` cfg type / bit-5 / single-writer-ordering the armed prompts missed. `/precoding-audit-gate` already composes the dedicated skills; ad-hoc `Agent`-tool fan-outs must too. → memory `feedback_run_dedicated_audit_skills_not_just_armed_prompts`.)**
