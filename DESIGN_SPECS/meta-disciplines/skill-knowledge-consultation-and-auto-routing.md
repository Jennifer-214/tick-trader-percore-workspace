...
> 6. **Nav-infra — the code-intelligence + impact/data-flow layer** (per `adversarial-multi-agent-audit-methodology.md` § "Nav-infra is a first-class input"). Regen + consult **CODE_MAP** (`./tools/gen_code_map.sh`; `--byte-context`/`--composition` for type surfaces); load the **dependency-graph DAG** + **findings-index** when the sprint has them; and for any finding/edit on an individual function, **trace its data-flow + downstream blast-radius FIRST** (`/dependency-chain-trace chain:<symbol>` → write/read sites by thread+cadence + which writes feed which reads + blast radius; `/trace-deps` for call-sequence). Two purposes, both first-class: **(a)** a completeness lens measures against the REAL surface set, not a hand-recalled one; **(b)** the per-edit question — *what does touching this function hit DOWNSTREAM, and where does its data flow IN from?* (Operator 2026-06-11: this is the line of reasoning the audits must carry — "how we proceed when making edits to individual functions to determine the impact downstream, and how the data flows into it.") This item is the cohort-wide reference; skills cite it, they don't paste a copy (Class-21 avoidance). CI-enforced via `check_navinfra_cohort_reference.py`.
