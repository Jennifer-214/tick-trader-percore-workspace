...
> **`.E.0.10` Net-1 (pre-`.E.1` characterization net) IN-FLIGHT (updated 2026-06-11):** D-190 P&L-gross capital bug FIXED + the **cross-thread torn-read CLASS** found (9 sites, 3 live capital-control) → **`.E.1` is now a HARD LIVE-ENABLE GATE**. **Progress 2026-06-11:** oms-ts-1 fee-exact characterization landed (suite 3359/0); adversarial-default made BINDING (TECH_DEBT-164 part 3 + AR-8/AR-9); context-aware loading shipped (TECH_DEBT-163). Pickup → `handoffs/2026-06-11-E.0.10-net1-adversarial-default-handoff.md` (the ACTIVE handoff; the 06-10 concurrency-checkpoint is superseded).
