...
- And that aggregator-published snapshot IS the foundation the decoupled monitoring plane needs: the **headless engine publishes the snapshot; viewers (the decoupled ImGui viewer / SSH / web) attach + read it  ⟵ `fox-tui` DROPPED per D-427.** Headless monitoring works *because* the viewer reads the published snapshot, not live state.
