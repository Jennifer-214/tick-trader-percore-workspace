...
1. **Python checkers** (exit-code authoritative):
   ...
   - `python3 /home/caramel/code/FoxML_Trader_v2/tools/check_plan_enumeration_completeness.py --plan <plan_path> --section "<enumeration-claim header>" --source-cmd "<gen_code_map/rg cmd>"` — **conditional** (run once per claimed set): when the plan claims an enumeration SET a code-intel tool can produce (relocation set / fee-site set / boundary-cast set / blast-radius), verify plan-set ⊇ tool-output. Catches **AR-1 summarize-and-drop** — the author RAN `gen_code_map`/`rg` then hand-summarized its output, dropping members (the `.E` Session-8 #11 step-6 fold tell; `feedback_paste_tool_output_dont_summarize`). The relocation/fee/boundary set IS the tool output pasted, never digested.
...
2. **Mechanical catalog rows** — apply the CP/WH rows of `meta-anti-pattern-index.md` that have mechanical detection (CP-1 cascade via `/capture-audit` Check 12; WH-1 link-resolution; WH-2 index-pointer). (CP-1 is now mechanical — **RUN** `python3 tools/check_amendment_cascade.py --since <window>`, built 2026-08-16, TECH_DEBT-148 closed. ADVISORY + PARTIAL: it catches a retired phrasing still live in an untouched doc, not an amendment that supersedes by pure addition, so the Check-12 judgment pass still applies on top.)
