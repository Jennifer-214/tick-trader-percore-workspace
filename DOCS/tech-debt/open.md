...
- **PROGRESS (2026-08-12, the 0.5-opener SUBSTRATE landed — ws this commit):** `tools/compile_command.py` is the ONE command source the fix-shape resolves through — exact per-TU invocation(s) from the engine-root `compile_commands.json` symlink (multi-target TUs emit ONE ROW PER ENTRY, never a silent collapse), `--json` = the NEW `compile_command/1` envelope (plugin-exempt-tier'd until the 0.5 card), MISSING = loud row (never a silent default-flags fallback), db-unreadable = rc-2 refusal; 7-teeth selftest on the floor. **Remaining (the closure bar, 0.5 proper):** the CONSUMERS cut over (`check_latency_path_conformance` · `check_cache_layout` probes · `foxtag layout` · the plugin asm view) + provenance stamped into emitted facts + the non-vacuity proof (deliberately-wrong flags ⇒ DIFFERENT answer) + the SHIPPING-build db (`CMAKE_EXPORT_COMPILE_COMMANDS` into `build/`, riding the build.sh 1:1-asm leaf — today's db is `build_clangd/`, editor-parity flags).
- **PROGRESS (2026-08-13, the BUILD half landed — engine `2654ad4`):** (i) `CMAKE_EXPORT_COMPILE_COMMANDS=ON` — every build dir now emits its REAL db at configure; **first look produced live wrong-flags evidence: `build/`'s Release db carries `-DNDEBUG` that `build_clangd/`'s editor db LACKS** — an editor-flag probe compiles live `assert()`s the shipped binary doesn't have (the entry's non-vacuity requirement demonstrated by nature, not planted). (ii) `./build.sh asm` — 1:1 sidecars `build*/asm/<binary>.asm` objdumped from the SHIPPED binaries (under `-flto` the only honest per-function asm — compiler `-S` describes pre-LTO TUs nobody ships), provenance-stamped (binary sha16 + mtime + HEAD; staleness detectable), sanitizer dirs excluded. Verified: 9 sidecars; `Notify_Send` = ZERO standalone copies in shipped `engine_gui` (inlined everywhere) — the honest fact behind the viewer's empty 1:1 render. **Remaining (closure):** consumer cutover (`check_latency_path_conformance` · `check_cache_layout` probes) + provenance stamped into emitted `[DERIVED]` facts + the deliberate wrong-flags tooth. **VIEWER half LANDED 2026-08-13 (plugin, same sitting):** `asmshipped.lua` `<leader>ds` — the shipped-asm card reads the sidecars (FOUND with provenance + cross-binary "also in" · INLINED-AWAY by name · ⚠ STALE banner off the sha16 · refusal without sidecars; newest-binary-first sweep); `asmexplorer` relabeled to the honest "editor-flags approximation" (its -g source-sync is the value the sidecar can't have). Live-smoked: Portfolio_Init 4 blocks/24 call-refs extracted exact; Notify_Send = 8 standalone blocks fleet-wide, inlined-away in engine_gui specifically — the cross-binary sweep resolving the original bare-`0` dogfood case both ways.
