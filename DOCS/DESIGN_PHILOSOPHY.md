...
|---|---|---|---|
| **M1 — Sister-registry parity verification** | Plan body references column on sister registry that doesn't exist at HEAD; or sister registry sig migrated but cohort siblings deferred without rationale | `canonical-sister-extension-discipline.md` § Temporal evolution + cohort migration; `/readiness` Check 36; `/trace-deps` § COHORT-PARITY verification (**landed 2026-07-19** — the row had claimed it since codification; it had never been written) | Class 14 sister-registry-shape-drift instance prevention |
| **M2 — Cross-tool emit-site enumeration** | Wire-format-changing plan enumerates engine code but misses cross-process emitters (CLI tools, training scripts, recording tools) | `wire-format-byte-preservation-discipline.md` Layer 7; `/parity-check` **Section F — cross-tool emit-site enumeration** + future-oriented-plan-template § Wire-format emit surface (**both landed 2026-07-19** — the row had claimed both since codification; neither existed; note the amendment is Section **F**, not E — E is stamp-body schema parity) | Wire-format drift across cross-process emit surfaces |
| **M3 — Anti-pattern codification distinguishes legitimate siblings** | New anti-pattern class text would false-positive on legitimate sibling patterns that match the textual shape but are semantically different | `RECURRING_BUG_PATTERNS.md` codification template requires an explicit "False-positive surface" subsection (**requirement actually written into the template 2026-07-19** — the row had claimed it since codification while the template listed only 6 fields; practice was live from class-25, classes 1-24 backfill = TECH_DEBT-251). M3 still has **no DESIGN_SPEC and no memory** — codification remains partial | False-positive recurrence reports on legitimate patterns |
| **M4 — Implementation-detail audit layer above SHAPE** | SHAPE audits (parity / trace / readiness / merge / dod) return GREEN-or-YELLOW after 3+ iterations; operator senses unaddressed concerns; type-change cascades / field-name collisions / context-dependent C++ constructs / row-order drift remain unchecked | `implementation-layer-blindspot-taxonomy.md` (12-category taxonomy); `/blindspot-scan` skill (Layer-2 audit); `/readiness` Checks 37-40 (NOT 36 — that is M1's; corrected 2026-07-19); `/trace-deps` TYPE-SENSITIVE classification; `/parity-check` claim→evidence + row-order amendments; CI tools `check_field_name_uniqueness.py` + `check_storage_t_coverage.py` | 12 categories of implementation-detail blind spots SHAPE audits miss |
...
| **M6 — Body-content arg enumeration before helper extract** | Plan body specifies helper SIGNATURE based on PROPOSED structural intent without enumerating ACTUAL body content + per-callee parameter requirements; surfaces as cascading "wait this needs more args" at coding time | `body-content-enumeration-at-plan-time-discipline.md` (the canonical spec — the row previously named only the memory, leaving the spec unreachable from the registry; corrected 2026-07-19) + memory `feedback_enumerate_helper_signature_args_before_extract`; `/readiness` Check 33; body-content enumeration CSV artifact at `plan_checks/<date>-<ship>-<helper>-body-content-enumeration.csv` | Helper-extract signature drift at body-content layer (worked examples: BootPerCore v1.6 O1 4→8 args + SlowPathCycleOneCore v1.7.3 N-6 6→9 args) |
| **M7 — Structural enforcement when memory codification proves insufficient** | Memory codification of discipline proves insufficient at next-cycle observation; same bug class recurs DESPITE codified memory at SAME surface in SAME cycle as codification; cognitive-load amplifier present (long files / mechanical amendments / cross-file drift) | `structural-enforcement-when-memory-insufficient.md` (Stage 3 first canonical at .B.4 v1.7.4); 6-stage lifecycle promotion (memory→audit→multi-agent→CI tool→pre-commit hook); `/capture-audit` skill; `tools/check_*.py` + ``.githooks/pre-commit` (the repo sets `core.hooksPath=.githooks`; `.git/hooks/` holds only samples — corrected 2026-07-19)` | Memory-only-discipline failure modes for cognitive-load-amplifier surfaces (worked example: Class 14 Stage 3 → 6 via B-Plus CI tool after v1.7.3 → v1.7.4 cycle introduced 6 NEW fabrications AFTER M6 codification) |
| **M8 — Definition-of-Done contract + armed scout-first subagent verification** | A fix-ship / ship-close is declared on a SUBSET (completeness criteria implicit, discovered-as-you-go) AND/OR verification subagents fire BLIND (boot with only the prompt → catch a subset) → the find-gap→fix-gap loop never terminates | `definition-of-done-and-armed-scout-verification.md` (Stage 3 first canonical at `.E.0.10` A25 close); the enumerated DoD contract (code+producer / test-per-fix / sanitizers / trackers-flipped / parity / promises-honored / obligations-homed / docs-indexed / meta-codified) + full-context scout-first subagents (load reference-docs+toolchain+nav-infra+domain-skill → scout → execute; adversarial-independence on the VERDICT only); `/decision-check` Stage 2.5; candidate CI `check_close_out_completeness.py` | Close-out incompleteness (worked example: the A25 close — F1 AUTO capital bug + stale trackers + untested A28 rode to "closed"; an armed scout-pass caught them; the M7 escalation of AR-8 mechanical-green-≠-semantic) |
| **M9 — Enumerate the set before a categorical risk-dismissal** | A risk is dismissed via a property asserted over a SET ("the rest are exact / safe / identical / unaffected") without the set being enumerated and each member individually checked; generalizes to any conclusion drawn over an un-enumerated population | `feedback_enumerate_set_before_categorical_claim`; cataloged **AR-1** in `meta-anti-pattern-index.md`. **Earmarked `.E.0.10` 2026-06-12; table row landed 2026-07-19.** ⚠️ **PARTIAL codification** — memory + AR-1 catalog only; DESIGN_SPEC / skill amendment / `/readiness` Check NOT yet built (tracked TECH_DEBT-248) | Categorical claims over un-enumerated populations (Class 33) |
| **M10 — Acceptance-oracle totality before delegation** | Delegated work is about to be accepted on "tests pass" / "build green" / "it works"; an acceptance criterion is phrased as BEHAVIOR rather than as a comparison against a REFERENCE; a verification step re-runs the implementer's own commands; a finding is reported as a COUNT of matches rather than read-and-classified matches | `acceptance-oracle-totality-before-delegation.md` (Stage 3 first canonical at `E.1.2.B` `0.3` close); `/decision-check` oracle-classification gate; `/readiness` Check 47; memory `feedback_delegate_on_total_oracle_handreview_on_partial`; toolchain invariant T12. CI stub deliberately OMITTED — oracle totality is a judgment about what a check covers, not a parseable property | Accepting PARTIAL-oracle work as verified (worked example: `0.3` shipped 12/12-green carrying a per-token subprocess storm + a hardcoded repo name in a publishable plugin; and the author's OWN false finding from an unanchored grep — a partial check misleads its own auditor, human included) |

**Aggregating index (2026-05-29; extended 2026-07-19):** M1-M10 above — plus the B14-B15 + B17-B19 implementation-blindspot pillars (`implementation-layer-blindspot-taxonomy.md`) and the workspace-hygiene / cascade shapes — are cross-indexed in `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`, the **META parallel of `RECURRING_BUG_PATTERNS`** (a greppable catalog of NON-code recurring errors; populated by the `/close-session` harvest; consumed by `/capture-audit` Check 12 + the hardened `/precoding-audit-gate`). The enumerate-set discipline (`feedback_enumerate_set_before_categorical_claim`, cataloged there as **AR-1**) was the standing M9-candidate from 2026-05-29 and **was promoted to M9 on 2026-07-19** — its table row above records that its codification is still PARTIAL (memory + catalog only). **M10 was originally written as `M9`** during `E.1.2.B` and renumbered the same day once the earlier earmark surfaced; prose or commits predating the fix may still read `M9` for the acceptance-oracle discipline and resolve to M10. That collision — one citable ID carrying two meanings — is H21's failure mode transposed to the META plane, and nothing currently guards it; see D-385's numbering tombstone.
...
| M5 — Train-serve execution-layer parity (meta-discipline) | (this doc § 11.5) | train-serve-execution-layer-parity.md | — |
| M6 — Body-content arg enumeration before helper extract (meta-discipline) | (this doc § 11.5) | (memory `feedback_enumerate_helper_signature_args_before_extract`) | — |
| M7 — Structural enforcement when memory codification proves insufficient (meta-discipline) | (this doc § 11.5) | structural-enforcement-when-memory-insufficient.md | — |
| M8 — Definition-of-Done + armed scout-first subagent verification (meta-discipline) | (this doc § 11.5) | definition-of-done-and-armed-scout-verification.md | — |
...
- **ExecutionCore** — runtime hot-path execution context WITHIN a Node; per-thread CPU-execution unit; distinct concept from the deployment-unit "Node". **RATIFIED at the `.E.1.1` design (2026-06-17): `ExecutionCore` STAYS (PRESERVE) — NOT renamed by Core→Node.** It names the per-tick execution ENGINE a Node owns (a CPU-execution unit), not the deployment shard, so it reads correctly post-rename; and renaming it would break the standing conformance gate's kernel manifest (`ExecutionCore_Tick`). D-27 renames `CoreContext`→`NodeContext` but the Core→Node rename's PRESERVE-list explicitly excludes `ExecutionCore`/`ExecutionCore_Tick`/`CoreFrameworks`/`MULTICORE` + CPU-hardware `core` (verified: the pinning surface uses `cpu_id`/`CPU`/`SMT sibling`/`physical core`, lexically disjoint from the `core_`/`Core*` shard tokens). **Disambiguation rule:** `core_*`/`Core*` shard ids → node; "core" = CPU hardware → stays core.
