...
v5.14.8     — Stamp body lineage + stale gating            [SHIPPED 2026-05-09]
              ...
              v5.14.8.A.0.b   — pre-flight foundation (BitmapMacros + 3 missing entries)
              v5.14.8.A.merged — Option 1 unification + bit-pack + parser/emitter
              v5.14.8.A.6/.7  — caller migration (~14 files) + round-trip HMAC test
              v5.14.8.B/C     — FOREACH_FAILURE_MODE pseudo-registry + PerCoreSnap migration
              v5.14.8.D       — 5 NEW v5.14.8 stamp body fields via POST_CFG split
              v5.14.8.E       — stale-model age gate + stale_feature_events failure mode
              v5.14.8.F       — CHANGELOG + close TECH_DEBT-006 + open -014 + -015
              v5.14.8         — umbrella tag (commit 165a988)
              Substantially exceeded original scope — closed TECH_DEBT-006 fully (32 fields).

v5.14.7     — Maker order MVP (off by default)             [DEFERRED-INDEFINITE 2026-05-10]
              Permanently deferred — no consistent order book data source.
              See TECH_DEBT-008 (status DEFERRED-INDEFINITE) + Phase 3 note above.
              Original sub-tags preserved in plan file for if-reactivated:
