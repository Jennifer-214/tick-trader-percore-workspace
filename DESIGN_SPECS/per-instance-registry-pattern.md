...
- `DESIGN_SPECS/cfg-scope-discipline.md` (NEW; sister spec) — discipline for choosing WHICH axis a row belongs to
- `DESIGN_SPECS/universal-cfg-field-registry-pattern.md` — parent pattern (universal registry shape); per-instance is an axis-specific specialization
- `DESIGN_SPECS/universal-registry-bitmap-dispatcher-pattern.md` — bitmap dispatcher consumes either registry unchanged
- `DESIGN_SPECS/type-trait-dispatch-via-tt-namespace.md` — tt:: dispatch is registry-agnostic; per-instance dispatch via destination-by-reference
- `DESIGN_SPECS/categorical-tag-applicability-pattern.md` — orthogonal axis (metadata vs structural)
- `DESIGN_SPECS/multi-state-dispatch-with-per-state-update-metadata.md` (NEW 2026-05-14) — orthogonal to per-instance; composes
- `DESIGN_SPECS/wire-format-byte-preservation-discipline.md` — per-instance Layer 5b applies (per-core stamps)
- `DESIGN_SPECS/pattern-codification-lifecycle.md` — workflow this spec followed (Stage 2 DRAFT ahead of first canonical application)
