...
- **Refuse-sentinel, not enum-0** — an unrecognized mode-string returns a refuse value, never the first enum: `BanditAlgorithm_FromString` returns `-1` on unrecognized (`ML_Headers/BanditAlgorithmRegistry.hpp:247`); the `ReconcileMode_FromString` / `ConfigField_Set` model returns a 0/1 KEY-MATCH bool (`CoreFrameworks/Reconcile.hpp:149` — `0` = no-match → caller handles the miss, not a silent enum-0 assignment). The point is a NON-coercing result on no-match, not literally `-1`.
...
- **Refuse-sentinel / key-match returns.** `BanditAlgorithm_FromString` returns `-1` (refuse) on unrecognized input (`ML_Headers/BanditAlgorithmRegistry.hpp:247`); `ReconcileMode_FromString` returns a 0/1 KEY-MATCH bool (`CoreFrameworks/Reconcile.hpp:149`, `0`=no-match) — both are non-coercing on no-match, the fix shape, not the bug.
