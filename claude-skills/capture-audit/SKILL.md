...
- **Mechanical NOW — memory sister-links (D-89 / TECH_DEBT-115):** `python3 /home/caramel/code/FoxML_Trader_v2/tools/check_doc_metadata.py --bidirectional --memories` — a one-way memory `sister_specs:` link is a RED build (exit 1). This closes the sister-link half of CP-1 for memories mechanically (the recurrence that motivated it: 3 memories' forward-refs written, reverse omitted). Sister: `tools/migrate_memory_frontmatter.py` re-derives + symmetrizes sisters from body `[[links]]` at `/sync-workspace`, so derived asymmetry self-heals; this guard catches MANUAL frontmatter asymmetry.
...
- **BUILT 2026-08-16 (TECH_DEBT-148 CLOSED) — RUN IT:** `python3 /home/caramel/code/FoxML_Trader_v2/tools/check_amendment_cascade.py --since <window-start>` (rc 0 clean / 1 candidates / 2 refusal). Wired ADVISORY in `check_session_docs.sh` with a HARD teeth row. It implements the four steps above — diff-detect, term-extract, corpus-grep, historical-record filter — by inverting step 2: it extracts the phrasing the amendment **RETIRED** (the diff's `-` side minus its `+` side), because that is what other docs can still be repeating. Both sets are closed, so the question is decidable.
- **Read its output as a TRIAGE QUEUE, not a verdict.** It is PARTIAL by construction (M10): a green means only *"no untouched live doc repeats a phrasing this window retired"* — an amendment that supersedes purely by ADDING is invisible to it, so the judgment half of Check 12 still applies. Findings are tiered HIGH (citable IDs / identifiers) vs LOW (prose phrasings, frequently ordinary English — the rarity discriminator was measured and failed: `first-order` and `gate-reachability` are equally rare). Work HIGH; browse LOW.
