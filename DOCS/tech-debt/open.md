...
- **🔶 A6 PROGRESS (2026-06-14 — reshaped + half-landed via a 2-I→2-A pre-coding gate; register § "A6 PRE-CODING GATE" + D-220):** the original "clamp ≥0 + post-compute `live_sl<entry<live_tp` assert" is **SUPERSEDED** — the gate found (a) a sign-only clamp MISSES NaN/+Inf (they launder to `+sat` at `FixedPointN.hpp:1897`); (b) the abs-price assert is the wrong lever (the pct clamp is root-cause; entry price unknown at emit); (c) the corrupt-model fallback silently swaps to SimpleDip → **operator-decided 2026-06-14: SHALT-the-node + retrain-warning, NOT degrade**. **EGRESS LANDED** (engine, not committed at this note): `GateParameters_FinalizeEmit` range-validate `[0,10000.0]` + override-first-wins `SHALT_BAD_PCT`, ONE dispatcher call (`StrategyParameters.hpp:1786`, M7-structural — a 7th strategy auto-inherits); suite **3612/0** + 12 non-vacuous char-tests + hot-path CLEAN. **INGRESS REMAINING** (next session, fresh 3-I→3-A swarm): `!isfinite||<0` refuse at `CoreModelZoo_TryLoadRole` (per-arm-drop) + a blend isnan-guard (`:1178-1180`) + the SHALT-the-node + retrain-warning + a new CoreState `MODEL_CORRUPT` flag (distinct from `MODEL_LOAD_FAILED`'s SimpleDip degrade). A5 part UNTOUCHED (rides `.E.1` adapter, D-123). Status: A6 now IN-PROGRESS (was deferred).
...
- **What:** ASan (run on the A6 ingress, 2026-06-15) caught a heap-use-after-free in `SPSCRing_TryPop<OrderEvent<64>,256>` (`CoreFrameworks/SPSCRing.hpp:180`) on thread **T101 = `OrderEventLog_AsyncWriterRoutine`** (`CoreFrameworks/OrderEventLog.hpp:355`). The OMS's async event-log writer thread keeps popping the order-event ring AFTER the OMS is freed: `OrderEventLog_Free(&oms->event_log)` (`tests/controller_test.cpp:26490`) + `delete oms` (`:26491`) on the main thread, while the writer thread is **never joined** → it pops freed memory. Surfaced in the **Ship-B P3 D-173** commission-booking test (`:26450-26492`).
- **PRE-EXISTING — NOT A6.** Introduced in the Ship-B decimal-money work; the A6 ingress doesn't touch OMS/SPSCRing/OrderEvent (confirmed: not in the A6 diff — hunks were 11886/12290/12343/27038). Latent because ASan is a ship-close gate (not per-commit) + the normal suite passes (the freed memory isn't reused enough to fail an assertion). → the gap the V-class codification (`feedback_v_class_post_implementation_verification`) closes.
- **Why / fix:** the async writer thread must be SIGNALED (shutdown flag) + JOINED before the ring it reads is freed. Likely `OrderEventLog_Free` (or `~OrderManagerState`) must stop+join the writer before returning; OR the fixture needs an explicit shutdown call. **VERIFY whether production engine shutdown shares this path** — if so it's a real shutdown-time UAF, not just a test issue.
- **Trigger / BLOCKS:** the **`.E.0.10` ship-close** (asan-green gate) — must be fixed before the `.E.0.10` tag. **Home: the `.E` multi-SPSC rework** (operator-flagged surface; the SPSC/event-log lifecycle deserves focused attention there).
- **Cross-ref:** `OrderEventLog.hpp:355` (the writer routine) · `SPSCRing.hpp:180` · `controller_test.cpp:26450-26492` (the triggering test) · `feedback_v_class_post_implementation_verification` (the discipline that catches this per-implementation, not per-ship).
