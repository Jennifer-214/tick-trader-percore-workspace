...
> **Deep-technical / Tier-2 / specific-work disciplines → `MEMORY_EXTENDED.md`** (on-demand; work-mode skills load it). This always-loaded index = every-turn collaboration / judgment / audit-posture / user / project memories.
...
- [Opportunistic tech-debt closure — subsumption not adjacency](feedback_opportunistic_tech_debt_closure.md) — close debt a ship SUBSUMES (≈0 marginal cost); ADJACENT → cross-link + leave tracked; discriminator = marginal-cost; BIDIRECTIONAL
...
- [Guards compound — enforcement is the highest-leverage investment](feedback_guards_compound_enforcement_is_leverage.md) — a guard protects a whole CLASS forever; code is one instance, the guard is permanent leverage; convention-only on a capital/determinism surface = a hole
...
- [Structural fix over belt-and-suspenders](feedback_structural_fix_over_belt_and_suspenders.md) — prefer the fix that REMOVES a special-case/category-error (fewer moving parts) over a redundant safety layer (bloat); test = add-vs-remove moving parts; don't offer redundant verification layers either
- [Delegate on a TOTAL oracle; hand-review on a PARTIAL one](feedback_delegate_on_total_oracle_handreview_on_partial.md) — classify the acceptance check before delegating; PARTIAL ⇒ context-carrying review before commit, green is NOT done; a partial check misleads its own auditor (incl. me)
...
- [Process weight by surface blast-radius](feedback_process_weight_by_surface_blast_radius.md) — engine/capital → heavy planning+gates; workspace/skill/doc apparatus → light dogfood-iterate; breadth ≠ blast-radius; light=validate-by-use not pre-gate; CARVE-OUT: a fact-producer/verifier toolchain (check_cache_layout/foxtag) is correctness-critical despite apparatus-plane (one-producer-N-consumers)
...
- [Delegate via the locked spec at implementation](feedback_delegate_via_locked_spec_at_implementation.md) — design=hands-on solo (taste/decisions un-delegatable); implementation=agents INFORM, main session WRITES; NEVER delegate code authoring; the docs ARE the delegation substrate
...
- [Name members, never tallies, in lasting docs](feedback_name_members_never_tallies_in_docs.md) — handoff/plan/MASTER state ANCHORS (SHA/id/member-list/re-derive cmd) not counts; a tally is stale on the commit that writes it and fails SILENTLY; counts live in re-derive fences only
...
- [Adversarial framing default for checks](feedback_adversarial_framing_default_for_checks.md) — audit/check/review default to ADVERSARIAL (FIND/REFUTE not confirm) + independent cross-check; BINDING; self-check = the opt-out with a reason; why → [[feedback_capture_and_check_are_model_bounded]]
...
- [Public snapshot attracts hedge funds; current work now PRIVATE](user_public_work_attracts_hedge_funds.md) — the EXISTING public snapshot is career-load-bearing; going-forward diverged to ALL-PRIVATE (2026-07-06), so the visibility pressure applies to what's out there, not new work
...
- [Viewer is ImGui, decoupled — not a TUI](project_viewer_is_imgui_decoupled_not_tui.md) — D-427 reverses the GUI hard-deprecate; `fox-tui` DROPPED; ⚠️ EngineTUI's TUISnapshot CONTRACT survives (it becomes the mmap region), only the renderer dies
