...
- **Disposition:** DELETE the dead branches as part of ③/D-253 (verified-at-close: grep confirms 0 reachable). KEEP the genuinely-live manual branches (HAS_SIDE_EFFECT/MANUAL_PARSER rows like `fee_rate_maker`/`taker` + the non-registry `min_kill_loss`/`starting_balance`/`min_sl_tp_ratio`/`min_hold_gain_pct`).
- **Cross-ref:** D-253 · H21 (`dead-code-and-identifier-retirement-discipline`).
