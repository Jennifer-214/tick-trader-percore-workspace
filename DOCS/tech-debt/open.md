...
- **⚠️ Depends on TECH_DEBT-257** (compile_commands 1:1) — an AST is only as correct as the flags it parses under, so the flag-provenance fix lands first or the matchers analyse a translation unit nobody builds.
