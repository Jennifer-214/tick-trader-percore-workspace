...
| `check_capital_adversarial_audit.py` | STANDING-CI | `check_session_docs` (advisory) | — | TECH_DEBT-164 part B / AR-8 failsafe — standing scan: every capital TEST block carries `// ADV-REFUTE` (independent refute ran) or `// ADV-SELF` (opt-out + reason); guards the binding adversarial-default against self-attestation |
