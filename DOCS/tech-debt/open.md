...
- **Why / fix:** publish the enable-flags at restore (set `cached_params.flags` TP/SL-enabled in the restore loop), OR prime one slow cycle before hot-thread spawn (boot-ordering). Either closes the window. Net-1 (wfa-1) characterizes the current warmup-inert behavior (warmup-disarm witness) + the armed-exit witness, with a coverage disclaimer.
