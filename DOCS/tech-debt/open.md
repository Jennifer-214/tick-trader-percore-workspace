...
- **Cross-ref:** D-359 (the gate) · D-360 (the policies + `[DEPRECATED]` vocab) · D-361 (foxtag reconciliation) · D-362 (calibration corpus) · C-359 (codegen-write corpus-clean → C5 is a regression guard) · D-349 (the migration contract) · plan § COMPLETENESS-GATE · TECH_DEBT-238 (sister tools-audit-cadence coverage-gate — the latency-budget denominator, built the same way on the same `foxtag units --tag` enumeration).
