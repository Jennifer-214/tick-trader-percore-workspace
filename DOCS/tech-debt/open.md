...
- **id:** TECH_DEBT-164 · **severity:** low · **opened:** 2026-06-10 · **status:** open (part 3/3 DONE 2026-06-11; parts 1+2 remain) · **surface_tags:** [audit-methodology, anti-pattern-catalog, design-spec, skill-default, M7, doc-system]
- **Title:** The `.E.0.10` D-190 saga codified the IMMEDIATE lessons (3 memories + D-190 + PARITY-038 + Landmine 8 + the structural guard) but left 3 DEEPER process codifications: (1) EXTEND the AP4 anti-pattern class ("absent/implicit rounding-mode on money mul/div") → also "single-source the COMPUTATION, not just the mode, for any money value derived ≥2 ways"; (2) NEW DESIGN_SPEC `characterization-test-discipline` (the complete / non-vacuous / not-frozen-bug 3-lens pass); (3) M7 structural — wire the ADVERSARIAL framing default (`feedback_adversarial_framing_default_for_checks`) + the 3-lens pass INTO the audit/review SKILLS so they fire by default, not just from memory.
- **Created:** 2026-06-10 — the codification queue from the `.E.0.10` finding-disposition register (process-error retro). Memories capture the WHAT; the skill-defaults + DESIGN_SPEC make it STRUCTURAL (M7 — the recurrence was despite codified memory).
- **Why DEFERRED (merit, NOT effort):** skill-cohort edits (audit/review `SKILL.md` files) + a new DESIGN_SPEC + an anti-pattern-class amendment are a focused doc/skill pass, distinct from the engine fix (DONE + guarded). Best done as one codification pass (sister-cohort completeness) rather than piecemeal mid-ship. Behavioral risk zero (process docs); cost of delay is only that the adversarial-default relies on memory until the skills enforce it.
- **Cost estimate:** ~2-3h; AP4 amendment + 1 DESIGN_SPEC + the skill-default edits across the audit-skill cohort (+ `check_doc_metadata --bidirectional` re-symmetrize).
- **Trigger:** a dedicated codification pass, OR the next audit-skill edit (wire the default in then, per the skill-edit-cohort-checklist), OR the `.F` professionalization sweep.
- **Cross-ref:** D-190; `feedback_adversarial_framing_default_for_checks`; `feedback_passing_test_is_not_verification`; M7 (`feedback_structural_enforcement_when_memory_insufficient`); AP4 (`RECURRING_BUG_PATTERNS` / #11 anti-pattern set); the `.E.0.10` register codification queue.
- **UPDATE 2026-06-11 (`.E.0.10` Net-1 close) — PART 3 of 3 DONE (adversarial-default into skills):** methodology spec → BINDING DEFAULT; the shared consult discipline (`skill-knowledge-consultation-and-auto-routing.md` § Default EXECUTION framing) carries the adversarial-default rule the audit skills inherit at Stage-0; auto-fire failsafe `tools/check_capital_adversarial_audit.py` (+ D-137 self-test, wired into `check_session_docs`) flags un-refuted capital tests; AR-8 + AR-9 catalogued; `feedback_adversarial_framing_default_for_checks` made BINDING. **STILL OPEN: part 1** (AP4 anti-pattern-class extension) + **part 2** (the `characterization-test-discipline` DESIGN_SPEC — proposed this session, not yet written). NOTE: a DUPLICATE TECH_DEBT-164 was mistakenly opened in `TECH_DEBT.md` this session (engine doesn't symlink `tech-debt/` so the real ledger was invisible) — REMOVED; the `/close-session` Stage 5.5 independent review caught it.
