...
- **id:** TECH_DEBT-226 · **severity:** LOW-MED · **opened:** 2026-06-30 · **status:** open · **surface_tags:** [oms, comment-drift, layout, order, subagent-arming-2.5]
- **What:** descriptive size/layout comments in `CoreFrameworks/Order.hpp` drifted from the code (surfaced by the E.1.2 constraint-driven-layout coverage i-class + the capture-completeness c-class; confirmed at HEAD `0ee227a`, per SUBAGENT_ARMING §2.5 comments-are-point-in-time): `:137` "Sized at **32 B** (2 × FPN_Binary)" + `:193` "**32 B** @ 160 (Ship-A 16B: 48→32)" are STALE — the live `static_assert(sizeof(OrderPreResolved<64>) == 48)` (`:149`) is truth (A25 added `Money tp_pct` → 3×16B = 48B). The same cluster continues at `:193-199` ("HOT subtotal: **192 B** / exactly 3 cache lines" + "exchange_id … @ **192**") — invalidated by the 48B `pre_resolved` (HOT is now 208B; `sizeof(Order<64>)==272` per `:404`). The `static_assert`s are all CORRECT; only the human comments drifted.
- **Severity rationale:** `OrderPreResolved`/`Order` are in-flight SPSC-only, NOT persisted → no wire/H21/determinism concern. Pure doc-drift; misleads a reader/agent (the §2.5 failure mode — a subagent nearly trusted a sibling stale comment this very session) but breaks nothing.
- **Disposition / trigger:** fold into the **E.1.2 relayout** (which re-derives the `Order`/serializer surface anyway — a natural fix-in-place) OR any next `Order.hpp`-touching ship; per §2.5 propose the corrected comment (reference the `static_assert`, don't restate the digit). If E.1.2 registry-drives the serializer off `FOREACH_POSITION_FIELD`, apply the same "derive the offset asserts, don't hand-comment sizes" discipline to `Order`.
- **Cross-ref:** SUBAGENT_ARMING §2.5 · `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md` (CODEGEN/size row) · `feedback_comments_point_in_time_verify_against_code` · `plan_checks/2026-06-30-E.1.2-per-field-declaration-table.md` §6 · `Order.hpp:137/149/193-199/404`.
