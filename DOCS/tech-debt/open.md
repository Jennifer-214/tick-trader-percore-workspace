...
- **🔶 A6 PROGRESS (2026-06-14 — reshaped + half-landed via a 2-I→2-A pre-coding gate; register § "A6 PRE-CODING GATE" + D-219):** the original "clamp ≥0 + post-compute `live_sl<entry<live_tp` assert" is **SUPERSEDED** — the gate found (a) a sign-only clamp MISSES NaN/+Inf (they launder to `+sat` at `FixedPointN.hpp:1897`); (b) the abs-price assert is the wrong lever (the pct clamp is root-cause; entry price unknown at emit); (c) the corrupt-model fallback silently swaps to SimpleDip → **operator-decided 2026-06-14: SHALT-the-node + retrain-warning, NOT degrade**. **EGRESS LANDED** (engine, not committed at this note): `GateParameters_FinalizeEmit` range-validate `[0,10000.0]` + override-first-wins `SHALT_BAD_PCT`, ONE dispatcher call (`StrategyParameters.hpp:1758`, M7-structural — a 7th strategy auto-inherits); suite **3612/0** + 12 non-vacuous char-tests + hot-path CLEAN. **INGRESS REMAINING** (next session, fresh 3-I→3-A swarm): `!isfinite||<0` refuse at `CoreModelZoo_TryLoadRole` (per-arm-drop) + a blend isnan-guard (`:1155-1158`) + the SHALT-the-node + retrain-warning + a new CoreState `MODEL_CORRUPT` flag (distinct from `MODEL_LOAD_FAILED`'s SimpleDip degrade). A5 part UNTOUCHED (rides `.E.1` adapter, D-123). Status: A6 now IN-PROGRESS (was deferred).
