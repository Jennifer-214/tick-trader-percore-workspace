...
| H17 | Cfg struct field declarations MUST come from `FOREACH_CFG_FIELD` via X-macro generation; manual cfg field declarations FORBIDDEN. Runtime/derived state stays manual but is documented in `MANUAL_FIELDS_INVENTORY.md` with rationale. CI build-fails on drift via `tools/check_per_core_registry_integrity.py`. | **STRONG at .F.4c.3 (per-core surface: `PerCoreCfg<F>` X-macro generated + `FOREACH_MANUAL_PER_CORE_FIELD` exemption inventory + CI cross-check)**; HARD at .F.4d (global surface extension; `FOREACH_GLOBAL_CFG_FIELD` X-macro struct gen + `FOREACH_MANUAL_GLOBAL_FIELD` exemption inventory) | DESIGN_SPECS/manual-fields-inventory-pattern.md (Stage 2 DRAFT at .F.4c.3 WIP2d-0); DESIGN_SPECS/universal-cfg-field-registry-pattern.md § Reverse-drift |
