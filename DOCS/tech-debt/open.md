...
- **Cross-ref:** meta-anti-pattern-index **WH-2** (the recurring shape — this is its Nth recurrence → M7 escalation) + **M7** (`structural-enforcement-when-memory-insufficient.md`); `/capture-audit` + `/close-session` (the surfaces to extend); `check_handoff_active_singleton.py` (the sister guard this composes with — singleton-of-active + MASTER-points-to-it); **D-385/M9** (same session — the TOTAL-vs-PARTIAL oracle; this guard is a TOTAL one, which is why it fired where a partial self-check did not).
