...
Consequence homed, not done: the `TUI` prefix then names a thing that's gone, but renaming needs its own leaf. The pin is NOT `tools/lib/cache_layout_baseline.txt` (measured: zero hits — that claim was wrong when first written); the pins that actually bite are `DataStream/EngineTUI.hpp`'s in-code `[ASSERT]_[LAYOUT_LOCK]` on `offsetof(PerNodeSnap, …)` and — the one that would fail SILENTLY — `tools/tsan_suppressions.txt`'s **name-keyed** `race:TUISnapshot_ReadInto` / `race:TUISnapshot_Publish_End`: rename the struct and those stop matching, turning a by-design seqlock race into a live TSAN failure. H21 doesn't bite — in-process struct names, not persisted identifiers.
