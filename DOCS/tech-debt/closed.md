...
### TECH_DEBT-125 — TOMBSTONE (two-meaning slot collision; SPLIT → -266 / -267 per D-409; slot RESERVED, never reuse)

- **id:** TECH_DEBT-125 · **severity:** LOW · **status:** tombstone · **surface_tags:** [ledger-hygiene, H21]
- **Disposition (operator identifier decision at the (f) triage; written 2026-08-07):** the slot was claimed by TWO unrelated meanings and neither ever got a defining row: **(A)** the `.B.6` subfolder-split pattern anchor — cited from `ROADMAP-2026-05-17-to-paper-test.md:80`, the `.B.4` frontmatter (`:31`), `.B.5:410`, and `.B.6:57/:343` — re-homed to **TECH_DEBT-266** (closed; the pattern landed and was proven at `.B.6`). **(B)** the DriftHistory ring→streaming-aggregate deferral — decision log `v5.15.5.F.4d.1.E-architecture-v2.md`, under D-297 — re-homed to **TECH_DEBT-267** (open; live scale-forward deferral). Historical citations resolve HERE and forward by meaning; the one LIVE site (the decision log) carries an inline re-home annotation.
- **Why a tombstone, not a pick-one (H21):** letting either meaning keep the slot silently re-points the other's citations at unrelated content — the doc-plane Knight-Capital slot-reuse failure. One id must mean exactly one thing; the slot is RESERVED permanently. Split executed per **D-409** (fresh INT ids + `split_from:`, never a suffix).
- **Cross-ref:** TECH_DEBT-266 · TECH_DEBT-267 · TECH_DEBT-101 (the sister re-home precedent) · D-409 · D-401.

### TECH_DEBT-266 — Subfolder-split pattern anchor (`.B.6` first canonical) — re-homed from the -125 collision; CLOSED as originally recorded

- **id:** TECH_DEBT-266 · **split_from:** TECH_DEBT-125 (meaning A) · **severity:** low · **opened:** 2026-05-25 (`.B.6`, as "-125") · **status:** closed (at open — the origin records "opened + immediately CLOSED (pattern application proven)") · **surface_tags:** [doc-discipline, refactor-pattern, file-size-split, ledger-hygiene]
- **What (transcribed from the citing sites — the slot never had a defining row):** the `EngineSharded.hpp` subfolder split (`EngineSharded/{Run,Async,HotSwap,Boot}.hpp`) as FIRST CANONICAL of the per-class subfolder pattern; `.B.5` cross-refs it as the test-side sister; sister to the TECH_DEBT-116 doc-split closure shape. The pattern application was proven at ship close; the entry existed only as plan-body claims until this row.
- **Cross-ref:** TECH_DEBT-125 (tombstone; the collision record) · `.B.6` plan (`2026-05-25-v5.15.5.F.4d.1.B.6-enginesharded-subfolder-split.md:57,:343`) · `file-size-split-discipline.md` · Class 32 (mega-file accumulation — the class the split closes).

### TECH_DEBT-268 — FPN sign-field elimination (v5.14 "Option C", plan-local id `034b`) — SUPERSEDED BY SHIP-A; the outcome landed by a different mechanism

- **id:** TECH_DEBT-268 · **severity:** low · **opened:** 2026-05-11 (plan-local `034b`, `v5.14.11.B`) · **status:** closed (2026-08-07 — superseded) · **surface_tags:** [numeric-core, fixed-point, ship-a-migration, ledger-hygiene]
- **Origin + why the number changed:** minted PLAN-LOCALLY as `034b` in `plans/v5.14-foxml-port-and-maker/subplans/2026-05-11-v5.14.11.B-branchless-math-mega-bundle.md:71` ("FPN sign elimination via w[N-1] high bit packing") and later promoted to a GLOBAL citable id by `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md` citing it with the ledger prefix — the exact plan-local→global scope-promotion the F-M1 prefix convention now exists to prevent (AR-14 shape 4). Per **D-409** (the letter-suffix id form is RETIRED; `-175a` alone grandfathered), the promotion is honored with a FRESH INT id rather than a second suffixed key; the citing site is repointed to this row in the same change. The `034b` string survives only in the frozen v5.14 plan and this provenance note.
- **Why CLOSED-SUPERSEDED (code-verified 2026-08-07):** the debt's outcome — eliminate the FPN sign FIELD — LANDED at Ship-A by a different mechanism than proposed: `FPN_Binary<64>` is a 16B `__int128 v` two's-complement core, "sign in the top bit; no sign field, no padding" (`FixedPoint/FixedPointN.hpp:43,:76-77`). The proposed `w[N-1]` high-bit packing targeted the pre-Ship-A 24B sign-magnitude representation, which no longer exists on the live path.
- **Residue, homed elsewhere:** the generic `FPN_*` op-family bodies still carry pre-Ship-A `.w[]`/`.sign` internals — tracked at **TECH_DEBT-242**; nothing remains here.
- **Cross-ref:** TECH_DEBT-242 · Ship-A / D-163 · `struct-padding-determinism-pattern.md` (the repointed citing site) · D-409 · F-M1 · AR-14.
...
---
- **Moved:** 2026-08-10 open.md → closed.md (STRAGGLER re-file — entry already read closed/resolved in its own text; c-class-verified set 2026-08-09; task #6)
...
- **Status:** CLOSED (2026-08-10 fired-trigger triage)
...
- **Closed:** 2026-08-10 — CLOSED-by-.B.4 (verified 2026-08-10 by code-read: CoreFrameworks/EngineCommon.hpp exists with EngineCommon_BootPerCore + EngineCommon_SlowPathCycle*; Version.hpp .B.4 changelog records the extract + 'closes PARITY-026/027/028/029/030/031'). The entry never moved at ship close — filed now at the D-416 fired-trigger triage.
...
- **Closed:** 2026-08-10 — CLOSED-SUPERSEDED (verified 2026-08-10): parity_harness RETIRED at .E.1.1 (its comparison a tautology after single_core deletion); BOTH compared arches (centralized + single_core) are GONE — sharded is the only path, and the .B.4 EngineCommon shared helpers made train-serve mirror byte-identical BY CONSTRUCTION (this entry's own two-step-close design, step 1). Residual explicit-regression desire folds to the M5 train-serve parity discipline (E-series).
...
- **Closed:** 2026-08-10 — CLOSED-by-landed-work (verified 2026-08-10 by code-read: Version.hpp:17-25 carries the EXACT two-axis design this entry specifies — internal ENGINE_VERSION ship-tag [wire-bound, preserved] + public RELEASE_VERSION_STRING '0.3' display-only pre-1.0. Landed without the entry moving). Note: repo went ALL-PRIVATE 2026-07-06, softening the external-positioning urgency the entry was opened under.
...
- **(c) CLOSED 2026-08-10 — the FIRST `stage: 6-cadence-locked` promotion, on EVIDENCE (never vocab-filling).** The entry's own named candidate `dead-code-and-identifier-retirement-discipline.md` meets the stage-6 definition mechanically: standing CI (`tools/check_identifier_retirement.py`) + pre-commit Check H + the golden retirement ledger + the H21 CLAUDE.md row — enforcement AND cadence both live. Frontmatter promoted (dated); indexes regenerated; the `CLAUDE.md` stage-6 grep recipe now returns a real member — the dangling promise resolved in the FORWARD direction (the spec earned the rung; the vocab did not conscript the spec). Operator-veto surfaced at promotion (2026-08-10 session). Verified in passing: (a)'s untyped-exclusion NOTE prints (memory-template named aloud, excluded by design) and regen is 0-diff-idempotent.
- **Status:** CLOSED (2026-08-10 — (a)(b) at `bda3475`; (c) this closure)
- **Closed:** 2026-08-10 — all three legs terminal.

### TECH_DEBT-258 — the plugin needs a registry-driven consumer surface, or every new tool becomes a bespoke integration

- **id:** TECH_DEBT-258 · **severity:** med · **opened:** 2026-07-20 · **status:** closed · **surface_tags:** [toolchain, plugin, framework-discipline, extensibility]
...
- **Status:** CLOSED (2026-08-12 — the full fix-shape landed; E.1.2.B plugin arc, operator-routed session)
- **Closed:** 2026-08-12 — all three fix-shape legs terminal; drift now loud at FOUR layers (runtime / :checkhealth / plugin suite / repo floor). **(1) producers as DATA** = `lib/toolio_schemas.json` (pre-existing, D-380/D-384). **(2) the plugin's consumed set as DATA, made LOAD-BEARING** = `toolio_kinds.lua` M.consumed/M.exempt (plugin `9d6c98b`) + `assert_consumed` wired at BOTH decode seams — docview `_decode` (defining_site/1 + cited_path/1, the refusal notify carries the reason) and nodemodel `decode` (grammar/1; live foxtag emit verified) — plugin `3566412`; an unregistered or wrong kind is a NAMED runtime refusal, never a wrong decode. `:checkhealth` renders the parity (tri-state; unreadable registry = refusal per Class 57). **(3) the parity assert at the floor** = `tools/check_toolio_kind_parity.py` (6-teeth `--selftest`: planted missing-consumer/ghost/stale-exempt FIRE; mutilated-Lua + missing-json = rc-2 REFUSAL, never empty-pass) wired **HARD** in `check_session_docs.sh` (teeth + live rows, 2026-08-12). The fix-shape's `parity_check.sh` home re-pointed to the live floor (that harness PARKED at D-415). "Derive views from the registry" delivered as consumed-set-as-data + the runtime assert — a renderer is code and cannot derive from a schema; the entry's own operationalization ("add a tool = a data edit on both sides") now holds with BOTH omissions loud (the exempt tier = the producer-first escape hatch, reason required, itself parity-checked).
...
- **id:** TECH_DEBT-257 · **severity:** **high** · **opened:** 2026-07-20 · **status:** closed · **surface_tags:** [toolchain, derived-facts, static-analysis, correctness, plugin]
...
- **Status:** CLOSED (2026-08-14 — every fix-shape leg terminal; E.1.2.B 0.5, the shipped-asm arc)
- **Closed:** 2026-08-14 — the flag boundary is sealed at every consumer. **(1) probe flags from the build's own db:** `check_latency_path_conformance` DERIVES from `build/compile_commands.json`'s engine entry (the pinned list it replaced had drifted on FOUR count-affecting axes — c++20-vs-gnu++17 · `-funroll-loops` · `-DNDEBUG` · the target defines — every prior count described a TU nobody ships); the plugin's `flags_for` (sizeprobe → layout probes + asmexplorer) and `compile_command.py` both resolve SHIPPING-FIRST (`build/`'s db, regenerated every configure) with the root symlink as the named editor-liveness fallback — the best-of-both split: FACTS read shipped truth, the EDITOR keeps its liveness db. The asm CARD goes further: it reads the `./build.sh asm` sidecars — the LINKED binary itself, 1:1 by construction under `-flto`. **(2) LOUD-fail:** latency checker rc=2 on a missing db (never silent defaults — the entry's own words); `compile_command.py` REFUSAL rc=2; the header-no-entry case is a NAMED MISSING (headers are not TUs — the enclosing-TU mapping stays homed as 0.5 design work, per this entry's original scope 'for the TU being analysed'). **(3) provenance asserted in the emitted facts:** the checker banner + the blessed budgets' `_provenance` key (db/mtime/cxx/flags — a future flag change gets a named advisory, never a mystery ratchet RED); sidecar headers carry binary-sha16/mtime/HEAD; `compile_command.py` prints which db answered. **(4) non-vacuity:** the wrong-flags tooth — the same probe under `-O0` measures 82-vs-30 instructions (flags proven load-bearing); nature supplied the field evidence first (`-DNDEBUG` present in the shipping db, absent in the editor db — live wrong-flags divergence on first contact). Budgets re-blessed by the operator at a TTY under the real flags (hot-path `ExecutionCore_Tick` HOLDS; slow-path shifts = measurement honesty). Compile-time visibility: `./build.sh test` chains the conformance REPORT (non-fatal; the gate stays pre-commit; bless stays TTY — now also reachable in-editor via the menu's terminal bless rows, a real pty). Cross-ref: D-419 · TD-256 (unblocked consumer) · ideas §2/§12 · engine `2654ad4`/`dc6affc` · ws `a328418`.
...
- **id:** TECH_DEBT-227 · **severity:** MED · **opened:** 2026-07-01 · **status:** closed · **surface_tags:** [oms, persistence, warm-restart, partial-exit, accounting, capital]
...
- **CLOSED 2026-08-15 at v5.15.5.F.4d.1.E.1.2** — by the exact mechanism this entry named as its own preferred trigger (*"the E.1.2 `FOREACH_NODE_PERSIST_FIELD` registry-drive — if the serializer is registry-driven there, these become persist rows automatically"*). Both halves are resolved, and differently:
  - `partner_pending_pnl` → **persisted**, wire row `MemHeaders/NodeCtxPersistRegistry.hpp:87` (`SCALAR/COMMIT`), present in `tools/goldens/node_persist_layout.txt`. Landed in the v10→v11 delta (D-420) as the paired half of the net-0 `node_dd_pct` swap.
  - `partner_pending_bitmap` → **deliberately NOT a wire row**, and that is now stated in the registry itself (`:76-77`): it is `EventLoopState`-level and RE-DERIVED on load via slot parity. A reason, at the field, that a reader can check — not an omission.
- **Found stale 2026-08-15**, by the pre-commit tech-debt overlap hook during D-421 step 2 — the trigger had fired at `.E.1.2 B` and the entry was never moved. Same shape as the ledger drift found the same session (counts reading `83/2/25` against a real `202/0/76`; two entries pointing at an empty `in-flight.md`): a disposition that lives anywhere other than the SSoT, flipped by hand, eventually isn't. → `feedback_tag_disposition_at_fix_time` · [[TECH_DEBT-272]].


### TECH_DEBT-196 — Per-core snapshot save/load block lacks a field-count-lock (silent wire-desync class)

- **id:** TECH_DEBT-196 · **severity:** medium (silent persistence desync; paper-mode continuity) · **opened:** 2026-06-14 · **status:** closed · **surface_tags:** [persistence, wire-format, ci-guard, count-lock, H9, H21, .E.1]
...
- **id:** TECH_DEBT-167 · **severity:** medium · **opened:** 2026-06-11 · **status:** closed · **surface_tags:** [accounting, H4, partial-exit, sizing, money, .E.1]
...
- **id:** TECH_DEBT-231 · **severity:** LOW · **opened:** 2026-07-05 · **status:** closed · **surface_tags:** [ci-tooling, doc-pipeline, cache-layout-gate, plugin, clang, gpp, E.1.2.A]
...
- **Closed:** 2026-08-19 (moved open → closed via check_tech_debt.py --close)
