...
**Scope at this ship:** per-core surface (`PerNodeCfg<F>` runtime cluster + parallel arrays on `ControllerConfig<F>`)
**Extension at .F.4d:** global cfg surface (`ControllerConfig<F>` global fields + manual global exemptions)

## Discipline

Every parallel-array entry below MUST have a corresponding row in `FOREACH_MANUAL_PER_NODE_FIELD` X-macro at `CoreFrameworks/CfgFieldRegistry.hpp`. Every `PerNodeCfg<F>` runtime-cluster field MUST be in the permitted list below.

CI cross-checks bidirectionally via `tools/check_per_node_registry_integrity.py` (invoked from `build.sh`); missing in either source = BUILD ERROR.

Adding a new exemption:
1. Add row to this table with full justification
2. Add row to `FOREACH_MANUAL_PER_NODE_FIELD` X-macro (parallel arrays) OR to the PerNodeCfg<F> runtime-cluster permitted list (bitmap fields)
3. Commit message explicitly justifies why the field can't fit `FOREACH_PER_NODE_CFG_FIELD` registry
4. If TRANSITIONAL: specify the ship that closes the exemption

Removing an exemption:
1. Confirm migration trigger fired (field now lives in `FOREACH_PER_NODE_CFG_FIELD` or has been deleted from codebase)
...
## Section A — `ControllerConfig<F>` parallel arrays (FOREACH_MANUAL_PER_NODE_FIELD entries)

| Field | Type | Suffix | Storage | Rationale | Migration trigger |
|---|---|---|---|---|---|
| `node_model_path` | `char` | `[256]` | `char node_model_path[16][256]` | Per-core legacy single-model path (used when ensemble dir not set). `char[N]` arrays don't fit current KIND enum spectrum (KIND_FILE_PATH ships at `.F.4e`). | `.F.4e` KIND_FILE_PATH cohort |
| `node_model_dir` | `char` | `[256]` | `char node_model_dir[16][256]` | Per-core model directory paths for ensemble auto-discovery. Same KIND_FILE_PATH constraint. | `.F.4e` KIND_FILE_PATH cohort |
| `node_horizon_list` | `char` | `[128]` | `char node_horizon_list[16][128]` | Per-core ensemble horizon CSV string. KIND_STRING constraint. | `.F.4e` KIND_STRING cohort |
| `node_ensemble_blend_mode` | `char` | `[16]` | `char node_ensemble_blend_mode[16][16]` | Per-core ensemble blend mode name string. May migrate to KIND_INT_ENUM at .F.4d if blend modes become enum-driven. | `.F.4d` (KIND_INT_ENUM) OR `.F.4e` (KIND_STRING) |
| `node_disabled_horizons` | `char` | `[128]` | `char node_disabled_horizons[16][128]` | Per-core disabled-horizons CSV string. KIND_STRING constraint. | `.F.4e` KIND_STRING cohort |
| `node_symbol` | `char` | `[32]` | `char node_symbol[16][32]` | **WIP2d-1.A** (.F.4c.3.A partial advance) — per-core symbol string (e.g., "BTCUSDT"). Operator-facing forward-compat for multi-symbol DataStream (when ready). Boot-time uniformity check enforces all active cores share symbol until DataStream multi-symbol support ships. Bridges to legacy BinanceConfig.symbol at boot init (main.cpp pre-EngineSharded_Run). | `.F.4e` KIND_STRING cohort + full .F.4c.3.A migration (UI design + multi-symbol DataStream) |
| `node_feature_mask` | `uint64_t` | (none) | `uint64_t node_feature_mask[16]` | Per-core feature bitmap (64-bit; one bit per FOREACH_FEATURE entry). Currently no Kind for typed hex64 cfg values. | `.F.4e` KIND_HEX64 cohort |
| `node_risk_pct` | `FPN<F>` | (none) | `FPN<F> node_risk_pct[16]` | **TRANSITIONAL** — duplicate source-of-truth during shadow window. `cores[c].risk_pct` (in FOREACH_PER_NODE_CFG_FIELD registry) is authoritative; `node_risk_pct[16]` survives during shadow for legacy `core_<N>_risk_pct=` parser path. `PopulateCoresFromFlat` syncs node_risk_pct[c] → cores[c].risk_pct. | `WIP2g` (this ship) — delete legacy array; rely on `[core N]` parser (Step 3) to write directly to `cores[c].risk_pct` |
| `node_strategies` | `uint8_t` | (none) | `uint8_t node_strategies[16]` | **TRANSITIONAL** — same shadow pattern as `node_risk_pct`. `strategy` row added to FOREACH_PER_NODE_CFG_FIELD at WIP2d-0 (Finding 1 closure); auto-generates `cores[c].strategy`. Legacy array survives shadow for `core_<N>_strategy=` parser path. | `WIP2g` (this ship) — delete legacy array |
| `node_time_exit_ticks` | `uint32_t` | (none) | `uint32_t node_time_exit_ticks[16]` | **TRANSITIONAL** — legacy per-core override of `max_hold_ticks`. `cores[c].max_hold_ticks` is authoritative per registry. Shadow walker syncs. | `WIP2g` (this ship) — delete legacy array |
| `node_max_drawdown_pct` | `FPN<F>` | (none) | `FPN<F> node_max_drawdown_pct[16]` | **TRANSITIONAL** — legacy per-core override of `max_drawdown_pct`. `cores[c].max_drawdown_pct` is authoritative per registry. Shadow walker syncs. | `WIP2g` (this ship) — delete legacy array |
| `node_overrides` | `PerNodeOverrides<F>` | (none) | `PerNodeOverrides<F> node_overrides[16]` | **TRANSITIONAL** — legacy global-default-with-override anti-pattern (cfg-scope-discipline.md § Anti-pattern 1). Entire mechanism (struct + array + ControllerConfig_ResolveForCore resolver) retires at WIP2f. | `WIP2f` (this ship) — delete with the legacy override mechanism |
...
## Section B — `PerNodeCfg<F>` runtime bitmap cluster (X-macro generated via meta-registry — WIP2d-0.B)

Post-WIP2d-0.B these fields are GENERATED by `FOREACH_PER_NODE_DOMAIN_BITMAP` meta-registry in `CoreFrameworks/CfgFieldRegistry.hpp`. The meta-registry has one row per bitmap domain — `(domain_token, field_name, storage_type, child_registry)` — and drives multiple auto-flows:
...
- **Permanent exemptions (Section A):** 6 (5 string arrays + node_feature_mask awaiting `.F.4e` Kind infrastructure)
- **TRANSITIONAL exemptions (Section A):** 5 (delete at WIP2g: 4 arrays; delete at WIP2f: node_overrides)
...
CI Check 7 (`tools/check_per_node_registry_integrity.py`) scans designated subsystem state types for scalar fields whose names match cfg field names; build-fails on unregistered matches. Exemptions to the check live here in Section C.
...
- `CoreFrameworks/CfgFieldRegistry.hpp` — `FOREACH_PER_NODE_FIELD_TYPE` + `FOREACH_MANUAL_PER_NODE_FIELD` X-macros
- `tools/check_per_node_registry_integrity.py` — CI cross-check enforcement (Section A + B = Checks 1-6; Section C = Check 7)

---

**Maintained at every per-core cfg field addition + every subsystem-state-cfg-mirror exemption. CI build-fails on drift between this file + FOREACH_MANUAL_PER_NODE_FIELD X-macro + FOREACH_PER_NODE_FIELD_TYPE X-macro + PerNodeCfg<F> body + designated subsystem state types.**
