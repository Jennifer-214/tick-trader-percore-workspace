...
## Planes are first-class + discipline-gating; the toolchain self-hosts (D-367)

The `[TAG]` plane values — `[ENGINE]` / `[DATA_PLANE]` / `[MONITORING_PLANE]` / `[DEV_PLANE]` (`doc-tag-vocabulary.md`) — are promoted to a FIRST-CLASS, discipline-GATING axis: each plane carries a distinct CONSTRAINT PROFILE, so the plane tells every consumer WHICH invariants + WHICH `[DERIVED]` axes apply. Not a label — a gate.

- **First-class concept, path-derived value.** The plane is a real axis, but the per-unit value DERIVES from location (`CoreFrameworks/`→`[ENGINE]`, `tools/`→`[DEV_PLANE]`), overridable only where plane ≠ path — never hand-typed on every unit (that is the N-site manual-sync anti-pattern; *auto-flow > manual sync*; the directory tree is the plane SSoT).
- **`FOREACH_PLANE` registry (registry-tuple-as-SSoT).** One row per plane = `{constraint profile (which H-invariants · latency-critical? · which gates fire) · valid [TAG] vocab · [DERIVED] axis-set}`. Fold a row → every consumer tracks it: the validator selects which checks fire, the cache-gate reads "does the straddle gate apply here," the plugin renders the plane-appropriate axis set. Graded profiles (monitoring inherits engine determinism/coherence but NOT hot-path latency) are why it's a REGISTRY, not a binary engine/dev flag.
- **Universal skeleton + plane-scoped vocabulary.** UNIVERSAL: block anatomy · parse rule · unit types (`FILE`/`STRUCT`/`FUNCTION`/…) · orient tags (`[TAG]`/`[SCHEMA]`/`[OVERVIEW]`/`[COMMENT]`/`[CODE]`/`[REFERENCE]`) · the ladder. PLANE-SCOPED: (1) the `[TAG]` VALUES (engine `HOT_PATH`/`BRANCHLESS` vs toolchain `PARSER`/`PRODUCER`/`CI_CHECK`/`GRAMMAR_DERIVED`); (2) the `[DERIVED]` AXES, which are a different KIND — engine DERIVED = *compiled-reality* (`SIZE`/`ALIGN`/`STRADDLE`/`BRANCHES`/`SIMD`), toolchain DERIVED = *pipeline-reality* (grammar-fences-read · parity-status · planes-validated; the call-graph axis is the one shared). A small SHARED cross-cutting core (`FRAMEWORK_DISCIPLINE`/`SSOT`/`DETERMINISM`) applies to all planes.
- **Structural gating.** A check proves the engine-only disciplines fire ONLY on `[ENGINE]`-profile units — a `[DEV_PLANE]` struct provably never trips the straddle gate (*compile-time/structural > convention*).
- **Multi-comment-syntax (north-star V1).** The parse rule is C++-`//`-only today; self-hosting the toolchain needs the parser to handle `#` (Python checkers) and `--` (Lua plugin) too — coordinated across all three parse implementations per § grammar-propagation above. foxtag C++ is free (same `//`; already partially tagged).
