...
- **What:** `tools/check_oms_per_slot_registry_integrity.py` **does not exist.** Nine sites cite it as real: `registry-coverage-ci-check-pattern.md:285` (Application 3's Tool row), `:298` (offered as a copy-me template), `:321` (counted among "three canonical CI tools exist at extraction time"), `:425`, `:436`; `decision-time-data-binding-pattern.md:414`; `cfg-scope-discipline.md:271`; `class-30-sibling-array-without-registry-enrollment.md:11` (the **frontmatter `closure_mechanism`**) and `:58/:61`. A comment inside the registry itself (`MemHeaders/OmsFieldRegistry.hpp:385-386`) tells the next contributor that *"NEW CI Check 8 enforces all `\w+[MAX_PORTFOLIO_POSITIONS]` arrays on OmsState are either enrolled here OR exempted"*.
...
- **Trigger:** D-421 step 5 (DOMAIN column) — at which point verify the `STRUCT:` domain actually covers `OmsState` per-slot arrays and retire all nine claims in one pass.
