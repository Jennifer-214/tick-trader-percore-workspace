...
### TECH_DEBT-163 — ALL THREE always-loaded docs (CLAUDE.md + CLAUDE.local.md + MEMORY.md) at their harness byte-caps (WH-6 structural)

- **id:** TECH_DEBT-163 · **severity:** medium · **opened:** 2026-06-10 · **status:** RESOLVED 2026-06-11 (context-aware loading shipped; SHOULD MOVE to closed.md) · **surface_tags:** [doc-system, always-loaded, memory, claude-local-md, context-budget, going-forward-index, WH-6]
- **Title:** All three always-loaded docs sit AT their caps (measured 2026-06-10: CLAUDE.md 39,720/40,000 · CLAUDE.local.md 39,691/40,000 · MEMORY.md 24,393/24,400). Landing any new memory requires compressing existing entries (hit 5× in the `.E.0.10` session); and — the broader consequence — **promoting new memories to CLAUDE.local.md's going-forward-rule index is now budget-BLOCKED**: the 6 `.E.0.10` memories (passing-test / single-source-computation / adversarial-framing-default / tag-disposition / consult-indexes / verify-by-context) are CAPTURED in MEMORY.md (recalled by relevance) but cannot get always-loaded CLAUDE.local.md going-forward lines without compression. So the always-loaded *index* drifts stale vs the memory corpus. The compression is whack-a-mole that degrades entry hooks; the structural fix is a context-aware-loading split, not more trimming.
- **Created:** 2026-06-10 — recurring blocker during the `.E.0.10` codification (the D-190 saga added 3 memories; each forced a multi-entry compaction pass to stay under cap). `tools/check_always_loaded_budget.py` enforces the cap (HARD in `check_session_docs.sh`) — the guard is correct; the index is genuinely full.
- **Why DEFERRED (merit, NOT effort):** the real fix is the `context-aware-loading` roadmap (load memories by relevance/surface instead of all-always) — a v6+ doc-system architecture change, far wider than any ship. Trimming buys a few hundred bytes at the cost of entry quality; stopgap, not solution.
- **Cost estimate:** stopgap trim ~15min/recurrence (degrading); real fix = the context-aware-loading roadmap (multi-day, v6+).
- **Trigger:** the next memory-add blocked by the cap where the operator wants the index intact, OR the v6+ doc-system-evolution work; until then, compress-to-fit per recurrence.
- **Cross-ref:** WH-6 (always-loaded byte-overflow, `meta-anti-pattern-index`); `feedback_file_size_split_discipline`; `plans/_future/2026-05-28-context-aware-claude-md-loading-roadmap.md`; `plans/_future/2026-05-28-docs-as-meta-code-roadmap.md`.
- **RESOLVED 2026-06-11 (`.E.0.10` Net-1 session):** shipped the context-aware split — a lighter native-nested-CLAUDE.md tiering (NOT the v6+ roadmap): engine detail → `CoreFrameworks/CLAUDE.md` (loads on file-edit), going-forward rules → `DOCS/GOING_FORWARD_RULES.md`, 15 deep-technical memories → `MEMORY_EXTENDED.md`; each move left an always-loaded pointer; the guards (`check_capture_audit` Check 1 + `check_doc_metadata`) span both indexes; 6 skills re-wired. Budget now CLAUDE.md 88% / CLAUDE.local.md 29% / MEMORY.md 88% — codification unblocked. 3-agent adversarial audit CLEAN. Committed `51aea35`. NEW pattern: `module-scoped-claude-md-pattern.md`. (open→closed.md move is a mechanical follow-up.)
...
### TECH_DEBT-175a — `cascade.py rename`/`audit` (rename-cascade enumerator + apparatus-currency guard)

- **id:** TECH_DEBT-175a · **severity:** medium · **opened:** 2026-06-21 · **status:** closed (2026-06-21 — both deliverables LANDED teeth-green + wired + enrolled: `tools/cascade.py rename` [enumerate-only; `cascade_selftest.sh` teeth: positive control + comment-resident regression-lock + PRESERVE/`experiments/` false-pos guards] + the `check_tools_inventory.py` `build.sh`-scan R2 fix [`test_check_tools_inventory.py` case-3 teeth]; wired → `run_all_tests.sh` + `DOCS/TOOLS.md` [SKILL-WIRED] + `rename-ship-methodology.md` §Mech + `/trace-deps` + `/dependency-chain-trace`; the Phase-2 enumeration freeze generated [`plan_checks/2026-06-21-E.1.1-rename-cascade-enumeration-freeze.md`, 3645 sites]. **Caught + fixed: (a) in-build, a comment-strip Class-33 under-count [626 missed comment-resident sites] + a REF_RE self-reference; (b) a post-build a-class completeness audit ["ensure the tool is finding all places"] caught a 2nd Class-33 [hand-list + START-anchor missed CoreSlowState / the ~50 `core_<stat>` field family / embedded `*_core_id` + `g_per_core_*` / the `*_cores` family / EventLoopAggregates.hpp / guard macros — 2118→3645 sites, ~42% missed], fixed via the family-regex sub-word-boundary model + a per-family selftest control + a mechanical freeze⊇raw-rg cross-check [closes AR-8 self-attestation; 0 real misses, the 33 residuals all PRESERVE-only]**. Physical move to closed.md batched at `.E.1.1` ship-close per convention.) · **surface_tags:** [ci-tooling, bulk-rename, static-analysis, class-36, class-51, structural-fix, .E.1.1]
- **What (REFINED 2026-06-21 by the /decision-check gate — R1+R2):** the grep-decidable slice carved from TD-175 (D-240). Two deliverables: (1) `tools/cascade.py` **`rename`** — on-demand **ENUMERATE-ONLY** worklist of a rename's whole-tree sites incl. the `tools/`/`build.sh`/`.githooks/` apparatus the compiler can't see + the `#include`-cascade + the expected-residual allowlist (NO code `--apply` — R1: the compiler is the code-token oracle per rename-ship-methodology Phase 4; a code `--apply` only adds a Class-36 silent-corruption surface). (2) the **R2 standing fix** — a 1-line `check_tools_inventory.py` `build.sh`-scan extension (closes the `build.sh:271` `[ -f tools/X.py ]` silent-skip). `cascade struct` is RESERVED → `.E.1.2`; distinct from the TD-175 remainder (AST struct-byte cascade).
- **Why:** the `.E.1.1` Core→Node rename touches `tools/`/`build.sh`/`.githooks/` which commit **GREEN on a stale regex** (the standing `.E.1.0` gates only catch engine-SOURCE slips; ≥8 tool files + 2 data ledgers + build.sh carry the tokens, 3 feeding pre-commit commit-blockers) — load-bearing, not optional. `rename` PLANS the co-migration; the existing `check_` guards + the `build.sh`-scan fix CATCH a miss.
- **Design + integration:** `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md` (stage-2, v1.2). Reuses the `check_struct_alignment.py` substrate IDIOM (new file beside it). `--selftest` (non-vacuity, Class-51) → `run_all_tests.sh`; `DOCS/TOOLS.md` row (PLANNED→done); `/trace-deps` + `/dependency-chain-trace` compose `rename`; `rename-ship-methodology.md` §Mechanization. **NOT** a new standing-guard subcommand wired into check_session_docs/pre-commit (R2 — the standing net rides the existing `check_tools_inventory`). **Noted-future (homed, not built):** `--apply` (apparatus-only, hard-gated, Class-36-algo extracted to `tools/lib/`) + the broader apparatus→dead-`FOREACH` standing guard (≈80% redundant w/ `check_meta_registry`; M7-recurrence-gated).
- **Close-condition:** `cascade.py rename` (enumerate-only + non-vacuity teeth) + the `check_tools_inventory.py` `build.sh`-scan extension (+ teeth) both GREEN + wired + `DOCS/TOOLS.md`-enrolled → CLOSE at `.E.1.1`. The rename then USES `rename`.
- **Cross-ref:** TECH_DEBT-175 (parent — the AST `struct` remainder → `.E.1.2`); `rename-cascade-enumeration-tooling.md` + `rename-ship-methodology.md` + `struct-change-cascade-impact-tooling.md`; RBP Class 36/51/33; D-239 + D-240; `check_struct_alignment.py` (substrate) + `check_meta_registry.py` (`audit` composes its enumeration) + `check_doc_rename_classification.py` (doc-side executor).
...
### TECH_DEBT-204 — `.githooks` not mirrored by `/sync-workspace`: engine hook edits silently drift from the tracked copy

- **id:** TECH_DEBT-204 · **severity:** MED (apparatus integrity — a CI-guard hook edit can be silently lost on a fresh checkout; non-capital) · **opened:** 2026-06-16 · **status:** CLOSED 2026-07-04 (D-300/D-301 — the recommended symlink fix implemented) · **surface_tags:** [tooling, ci, pre-commit, sync-workspace, workspace-hygiene, apparatus]
- **What:** there are TWO `.githooks/pre-commit` files — the ENGINE's (live, the one `core.hooksPath=.githooks` runs; gitignored in the engine, `.gitignore:151`) and the WORKSPACE's (tracked, the backup). `/sync-workspace` does NOT mirror `.githooks/` (absent from the skill's mirror table), so an edit to the live engine hook does NOT reach the tracked workspace copy → silently lost on a fresh checkout. Surfaced at the `.E.1.0` session close: the new Check F/M wiring had landed only in the engine copy; manually propagated to the tracked copy this session so it survives — but the recurrence is open.
- **Why:** a CI-guard hook edit that can't survive is the "built, never wired, silently inert" rot class at the hook surface (sister to the `check_tools_inventory` D-136 close for `tools/*`). Convention (remember to copy) is insufficient — it already drifted once.
- **Fix (structural):** make engine `.githooks` a SYMLINK to the workspace `.githooks` (matching `tools/`/`plans/` — one source of truth, auto-propagating + tracked + live; verify the hook still fires via a test commit), OR add `.githooks/pre-commit` to the `/sync-workspace` `sync_if_newer` mirror block. Lean: the symlink (consistent with the existing dir-symlink pattern).
- **Cross-ref:** `.E.1.0` handoff `2026-06-16-E.1.0-gates-1-3-wired-next-H8` (findings) + **D-227** (the session's apparatus finding) · `/sync-workspace` SKILL.md mirror table (the gap) · the `tools/`/`plans/` dir-symlink pattern (the model fix).
  - **Update 2026-06-16 s4:** Check N (conformance gate) was again manually propagated engine→workspace `.githooks/pre-commit` this session (the interim continues); the structural symlink/sync-mirror fix above remains the open item.
  - **CLOSED 2026-07-04 (D-300/D-301):** implemented the recommended structural fix — the workspace `.githooks/pre-commit` is now the SINGLE canonical UNION hook (tracked, Checks A-P); the engine `.githooks` is a DIR-symlink to it (matching `tools/`/`plans/`); both repos' `core.hooksPath=.githooks`. The `/sync-workspace` mirror gap is MOOT (the canonical is directly tracked/committed, not mirrored). Durability: `tools/setup_hooks.sh` (idempotent) re-creates the whole gate on a fresh clone. Verified: engine Check O + workspace Check P both fire via the symlink. (Surfaced by the `.E.1.2` close "does this keep happening" probe — I re-solved it via D-300 before spotting this ledger entry; the check-the-ledger-first lapse noted in D-301.) Archive to `closed.md` at the next ledger sweep.
...
### TECH_DEBT-227 — `partner_pending_pnl` (partial-exit pairing accumulator) is NOT persisted → per-trade W/L miscount on a mid-partial-exit warm restart
- **id:** TECH_DEBT-227 · **severity:** MED · **opened:** 2026-07-01 · **status:** open · **surface_tags:** [oms, persistence, warm-restart, partial-exit, accounting, capital]
- **What:** `NodeContext.partner_pending_pnl` (`ControllerEventLoop.hpp:424` — the leg-A↔leg-B pairing-net accumulator) + `partner_pending_bitmap` (on `EventLoopState`) are NOT in the snapshot persist set (surfaced by the C3 wire-ledger c-class of the 2026-07-01 Fight #4 / D-288 sweep; confirmed at HEAD `0ee227a`). A snapshot taken BETWEEN leg-A-close and leg-B-close loses the in-flight pairing net → the restored engine miscounts that trade's per-node W/L (and the paired realized split) after a warm restart. Latent ONLY under `partial_exit_enabled=1`.
- **Severity rationale:** capital-accounting correctness (W/L attribution), but bounded — (a) only in the narrow leg-A-closed-but-leg-B-open snapshot window, (b) under partials-on, (c) no live exposure until live-enable (E.1.3 ∧ E.1.5). A persist-completeness gap, not a freeze/wire blocker.
- **Disposition / trigger:** fold into **E.1.4** (fill-completeness — the leaf that owns the partial-exit pairing + the fill read-set) OR the E.1.2 `FOREACH_NODE_PERSIST_FIELD` registry-drive — if the serializer is registry-driven there, `partner_pending_pnl`/`partner_pending_bitmap` become persist rows automatically (the natural structural close: the WHOLE point of registry-driving the serializer is that no field can be silently dropped — D-288 sweep's compose-sub-registries shape). Decide the home at the E.1.4 (or E.1.2-serializer) dive.
- **Cross-ref:** `ControllerEventLoop.hpp:424` · D-288 (the Fight #4 / wire-ledger sweep) · `plan_checks/2026-06-30-E.1.2-per-field-declaration-table.md` · `feedback_no_unhomed_debt_code_smell` · `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md` v1.4.
...
- **id:** TECH_DEBT-250 · **severity:** medium-high (the close ritual is where capture loss happens, and it is guarded by nothing mechanical — every catch to date has come from the operator asking) · **opened:** 2026-07-19 · **status:** closed · **surface_tags:** [ci-tooling, close-ritual, capture-audit, m8, ar-8, m7, wh-2]
- **What:** § 11.5's **M8** row names `check_close_out_completeness.py` as its "candidate CI". **Verified absent** — neither `tools/check_close_out_completeness.py` nor `tools/check_close_completeness.py` exists, and neither is referenced in `check_session_docs.sh`. `meta-anti-pattern-index.md` records the **AR-8** close-completeness recurrence across **S17 → S18 → S20 → S21 → S22 and the 2026-07-18 close**, each entry closing with a variant of *"STILL UNBUILT … operator pushback remains the trigger."*
- **This session is recurrence N+1, twice over.** The `E.1.2.B` close-out committed with (a) a handoff missing its capture-completeness + TaskList sections and (b) a stale MASTER banner — both caught by the *pointer* guard and the *handoff-section* guard, which exist. But the things those guards do **not** cover were caught only because the operator said *"be thorough on the capture"* and then *"be sure we actually captured and harvested everything"*: an **un-promoted 7-week-old M9 earmark**, a **shipped citable-ID collision**, **4-of-8 defective §11.5 rows**, a **dangling memory cite**, and the **highest-capital meta-discipline (M5) reachable from zero skills**. Six real defects, zero mechanical detection, one operator prompt.
- **Why this is the M7 signal, not just a TODO:** the discipline is fully codified (spec + memory + skill + a §11.5 row) and has still failed at the same surface across 7 sessions. That is precisely M7's escalation criterion — *memory codification proven insufficient at the same surface* — and the escalation target it names is a CI tool. The recurrence tally living in an anti-pattern index is a RECORD, not a home with a trigger (`feedback_no_unhomed_debt_code_smell`); this entry is the home.
- **Fix shape:** feed it the session's git diff change-set and RED on the four dimensions the index already names — **captured-files** (every touched surface has a ledger/log/plan entry) · **owning-doc staleness** (a doc whose subject changed but whose body did not) · **conversation→artifact** (decisions/findings named in-session with no artifact) · **auto-write contracts** (CLAUDE.local.md's table honored: PARITY / TECH_DEBT / FEATURE_LOOKUP / decoupling / landmines). Ship with `--selftest`; wire HARD into `check_session_docs.sh`.
- **Oracle honesty (M10):** dimensions 1 and 4 are **TOTAL**-shaped (a touched file either has a ledger entry or it does not). Dimensions 2 and 3 are irreducibly **PARTIAL** — "did the doc need updating?" is judgment. Build the total half and label the partial half as advisory rather than letting a green run imply full coverage (Class-51 vacuous-guard risk).
- **Sequencing:** natural sibling of TECH_DEBT-249 (both are meta-layer citation/coverage guards) → build alongside it at **`E.1.2.B` `0.2`**, the toolchain CONTRACT + GATE increment.
- **Cross-ref:** `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md` (M8 — the spec whose step 5 this is) · `meta-anti-pattern-index.md` **AR-8** (the recurrence ledger) + **WH-2** · **M7** (`structural-enforcement-when-memory-insufficient.md` — the escalation this triggers) · TECH_DEBT-249 (the §11.5 citation guard; same increment) · TECH_DEBT-194 (the index-currency guard — the ONE close-ritual guard that DOES exist and did fire this session, i.e. proof the shape works) · M10/D-385.

- **CLOSED (2026-07-20, `E.1.2.B 0.2`):** `tools/check_close_out_completeness.py` exists and is wired into `tools/check_session_docs.sh` as an advisory. It enforces BOTH halves of the close-out — the MECHANICAL half (every auto-write surface named in `CLAUDE.local.md` received a commit, or carries an explicit `--explain` reason) and the JUDGMENT half (no volatile counts in handoff prose · a RE-DERIVE block present · a greppable `Check N` judgment ledger · a recorded independent review per AR-8). 13 selftest cases; every new check has a planted positive AND a negative control, so a check that fires unconditionally cannot pass for a check that works.
- **The "Verified absent" evidence above is HISTORICAL** — true when written 2026-07-19, false from 2026-07-20. Left in place rather than edited, because an entry that silently rewrites its own evidence cannot be audited afterwards. Flagged by the a-class adversarial review of the `0.2` handoff, which correctly noted a fresh session reading this entry would rebuild a tool that already exists.
...
### TECH_DEBT-254 — `rebuild_doc_indexes` silently drops one-off spec types; `DESIGN_SPECS/README.md` claims 15 types and renders 11

- **id:** TECH_DEBT-254 · **severity:** low-med (the generated catalog is the documented discovery surface for 191 specs; a spec absent from it is effectively uncodified — the "not discoverable = not codified" problem) · **opened:** 2026-07-19 · **status:** open · **surface_tags:** [doc-system, index-currency, design-specs, generated-artifact, discoverability]
- **What:** measured 2026-07-19 by the `0.2` precedent sweep. `DESIGN_SPECS/README.md:5` claims *"191 specs across 15 types"* but the generated catalog renders only **11 type sections**. `rebuild_doc_indexes.py` silently drops the one-off types — **`subsystem-design`, `north-star`, `input-space-taxonomy`** — plus the untyped `plan-templates/memory-template.md`. Separately: **3 specs lack required frontmatter** (`doc-disciplines/in-code-doc-system-north-star.md` and `doc-disciplines/format-input-space-taxonomy.md` have no `stage:`; `plan-templates/memory-template.md` has neither `type:` nor `stage:`). And **`stage: 6-cadence-locked` has ZERO users** across all 191 specs despite being valid vocab (`doc-frontmatter-convention.md:40`) **and advertised as a grep recipe in `CLAUDE.md`** — a documented discovery path that returns nothing.
- **Why it matters:** the README/TAG_INDEX pair IS the discovery surface — `CLAUDE.md`'s "How to find anything" recipes route through it. A spec that exists but is unrendered is unreachable by every documented path, which is the same failure as not writing it. The north-star doc is one of the dropped ones, and it is cited by the active plan body.
- **Fix shape:** (a) make `rebuild_doc_indexes.py` render EVERY `type:` value it encounters (or loud-fail on an unknown one — silent-drop is the defect); (b) backfill the 3 missing frontmatter fields; (c) decide `6-cadence-locked` — either promote the specs that qualify, or retire the vocab value and the `CLAUDE.md` recipe that advertises it. **A vocab value with zero users that is advertised as a search is a dangling promise.**
- **Trigger:** the `0.2` doc-CI pass (it sits beside the citable-ID guard work — same substrate, same `rebuild_doc_indexes` writer). Opportunistic otherwise.
- **Cross-ref:** **D-395** (the sweep that measured it) · `doc-frontmatter-convention.md:40,243` (the 6-value `stage:` vocab) · `DESIGN_SPECS/README.md:5` (the false count) · TECH_DEBT-249 (the citable-ID guard — adjacent, same `rebuild_doc_indexes` surface) · TECH_DEBT-166/194 (doc-index/SSoT-currency drift — the recurring family this joins).
- **PARTIALLY CLOSED 2026-07-20** (E.1.2.B `0.2`, commit `bda3475`) — (a) and (b) done; (c) deliberately left open.
  - **(a) silent-drop FIXED at the root.** `rebuild_doc_indexes` iterated a hardcoded 12-entry `TYPE_ORDER` and never visited any other type, so 4 types were dropped: `subsystem-design`, `north-star`, `input-space-taxonomy` (+ a phantom, below). It now renders EVERY type — canonical order first, remainder after — and prints a LOUD note naming strays so they get promoted or corrected instead of vanishing. The three real categories were promoted into `TYPE_ORDER`. **`in-code-doc-system-north-star.md` was one of the dropped ones and is cited by the LIVE plan body** — unreachable from every documented discovery path while being load-bearing.
  - **A PHANTOM TYPE, found in passing.** `parse_frontmatter` called `line.strip()` before parsing, erasing the indentation that marks a key as NESTED — so `memory-template.md`'s `metadata:` → `  type: feedback` was read as a top-level `type: feedback`, rendering a spec-type no document declares. A parser that flattens structure reports fields the document never stated. Top-level keys only now.
  - **The count defect was TWO NUMBERS FROM TWO SOURCES**, which is why 15-vs-11 survived: the header used `len(by_type)` (including the `unknown` bucket) while the body rendered from `TYPE_ORDER`, and neither checked the other. Both now derive from the render set. Catalog is coherent: header 191/14, sections 14, sum-of-rows 191.
  - **(b) the 3 frontmatter gaps.** `memory-template.md` is EXCLUDED and said so aloud — it is a template whose frontmatter IS example content to be copied, so giving it spec frontmatter would corrupt the thing it exists to be. The other two carried `status:` (a free-text drafting note) but not `stage:` (the promotion rung) — different fields, so `status` was left intact: north-star → `3-first-canonical`, format-input-space-taxonomy → `2-draft` (it self-describes as accumulating, 1 of 3 surveys).
  - **(c) `stage: 6-cadence-locked` — STILL OPEN, deliberately.** Zero users while `CLAUDE.md` advertises it as a grep recipe. NOT resolved mechanically: deciding which specs have earned the top rung is a lifecycle judgment, and promoting specs unilaterally to make a vocab value non-empty would be backwards reasoning. Needs operator triage — promote the qualifying specs (`dead-code-and-identifier-retirement-discipline.md` is the obvious candidate: CI check + pre-commit Check H + golden ledger) or retire the value and its `CLAUDE.md` recipe together.
...
### TECH_DEBT-257 — the asm/layout analyzers use editor-supplied flags, not the real build's compile_commands (derived facts describe a TU nobody builds)

- **id:** TECH_DEBT-257 · **severity:** **high** · **opened:** 2026-07-20 · **status:** open · **surface_tags:** [toolchain, derived-facts, static-analysis, correctness, plugin]
- **What:** operator-raised 2026-07-20 — *"we should use the actual compile commands instead of relying on whatever nvim is using, so its always 1:1, since the asm analyzer in the plugin doesnt exactly work."* The layout/asm/codegen producers compile probes with flags assembled by the tool (or inherited from the editor's LSP config) rather than read from the build's `compile_commands.json`. When those differ, **the derived fact describes a translation unit that is never built.**
- **Why this is HIGH and not a polish item:** the entire `[DERIVED]` tier rests on the premise that a written fact reflects COMPILED REALITY (D-307/D-369; `mechanical-verification-of-derived-code-facts.md`). A size, an alignment, an instruction count or a branch classification taken under different flags is not a weaker fact — it is a **wrong** fact, written into the corpus by a green gate, and then propagated by every future conversion that copies the pattern. This is the same failure shape as the `check_cache_layout` template hazard (C-396), one layer earlier: at the flag boundary rather than the corpus boundary.
- **Concretely at risk:** `check_latency_path_conformance` (instruction/branch ratchet vs PRODUCTION disassembly — the flags ARE the measurement), `check_cache_layout` (sizeof/alignof probes), `check_struct_size_budget`, `foxtag layout`/`codegen`, and the plugin's asm view. `build_clangd/` already exists in-tree, so a `compile_commands.json` is at least partly available.
- **Fix shape:** resolve probe flags from `compile_commands.json` for the TU being analysed, with a LOUD failure when no entry exists (never a silent fallback to defaults — that is how the divergence became invisible). Assert the provenance in the emitted fact so a reader can see which flags produced it. Non-vacuity: a probe compiled under deliberately-wrong flags must produce a DIFFERENT answer, proving the flags are load-bearing rather than decorative.
- **Trigger:** before any further `[DERIVED]` fact-producer work, and **before TECH_DEBT-256** (AST matchers inherit this exact problem). Candidate home: `0.5` (RC-F/layout surface) or its own increment.
- **Cross-ref:** TECH_DEBT-256 (depends on this) · D-307 · D-369 · `mechanical-verification-of-derived-code-facts.md` · C-396 (the template-propagation hazard this front-runs).

### TECH_DEBT-258 — the plugin needs a registry-driven consumer surface, or every new tool becomes a bespoke integration

- **id:** TECH_DEBT-258 · **severity:** med · **opened:** 2026-07-20 · **status:** open · **surface_tags:** [toolchain, plugin, framework-discipline, extensibility]
- **What:** operator-raised 2026-07-20 — *"we will need plugin functionality to implement the new tools and everything from this dev cycle for the toolchain, so making the plugin easily extensible is a priority."* `E.1.2.B` `0.2` alone added four consumer-facing surfaces (the corpus contract + membership pin, the bless path, the tools-gate verdict, the widened import lint) and **none of them has a plugin surface.** At the current rate each new producer is a hand-written Lua integration, which makes the plugin the bottleneck for the whole toolchain rather than its display layer.
- **Why it is structural, not a backlog of N integrations:** this is the same shape the engine solved with X-macro registries — a new capability should be **one row**, with the surface auto-flowing. The plugin already consumes `foxtag grammar --json` for its node model (`0.3`), which proves the data path; what is missing is that the SET of consumable producers is hardcoded rather than enumerated. A producer registry the plugin reads (kind → renderer) makes "add a tool" a data edit on both sides.
- **Fix shape:** enumerate producers as DATA (the `toolio_schemas.json` registry already catalogues payload kinds — extend rather than mint, per the canonical-sister discipline), have the plugin derive its available views from that registry, and add a `parity_check.sh` section asserting the plugin's producer set matches the registry (so a new producer with no plugin surface is VISIBLE rather than silently absent — the advertised-capability shape).
- **⚠️ Distinct from plugin POLISH.** The operator was explicit that the existing features *work* and want taste/refinement (branchless tag rendering, floating-panel placement, git-blame virtual text overlapping the HUD). Those are a UX backlog, NOT this entry and NOT a correctness register — see `advertised-capability-never-exercised.md` § False-positive surface.
- **Trigger:** before `0.4` (tag-native surfaces) is scoped — it is a design constraint on that increment, not a follow-up to it.
- **Cross-ref:** D-337 · D-349 · `doc-intelligence-toolchain-architecture.md` · `standardized-tool-io-envelope-and-payload.md` (the registry to extend) · TECH_DEBT-252 (the plugin repo's ungated-commit gap).
