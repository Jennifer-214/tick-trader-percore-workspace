# Tag → files index (auto-generated snapshot)

**Auto-generated** by `tools/rebuild_doc_indexes.py`. Canonical reverse-lookup is `rg`:

```bash
rg -l "^tags:.*\bframework-discipline\b" DESIGN_SPECS/
rg -l "^surface:.*\bhot-path\b"
```

This file is a snapshot for static browsing.

## CONCERN tags

### abi-versioning (1 files)

- `DESIGN_SPECS/framework-patterns/dynamic-library-strategy-loading-pattern.md`

### adversarial-default (1 files)

- `claude-skills/decision-check/SKILL.md`

### aes-gcm (2 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### agent-arming (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`

### aggregator (2 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`
- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`

### ai-plane (1 files)

- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`

### anti-pattern-index (1 files)

- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`

### asset-class (1 files)

- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`

### async-io (1 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### atomic-add (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### atomic-flags (1 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### atomic-swap (1 files)

- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### audit-log (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### audit-methodology (86 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/audit-driven-pre-coding-gate.md`
- `DESIGN_SPECS/audit-methodologies/audit-finding-kind-taxonomy.md`
- `DESIGN_SPECS/audit-methodologies/audit-report-format.md`
- `DESIGN_SPECS/audit-methodologies/audit-scope-taxonomy.md`
- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`
- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`
- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`
- `DESIGN_SPECS/meta-disciplines/implementation-layer-blindspot-taxonomy.md`
- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`
- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/dead-code-trace/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/dust/SKILL.md`
- `claude-skills/finding-analyzer/SKILL.md`
- `claude-skills/metadata-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/plan-check/SKILL.md`
- `claude-skills/plan-context-sweep/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `claude-skills/second-opinion/SKILL.md`
- `claude-skills/test-strength-audit/SKILL.md`
- `claude-skills/trace-deps/SKILL.md`
- `memory/feedback_a_class_i_class_fanout_vocab.md`
- `memory/feedback_address_med_low_findings_not_just_high_crit.md`
- `memory/feedback_adversarial_framing_default_for_checks.md`
- `memory/feedback_arm_subagents_plan_and_future_aware.md`
- `memory/feedback_audit_canonical_sister_before_new_infra.md`
- `memory/feedback_audit_own_proposals_with_same_rigor.md`
- `memory/feedback_capture_and_check_are_model_bounded.md`
- `memory/feedback_comments_point_in_time_verify_against_code.md`
- `memory/feedback_consult_indexes_before_full_reads.md`
- `memory/feedback_consult_on_audit_findings.md`
- `memory/feedback_domain_guards_for_bulk_transforms.md`
- `memory/feedback_enumerate_set_before_categorical_claim.md`
- `memory/feedback_future_headache_vs_optimization_scope_framework.md`
- `memory/feedback_golden_master_over_reimplemented_oracle.md`
- `memory/feedback_ground_design_in_real_code.md`
- `memory/feedback_guard_matrix_bounds_foundation_hardening.md`
- `memory/feedback_heavier_default_audit_posture_for_capital.md`
- `memory/feedback_implementation_detail_blindspot_recovery_via_taxonomy.md`
- `memory/feedback_independence_for_judgment_not_mechanical.md`
- `memory/feedback_iteration_spiral_signals_audit_meta_gap.md`
- `memory/feedback_lead_with_architectural_merit_not_operator_tone.md`
- `memory/feedback_metadata_audit_quarterly.md`
- `memory/feedback_never_skip_thoroughness_unless_explicit.md`
- `memory/feedback_operator_pushback_as_audit_signal.md`
- `memory/feedback_passing_test_is_not_verification.md`
- `memory/feedback_paste_tool_output_dont_summarize.md`
- `memory/feedback_phased_pre_rework_correctness_foundation.md`
- `memory/feedback_plans_cite_sister_registry_inspection.md`
- `memory/feedback_proactive_novel_alternative_consideration.md`
- `memory/feedback_process_weight_by_surface_blast_radius.md`
- `memory/feedback_proportionate_response_to_audit_findings.md`
- `memory/feedback_recascade_audit_on_corrected_shape.md`
- `memory/feedback_recheck_designspecs_on_pushback.md`
- `memory/feedback_rename_enumerator_is_world2_aid_compiler_is_world1.md`
- `memory/feedback_run_doc_ci_tools_first_never_hand_verify.md`
- `memory/feedback_runtime_executor_mode_for_judgment_skills.md`
- `memory/feedback_sequential_audit_for_granular_operator_triage.md`
- `memory/feedback_single_source_the_computation_not_just_the_mode.md`
- `memory/feedback_structure_judgment_loop_not_output.md`
- `memory/feedback_tag_disposition_at_fix_time.md`
- `memory/feedback_tiered_audit_discipline_per_plan_scope.md`
- `memory/feedback_train_serve_execution_layer_meta_gap.md`
- `memory/feedback_two_foundations_determinism_vs_correctness.md`
- `memory/feedback_unconditionalization_latent_assumption_audit.md`
- `memory/feedback_verify_by_context_not_count.md`
- `memory/feedback_verify_every_enumerated_site_at_close.md`
- `memory/feedback_verify_symbol_existence_at_plan_drafting_time.md`
- `memory/project_anti_spaghetti_audit_cadence.md`

### avx-512 (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### backtest (1 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`

### backtest-to-live (1 files)

- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### backwards-compat (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### block-scope-statics (1 files)

- `memory/feedback_enumerate_block_scope_statics_before_hoist.md`

### boot-time-check (1 files)

- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`

### branchless (1 files)

- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`

### branchless-discipline (21 files)

- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/data-disciplines/partner-core-bitmap-pattern.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/composed-filter-mask-pattern.md`
- `DESIGN_SPECS/framework-patterns/curve-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-axis-y3-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/multi-state-dispatch-with-per-state-update-metadata.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/sink-fn-pointer-for-optional-side-effect-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/framework-patterns/x-macro-registry-with-presence-dispatch.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/multi-bit-state-encoding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/transient-aggregation-bitmap-pattern.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`

### breaking-changes (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### bulk-rename (1 files)

- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`

### cache-line (1 files)

- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`

### cache-locality (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### capital-allocation (1 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`

### capital-safety (10 files)

- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`

### cascade-propagation (1 files)

- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`

### cfg-driven (1 files)

- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`

### cfg-mutability (1 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`

### cfg-parser (1 files)

- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`

### ci-tooling (4 files)

- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`

### ci-tooling-surface (1 files)

- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`

### clean-break (1 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`

### cluster-node-hierarchy (1 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`

### code-loc-counting (4 files)

- `memory/feedback_count_code_loc_not_total_lines.md`
- `memory/feedback_dont_measure_structural_work_by_loc.md`
- `memory/feedback_file_size_split_discipline.md`
- `memory/feedback_plan_body_length_no_target_loc.md`

### codegen (1 files)

- `DESIGN_SPECS/data-disciplines/register-spill-discipline.md`

### concurrency (21 files)

- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-snapshot-publish-cluster-isolation.md`
- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/phase-separated-drainer-for-safe-cross-temporal-derives.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-ring-embedded-in-hot-struct-cluster-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`
- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`
- `DESIGN_SPECS/data-disciplines/cache-layout-discipline-for-hot-side-structs.md`
- `DESIGN_SPECS/data-disciplines/cache-line-discipline.md`
- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/raii-destructor-with-cluster-reorg-interaction.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-erased-per-core-resource-handle-pattern.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`

### concurrency-safety (1 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`

### config-validation (2 files)

- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`

### conservative-bias (1 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`

### context-aware-loading (1 files)

- `DESIGN_SPECS/doc-disciplines/module-scoped-claude-md-pattern.md`

### convergence-discipline (1 files)

- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`

### cpp17 (3 files)

- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`
- `memory/feedback_cpp17_inline_variable_for_shared_state_across_tus.md`
- `memory/feedback_forward_decl_at_global_scope_not_namespace.md`

### crash-recovery (2 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-action-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`

### credentials (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`

### cross-reference (1 files)

- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`

### cross-ship-integration (1 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`

### cross-tool-decoupling (3 files)

- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `memory/feedback_machine_portable_resolver_for_committed_tool_paths.md`

### currency-check (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`

### custom-ide (1 files)

- `DESIGN_SPECS/doc-disciplines/in-code-doc-system-north-star.md`

### cycle-recompute (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### data-discipline (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### data-layout (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### data-oriented-design (38 files)

- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-snapshot-publish-cluster-isolation.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-ring-embedded-in-hot-struct-cluster-discipline.md`
- `DESIGN_SPECS/data-disciplines/aggressive-memory-reduction-techniques.md`
- `DESIGN_SPECS/data-disciplines/cache-layout-discipline-for-hot-side-structs.md`
- `DESIGN_SPECS/data-disciplines/cache-line-discipline.md`
- `DESIGN_SPECS/data-disciplines/decision-first-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/function-struct-alignment-for-single-mov-access.md`
- `DESIGN_SPECS/data-disciplines/hot-side-array-element-alignment-for-sparse-access.md`
- `DESIGN_SPECS/data-disciplines/partner-core-bitmap-pattern.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/raii-destructor-with-cluster-reorg-interaction.md`
- `DESIGN_SPECS/data-disciplines/register-spill-discipline.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`
- `DESIGN_SPECS/refactor-patterns/latency-vs-cache-decision-framework.md`
- `DESIGN_SPECS/refactor-patterns/loop-fusion-pattern.md`
- `DESIGN_SPECS/refactor-patterns/multi-bit-state-encoding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/slow-path-cfg-resolution-cache-pattern.md`
- `DESIGN_SPECS/refactor-patterns/transient-aggregation-bitmap-pattern.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`

### decision-gate (1 files)

- `claude-skills/decision-check/SKILL.md`

### decoupling (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### definition-of-done (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### delegation (1 files)

- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`

### deletion-discipline (5 files)

- `memory/feedback_archived_changelog_preservation_discipline.md`
- `memory/feedback_enumerate_consumers_before_registry_row_deletion.md`
- `memory/feedback_multi_surface_deletion_ordering_discipline.md`
- `memory/feedback_operator_facing_doc_cohort_at_cfg_deletion.md`
- `memory/feedback_unconditionalization_latent_assumption_audit.md`

### deletion-scope (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### deployment-mode (1 files)

- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`

### design-discipline (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### determinism (4 files)

- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`

### dev-mode (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### dev-plane (3 files)

- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`
- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`

### dlopen (2 files)

- `DESIGN_SPECS/framework-patterns/dynamic-library-strategy-loading-pattern.md`
- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### doc-discipline (55 files)

- `DESIGN_SPECS/audit-methodologies/audit-report-format.md`
- `DESIGN_SPECS/doc-disciplines/categorical-triggers-in-always-loaded-docs.md`
- `DESIGN_SPECS/doc-disciplines/file-size-split-discipline.md`
- `DESIGN_SPECS/doc-disciplines/format-input-space-taxonomy.md`
- `DESIGN_SPECS/doc-disciplines/in-code-doc-system-north-star.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/doc-disciplines/module-scoped-claude-md-pattern.md`
- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/ledger-templates/ledger-entry-templates.md`
- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`
- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/doc-frontmatter-convention.md`
- `DESIGN_SPECS/meta-disciplines/doc-tag-vocabulary.md`
- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/pattern-codification-lifecycle.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `DESIGN_SPECS/plan-templates/design-spec-template.md`
- `DESIGN_SPECS/plan-templates/future-oriented-plan-template.md`
- `DESIGN_SPECS/plan-templates/postmortem-template.md`
- `DESIGN_SPECS/plan-templates/sprint-master-plan-template.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`
- `claude-skills/accept-handoff/SKILL.md`
- `claude-skills/capture-audit/SKILL.md`
- `claude-skills/doc-create/SKILL.md`
- `claude-skills/find/SKILL.md`
- `claude-skills/foxlib-promotion/SKILL.md`
- `claude-skills/handoff/SKILL.md`
- `claude-skills/index-rebuild/SKILL.md`
- `claude-skills/metadata-audit/SKILL.md`
- `claude-skills/plan-check/SKILL.md`
- `claude-skills/plan-draft/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `claude-skills/ship/SKILL.md`
- `claude-skills/sync-models/SKILL.md`
- `claude-skills/sync-workspace/SKILL.md`
- `memory/feedback_categorical_triggers_over_hardcoded_refs.md`
- `memory/feedback_claude_md_guidelines_not_stuff_to_do.md`
- `memory/feedback_count_code_loc_not_total_lines.md`
- `memory/feedback_document_as_you_go_over_catch_at_end.md`
- `memory/feedback_file_size_split_discipline.md`
- `memory/feedback_fold_findings_into_destination_plan.md`
- `memory/feedback_ground_design_in_real_code.md`
- `memory/feedback_metadata_audit_quarterly.md`
- `memory/feedback_operator_facing_doc_cohort_at_cfg_deletion.md`
- `memory/feedback_sister_cohort_amendment_completeness.md`
- `memory/feedback_skill_edit_cohort_checklist.md`
- `memory/feedback_terminology_evolution_bridge_not_history_rewrite.md`
- `memory/project_engine_clauder_md_is_symlink.md`

### doc-pipeline (2 files)

- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`

### dpdk (2 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### drift-management (1 files)

- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### dual-format (1 files)

- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### economic-isolation (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`

### ecosystem (2 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`
- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### encryption (1 files)

- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### encryption-offload (1 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`

### enumeration-discipline (9 files)

- `memory/feedback_enumerate_block_scope_statics_before_hoist.md`
- `memory/feedback_enumerate_consumers_before_registry_row_deletion.md`
- `memory/feedback_enumerate_helper_signature_args_before_extract.md`
- `memory/feedback_enumerate_set_before_categorical_claim.md`
- `memory/feedback_multi_surface_deletion_ordering_discipline.md`
- `memory/feedback_paste_tool_output_dont_summarize.md`
- `memory/feedback_rename_enumerator_is_world2_aid_compiler_is_world1.md`
- `memory/feedback_sister_cohort_amendment_completeness.md`
- `memory/feedback_test_change_enumeration_per_plan_body.md`

### event-sourcing (1 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`

### exchange-adapter (2 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`
- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`

### exchange-enforced (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`

### failure-domain (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`

### failure-observability (4 files)

- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/test-strength-audit/SKILL.md`

### filesystem-layout (1 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`

### fill-path (1 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`

### financial-advisor (1 files)

- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`

### finding-triage (1 files)

- `DESIGN_SPECS/audit-methodologies/audit-finding-kind-taxonomy.md`

### fix-protocol (1 files)

- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`

### fix-scoping (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### fixed-point-math (7 files)

- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/sliding-window-online-statistics-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/prng-choice-for-replay-determinism.md`
- `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`

### forward-decl-shadow (1 files)

- `memory/feedback_forward_decl_at_global_scope_not_namespace.md`

### framework-discipline (143 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/audit-driven-pre-coding-gate.md`
- `DESIGN_SPECS/audit-methodologies/audit-report-format.md`
- `DESIGN_SPECS/audit-methodologies/audit-scope-taxonomy.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/data-disciplines/aggressive-memory-reduction-techniques.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/doc-disciplines/categorical-triggers-in-always-loaded-docs.md`
- `DESIGN_SPECS/doc-disciplines/module-scoped-claude-md-pattern.md`
- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md`
- `DESIGN_SPECS/feature-patterns/runtime-toggleable-bench-gate-pattern.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-from-arity-macro-family.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-pattern-for-production-caller-class.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`
- `DESIGN_SPECS/framework-patterns/calibration-log-column-registry.md`
- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/categorical-tag-applicability-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`
- `DESIGN_SPECS/framework-patterns/composed-filter-mask-pattern.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/crash-recovery-action-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`
- `DESIGN_SPECS/framework-patterns/curve-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`
- `DESIGN_SPECS/framework-patterns/display-execution-invariant-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/dual-axis-y3-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`
- `DESIGN_SPECS/framework-patterns/dynamic-library-strategy-loading-pattern.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`
- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`
- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`
- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`
- `DESIGN_SPECS/framework-patterns/heterogeneous-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`
- `DESIGN_SPECS/framework-patterns/hybrid-reconciliation-cadence-pattern.md`
- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`
- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`
- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`
- `DESIGN_SPECS/framework-patterns/manual-fields-inventory-pattern.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/multi-action-registry-walker-family.md`
- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`
- `DESIGN_SPECS/framework-patterns/multi-state-dispatch-with-per-state-update-metadata.md`
- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`
- `DESIGN_SPECS/framework-patterns/per-instance-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`
- `DESIGN_SPECS/framework-patterns/persisted-struct-with-ephemeral-field-coexistence-pattern.md`
- `DESIGN_SPECS/framework-patterns/postloadsetup-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-tuple-as-single-source-of-truth.md`
- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`
- `DESIGN_SPECS/framework-patterns/sidecar-override-pattern-for-registry-auto-flows.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/framework-patterns/sink-fn-pointer-for-optional-side-effect-pattern.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/slow-path-gate-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`
- `DESIGN_SPECS/framework-patterns/tls-session-resumption-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-erased-per-core-resource-handle-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/framework-patterns/x-macro-registry-with-presence-dispatch.md`
- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`
- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/doc-frontmatter-convention.md`
- `DESIGN_SPECS/meta-disciplines/doc-tag-vocabulary.md`
- `DESIGN_SPECS/meta-disciplines/implementation-layer-blindspot-taxonomy.md`
- `DESIGN_SPECS/meta-disciplines/pattern-codification-lifecycle.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `DESIGN_SPECS/meta-disciplines/structural-fix-preferred-decision-framework.md`
- `DESIGN_SPECS/plan-templates/design-spec-template.md`
- `DESIGN_SPECS/plan-templates/sprint-master-plan-template.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-flag-eligibility-criteria.md`
- `DESIGN_SPECS/refactor-patterns/cfg-scope-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-section-parser-state-machine.md`
- `DESIGN_SPECS/refactor-patterns/cross-walker-struct-field-uniqueness-discipline.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/refactor-patterns/generic-ring-buffer-template-pattern.md`
- `DESIGN_SPECS/refactor-patterns/orchestration-helper-with-pod-args-pattern.md`
- `DESIGN_SPECS/refactor-patterns/post-parse-normalize-with-explicit-key-bitmap-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/shared-helper-extract-for-train-serve-mirror-close.md`
- `DESIGN_SPECS/refactor-patterns/template-deferred-dependency-injection.md`
- `DESIGN_SPECS/wire-format-patterns/pre-post-cfg-registry-split-for-emit-order-preservation.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-canonical-body-invariants-helper.md`
- `claude-skills/accept-handoff/SKILL.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/capture-audit/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/index-rebuild/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/metadata-audit/SKILL.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/plan-draft/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `claude-skills/registry-fit-audit/SKILL.md`
- `claude-skills/second-opinion/SKILL.md`
- `claude-skills/strategy-template/SKILL.md`
- `claude-skills/trace-deps/SKILL.md`
- `memory/feedback_audit_canonical_sister_before_new_infra.md`
- `memory/feedback_auto_route_input_to_matching_skill.md`
- `memory/feedback_categorize_by_consumer_pattern_not_field_name.md`
- `memory/feedback_cfg_field_categorization_at_registry_add_time.md`
- `memory/feedback_defer_to_source_authority_for_external_semantics.md`
- `memory/feedback_enumerate_consumers_before_registry_row_deletion.md`
- `memory/feedback_framework_layer_payoff_diminishing_returns.md`
- `memory/feedback_plans_cite_sister_registry_inspection.md`
- `memory/feedback_prefer_action_parameterized_walker_over_per_consumer_walker_bodies.md`
- `memory/feedback_run_doc_ci_tools_first_never_hand_verify.md`
- `memory/feedback_structural_fix_for_recurring_class.md`
- `memory/feedback_verify_every_enumerated_site_at_close.md`

### framework-pattern (2 files)

- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`

### future-aware (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`

### future-expansion (3 files)

- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`

### future-oriented (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### grace-period (1 files)

- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### gui-deprecation (1 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`

### h13-compliance (1 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`

### h5 (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### h9 (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### handoff (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### hardware-required (1 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`

### header-only (2 files)

- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`
- `memory/feedback_cpp17_inline_variable_for_shared_state_across_tus.md`

### headless-service (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### hierarchical (1 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### hierarchical-config (1 files)

- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`

### hot-path (1 files)

- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`

### hot-reload (2 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`
- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### hybrid-cadence (1 files)

- `DESIGN_SPECS/framework-patterns/hybrid-reconciliation-cadence-pattern.md`

### ibkr (1 files)

- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`

### implementation-contract (1 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`

### in-code-documentation (2 files)

- `DESIGN_SPECS/doc-disciplines/format-input-space-taxonomy.md`
- `DESIGN_SPECS/doc-disciplines/in-code-doc-system-north-star.md`

### inline-variable (1 files)

- `memory/feedback_cpp17_inline_variable_for_shared_state_across_tus.md`

### instantiate (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### institutional-memory (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### io-uring (2 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`

### isolcpus (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### iteration-spiral (1 files)

- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`

### jsonl (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### keepalive (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`

### kernel-bypass (1 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`

### kernel-bypass-lite (1 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### kernel-tls (1 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`

### kill-switch (1 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### ktls (1 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`

### latency (1 files)

- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`

### latency-discipline (23 files)

- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-snapshot-publish-cluster-isolation.md`
- `DESIGN_SPECS/concurrency-patterns/phase-separated-drainer-for-safe-cross-temporal-derives.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-ring-embedded-in-hot-struct-cluster-discipline.md`
- `DESIGN_SPECS/data-disciplines/cache-layout-discipline-for-hot-side-structs.md`
- `DESIGN_SPECS/data-disciplines/cache-line-discipline.md`
- `DESIGN_SPECS/data-disciplines/decision-first-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/function-struct-alignment-for-single-mov-access.md`
- `DESIGN_SPECS/data-disciplines/hot-side-array-element-alignment-for-sparse-access.md`
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/register-spill-discipline.md`
- `DESIGN_SPECS/feature-patterns/runtime-toggleable-bench-gate-pattern.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/latency-vs-cache-decision-framework.md`
- `DESIGN_SPECS/refactor-patterns/loop-fusion-pattern.md`
- `DESIGN_SPECS/refactor-patterns/sliding-window-online-statistics-pattern.md`
- `DESIGN_SPECS/refactor-patterns/slow-path-cfg-resolution-cache-pattern.md`
- `claude-skills/finding-analyzer/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/latency-track/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`

### latency-histograms (1 files)

- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`

### ledger-discipline (7 files)

- `DESIGN_SPECS/ledger-templates/ledger-entry-templates.md`
- `claude-skills/latency-track/SKILL.md`
- `claude-skills/ship/SKILL.md`
- `memory/feedback_archived_changelog_preservation_discipline.md`
- `memory/feedback_bump_version_per_ship.md`
- `memory/feedback_forward_promise_auto_write_verification.md`
- `memory/feedback_tag_disposition_at_fix_time.md`

### linker-deduplication (1 files)

- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`

### linux-specific (1 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### live-trading (2 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### locale (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### machine-portable (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### market-data-fan-out (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`

### max-per-node (1 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`

### mbs-encoding (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### meta-discipline (50 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/audit-driven-pre-coding-gate.md`
- `DESIGN_SPECS/audit-methodologies/audit-scope-taxonomy.md`
- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`
- `DESIGN_SPECS/doc-disciplines/categorical-triggers-in-always-loaded-docs.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`
- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`
- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`
- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `DESIGN_SPECS/meta-disciplines/doc-frontmatter-convention.md`
- `DESIGN_SPECS/meta-disciplines/doc-tag-vocabulary.md`
- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`
- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`
- `DESIGN_SPECS/meta-disciplines/implementation-layer-blindspot-taxonomy.md`
- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`
- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`
- `DESIGN_SPECS/meta-disciplines/pattern-codification-lifecycle.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `DESIGN_SPECS/meta-disciplines/single-source-of-truth-discipline.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `DESIGN_SPECS/meta-disciplines/structural-fix-preferred-decision-framework.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `claude-skills/accept-handoff/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/capture-audit/SKILL.md`
- `claude-skills/second-opinion/SKILL.md`
- `memory/feedback_auto_route_input_to_matching_skill.md`
- `memory/feedback_capture_and_check_are_model_bounded.md`
- `memory/feedback_categorical_triggers_over_hardcoded_refs.md`
- `memory/feedback_guard_matrix_bounds_foundation_hardening.md`
- `memory/feedback_implementation_detail_blindspot_recovery_via_taxonomy.md`
- `memory/feedback_independence_for_judgment_not_mechanical.md`
- `memory/feedback_iteration_spiral_signals_audit_meta_gap.md`
- `memory/feedback_phased_pre_rework_correctness_foundation.md`
- `memory/feedback_process_weight_by_surface_blast_radius.md`
- `memory/feedback_run_doc_ci_tools_first_never_hand_verify.md`
- `memory/feedback_structural_enforcement_when_memory_insufficient.md`
- `memory/feedback_train_serve_execution_layer_meta_gap.md`
- `memory/feedback_two_foundations_determinism_vs_correctness.md`
- `memory/feedback_verify_every_enumerated_site_at_close.md`
- `memory/project_engine_done_edge_is_the_frontier.md`
- `memory/project_no_live_models_dev_test_only.md`

### meta-error-tracking (1 files)

- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`

### meta-registry (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`

### metrics (1 files)

- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`

### metrics-publication (1 files)

- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### migration-discipline (8 files)

- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `memory/feedback_archived_changelog_preservation_discipline.md`
- `memory/feedback_close_the_class_vs_migrate_every_site.md`
- `memory/feedback_domain_guards_for_bulk_transforms.md`
- `memory/feedback_proactive_rename_candidate_surfacing.md`
- `memory/feedback_surface_operator_migration_path_proactively.md`
- `memory/feedback_terminology_evolution_bridge_not_history_rewrite.md`

### mirror-prevention (1 files)

- `DESIGN_SPECS/meta-disciplines/single-source-of-truth-discipline.md`

### mmap (3 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`
- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### multi-asset (1 files)

- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`

### multi-exchange (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`

### multi-exchange-substrate (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`

### networking-stack (1 files)

- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`

### notcurses (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### numa (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### numa-aware (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`

### o1-compute (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### o1-latency (1 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`

### observability (1 files)

- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`

### oms-drainer (1 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`

### operator-collaboration (41 files)

- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md`
- `claude-skills/accept-handoff/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/handoff/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/second-opinion/SKILL.md`
- `claude-skills/ship/SKILL.md`
- `claude-skills/sync-models/SKILL.md`
- `claude-skills/sync-workspace/SKILL.md`
- `memory/feedback_address_user_as_caramel.md`
- `memory/feedback_audit_own_proposals_with_same_rigor.md`
- `memory/feedback_auto_pick_future_oriented.md`
- `memory/feedback_auto_route_input_to_matching_skill.md`
- `memory/feedback_backwards_compat_not_default_concern.md`
- `memory/feedback_close_out_now_over_defer_when_small.md`
- `memory/feedback_consult_on_audit_findings.md`
- `memory/feedback_deferral_reasons_merit_not_effort_or_context.md`
- `memory/feedback_design_once_maintain_forever.md`
- `memory/feedback_evaluate_options_on_robustness_latency_design_not_time.md`
- `memory/feedback_guards_compound_enforcement_is_leverage.md`
- `memory/feedback_lead_with_architectural_merit_not_operator_tone.md`
- `memory/feedback_listen_and_execute_simply.md`
- `memory/feedback_motivated_collaborator_for_caramel.md`
- `memory/feedback_never_skip_thoroughness_unless_explicit.md`
- `memory/feedback_no_defer_for_effort.md`
- `memory/feedback_no_question_boxes.md`
- `memory/feedback_operator_pushback_as_audit_signal.md`
- `memory/feedback_opportunistic_tech_debt_closure.md`
- `memory/feedback_plan_right_not_fast.md`
- `memory/feedback_process_weight_by_surface_blast_radius.md`
- `memory/feedback_recheck_designspecs_on_pushback.md`
- `memory/feedback_runtime_executor_mode_for_judgment_skills.md`
- `memory/feedback_sequential_audit_for_granular_operator_triage.md`
- `memory/feedback_structure_judgment_loop_not_output.md`
- `memory/feedback_surface_operator_migration_path_proactively.md`
- `memory/user_adhd_deferred_reward_discipline.md`
- `memory/user_deep_design_work_intrinsic_value.md`
- `memory/user_owns_core_design_meet_confidence_genuinely.md`
- `memory/user_structure_is_correctness_risk_control_for_capital.md`

### operator-policy (1 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-action-policy-pattern.md`

### operator-workflow (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### oss-project (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### paper-mode (1 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`

### paper-trading (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### parsing (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### pattern-codification (23 files)

- `DESIGN_SPECS/doc-disciplines/file-size-split-discipline.md`
- `DESIGN_SPECS/framework-patterns/categorical-tag-applicability-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/heterogeneous-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/manual-fields-inventory-pattern.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-tuple-as-single-source-of-truth.md`
- `DESIGN_SPECS/framework-patterns/sidecar-override-pattern-for-registry-auto-flows.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/pattern-codification-lifecycle.md`
- `DESIGN_SPECS/meta-disciplines/structural-fix-preferred-decision-framework.md`
- `DESIGN_SPECS/plan-templates/future-oriented-plan-template.md`
- `DESIGN_SPECS/refactor-patterns/cfg-flag-eligibility-criteria.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/plan-draft/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/registry-fit-audit/SKILL.md`

### pattern-selection (1 files)

- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`

### per-cluster-producer (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`

### per-cluster-resources (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`

### per-exchange (3 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`

### per-node-folders (1 files)

- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`

### per-node-io (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`

### per-node-isolation (1 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`

### per-node-mode (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### per-node-purity (1 files)

- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`

### persistent-connection (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`

### plan-decomposition (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`

### plan-hierarchy (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### plan-template (14 files)

- `DESIGN_SPECS/ledger-templates/ledger-entry-templates.md`
- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `DESIGN_SPECS/plan-templates/design-spec-template.md`
- `DESIGN_SPECS/plan-templates/future-oriented-plan-template.md`
- `DESIGN_SPECS/plan-templates/postmortem-template.md`
- `DESIGN_SPECS/plan-templates/sprint-master-plan-template.md`
- `claude-skills/doc-create/SKILL.md`
- `claude-skills/handoff/SKILL.md`
- `claude-skills/plan-check/SKILL.md`
- `claude-skills/plan-context-sweep/SKILL.md`
- `claude-skills/plan-draft/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `memory/feedback_new_plans_use_future_oriented_template.md`

### plan-trajectory-verification (1 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`

### planning (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### planning-discipline (13 files)

- `memory/feedback_address_med_low_findings_not_just_high_crit.md`
- `memory/feedback_auto_pick_future_oriented.md`
- `memory/feedback_new_plans_use_future_oriented_template.md`
- `memory/feedback_no_mvp_for_plumbing_only_for_unknown_unknowns.md`
- `memory/feedback_plan_body_length_no_target_loc.md`
- `memory/feedback_plan_right_not_fast.md`
- `memory/feedback_plans_cite_sister_registry_inspection.md`
- `memory/feedback_plans_have_explicit_end_goal.md`
- `memory/feedback_session_decision_log_discipline.md`
- `memory/feedback_test_change_enumeration_per_plan_body.md`
- `memory/feedback_tiered_audit_discipline_per_plan_scope.md`
- `memory/feedback_verify_symbol_existence_at_plan_drafting_time.md`
- `memory/project_e_series_is_vision_convergence_not_scope_balloon.md`

### post-coding (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### power-user-design (1 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`

### pre-coding-gate (1 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`

### privacy-boundary (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### process-state (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### production-mode (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### project-state (8 files)

- `memory/feedback_bump_version_per_ship.md`
- `memory/project_anti_spaghetti_audit_cadence.md`
- `memory/project_e_series_is_vision_convergence_not_scope_balloon.md`
- `memory/project_engine_clauder_md_is_symlink.md`
- `memory/project_engine_done_edge_is_the_frontier.md`
- `memory/project_foxml_suite_refactor_queued.md`
- `memory/user_mvp_to_professional_transition.md`
- `memory/user_public_work_attracts_hedge_funds.md`

### prometheus (2 files)

- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### protocol-normalization (1 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`

### push-model (1 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`

### read-only (2 files)

- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`
- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### reconcile-recovery (1 files)

- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`

### reconciliation (2 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`
- `DESIGN_SPECS/framework-patterns/hybrid-reconciliation-cadence-pattern.md`

### reconnect (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`

### reconnect-optimization (1 files)

- `DESIGN_SPECS/framework-patterns/tls-session-resumption-pattern.md`

### refactor-discipline (9 files)

- `DESIGN_SPECS/meta-disciplines/single-source-of-truth-discipline.md`
- `memory/feedback_avoid_substring_replace_all_on_member_access.md`
- `memory/feedback_enumerate_block_scope_statics_before_hoist.md`
- `memory/feedback_enumerate_helper_signature_args_before_extract.md`
- `memory/feedback_machine_portable_resolver_for_committed_tool_paths.md`
- `memory/feedback_prefer_action_parameterized_walker_over_per_consumer_walker_bodies.md`
- `memory/feedback_reduce_touch_sites.md`
- `memory/feedback_rename_enumerator_is_world2_aid_compiler_is_world1.md`
- `memory/project_foxml_suite_refactor_queued.md`

### refactor-pattern (2 files)

- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`

### repo-hygiene (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### reserve (1 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`

### risk-management (1 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`

### rolling-window (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### running-aggregate (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### safety (1 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### sanitizers (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### scaffold-lifecycle (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### scale-invariance (4 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`

### schema-completion (1 files)

- `DESIGN_SPECS/doc-disciplines/format-input-space-taxonomy.md`

### scope-discipline (24 files)

- `DESIGN_SPECS/audit-methodologies/audit-finding-kind-taxonomy.md`
- `memory/feedback_address_med_low_findings_not_just_high_crit.md`
- `memory/feedback_auto_pick_future_oriented.md`
- `memory/feedback_backwards_compat_not_default_concern.md`
- `memory/feedback_close_out_now_over_defer_when_small.md`
- `memory/feedback_close_the_class_vs_migrate_every_site.md`
- `memory/feedback_consult_indexes_before_full_reads.md`
- `memory/feedback_deferral_reasons_merit_not_effort_or_context.md`
- `memory/feedback_design_once_maintain_forever.md`
- `memory/feedback_document_as_you_go_over_catch_at_end.md`
- `memory/feedback_dont_measure_structural_work_by_loc.md`
- `memory/feedback_evaluate_options_on_robustness_latency_design_not_time.md`
- `memory/feedback_framework_layer_payoff_diminishing_returns.md`
- `memory/feedback_future_headache_vs_optimization_scope_framework.md`
- `memory/feedback_guard_matrix_bounds_foundation_hardening.md`
- `memory/feedback_guards_compound_enforcement_is_leverage.md`
- `memory/feedback_heavier_default_audit_posture_for_capital.md`
- `memory/feedback_listen_and_execute_simply.md`
- `memory/feedback_no_defer_for_effort.md`
- `memory/feedback_no_mvp_for_plumbing_only_for_unknown_unknowns.md`
- `memory/feedback_opportunistic_tech_debt_closure.md`
- `memory/feedback_overengineering_boundary_when_future_easier.md`
- `memory/feedback_proportionate_response_to_audit_findings.md`
- `memory/feedback_verify_by_context_not_count.md`

### seqlock (1 files)

- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`

### seqlock-blackboard (1 files)

- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`

### sequence-numbers (1 files)

- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`

### session-continuity (6 files)

- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `memory/feedback_compaction_degrades_treat_handoffs_as_hints.md`
- `memory/feedback_forward_promise_auto_write_verification.md`
- `memory/feedback_session_decision_log_discipline.md`
- `memory/feedback_sister_cohort_amendment_completeness.md`
- `memory/feedback_skill_edit_cohort_checklist.md`

### session-management (1 files)

- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`

### sha256-chain (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### shadow-mode (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### shared-state (2 files)

- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`
- `memory/feedback_cpp17_inline_variable_for_shared_state_across_tus.md`

### sidecar (1 files)

- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`

### simd (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### single-writer (2 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`
- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`

### sister-cohort (1 files)

- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`

### soa (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### spsc-rings (1 files)

- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`

### ssot (16 files)

- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/single-source-of-truth-discipline.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `memory/feedback_defer_to_source_authority_for_external_semantics.md`
- `memory/feedback_golden_master_over_reimplemented_oracle.md`
- `memory/feedback_machine_portable_resolver_for_committed_tool_paths.md`
- `memory/feedback_single_source_of_truth_discipline.md`
- `memory/feedback_single_source_the_computation_not_just_the_mode.md`
- `memory/feedback_tag_disposition_at_fix_time.md`

### state-persistence (1 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`

### static-analysis (2 files)

- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`

### strategy-lifecycle (1 files)

- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### strategy-loader (1 files)

- `DESIGN_SPECS/framework-patterns/dynamic-library-strategy-loading-pattern.md`

### structural-enforcement (2 files)

- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`

### structural-fix (104 files)

- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/concurrency-patterns/phase-separated-drainer-for-safe-cross-temporal-derives.md`
- `DESIGN_SPECS/data-disciplines/aggressive-memory-reduction-techniques.md`
- `DESIGN_SPECS/doc-disciplines/categorical-triggers-in-always-loaded-docs.md`
- `DESIGN_SPECS/doc-disciplines/file-size-split-discipline.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-from-arity-macro-family.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-pattern-for-production-caller-class.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/calibration-log-column-registry.md`
- `DESIGN_SPECS/framework-patterns/categorical-tag-applicability-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/composed-filter-mask-pattern.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/display-execution-invariant-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/framework-patterns/manual-fields-inventory-pattern.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/multi-action-registry-walker-family.md`
- `DESIGN_SPECS/framework-patterns/multi-state-dispatch-with-per-state-update-metadata.md`
- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/per-instance-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/persisted-struct-with-ephemeral-field-coexistence-pattern.md`
- `DESIGN_SPECS/framework-patterns/postloadsetup-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-tuple-as-single-source-of-truth.md`
- `DESIGN_SPECS/framework-patterns/sidecar-override-pattern-for-registry-auto-flows.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/slow-path-gate-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/framework-patterns/type-erased-per-core-resource-handle-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/framework-patterns/x-macro-registry-with-presence-dispatch.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `DESIGN_SPECS/meta-disciplines/structural-fix-preferred-decision-framework.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-flag-eligibility-criteria.md`
- `DESIGN_SPECS/refactor-patterns/cfg-scope-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-section-parser-state-machine.md`
- `DESIGN_SPECS/refactor-patterns/cross-walker-struct-field-uniqueness-discipline.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/refactor-patterns/generic-ring-buffer-template-pattern.md`
- `DESIGN_SPECS/refactor-patterns/multi-bit-state-encoding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/orchestration-helper-with-pod-args-pattern.md`
- `DESIGN_SPECS/refactor-patterns/post-parse-normalize-with-explicit-key-bitmap-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/shared-helper-extract-for-train-serve-mirror-close.md`
- `DESIGN_SPECS/refactor-patterns/slow-path-cfg-resolution-cache-pattern.md`
- `DESIGN_SPECS/refactor-patterns/template-deferred-dependency-injection.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/pre-post-cfg-registry-split-for-emit-order-preservation.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-canonical-body-invariants-helper.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/capture-audit/SKILL.md`
- `claude-skills/dead-code-trace/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/dust/SKILL.md`
- `claude-skills/foxlib-promotion/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/plan-context-sweep/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/registry-fit-audit/SKILL.md`
- `claude-skills/strategy-template/SKILL.md`
- `claude-skills/trace-deps/SKILL.md`
- `memory/feedback_audit_canonical_sister_before_new_infra.md`
- `memory/feedback_categorize_by_consumer_pattern_not_field_name.md`
- `memory/feedback_cfg_field_categorization_at_registry_add_time.md`
- `memory/feedback_close_the_class_vs_migrate_every_site.md`
- `memory/feedback_design_once_maintain_forever.md`
- `memory/feedback_domain_guards_for_bulk_transforms.md`
- `memory/feedback_golden_master_over_reimplemented_oracle.md`
- `memory/feedback_guards_compound_enforcement_is_leverage.md`
- `memory/feedback_paste_tool_output_dont_summarize.md`
- `memory/feedback_phased_pre_rework_correctness_foundation.md`
- `memory/feedback_reduce_touch_sites.md`
- `memory/feedback_rename_enumerator_is_world2_aid_compiler_is_world1.md`
- `memory/feedback_single_source_of_truth_discipline.md`
- `memory/feedback_structural_enforcement_when_memory_insufficient.md`
- `memory/feedback_structural_fix_for_recurring_class.md`
- `memory/feedback_wire_context_vs_cfg_file_parser_separation.md`

### structured-logging (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### sub-account-hierarchy (1 files)

- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`

### sub-accounts (2 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`

### sub-master (1 files)

- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### sub-sprint-discipline (3 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`
- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`
- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### subagent-arming (1 files)

- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`

### submit-protocol (1 files)

- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`

### symbol-normalization (1 files)

- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`

### tag-system (2 files)

- `DESIGN_SPECS/doc-disciplines/format-input-space-taxonomy.md`
- `DESIGN_SPECS/doc-disciplines/in-code-doc-system-north-star.md`

### terminology-evolution (2 files)

- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`

### test-discipline (5 files)

- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `memory/feedback_domain_guards_for_bulk_transforms.md`
- `memory/feedback_golden_master_over_reimplemented_oracle.md`
- `memory/feedback_passing_test_is_not_verification.md`
- `memory/feedback_test_change_enumeration_per_plan_body.md`

### test-infrastructure (1 files)

- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`

### thread-topology (2 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`

### tls-resumption (1 files)

- `DESIGN_SPECS/framework-patterns/tls-session-resumption-pattern.md`

### tooling (1 files)

- `DESIGN_SPECS/doc-disciplines/in-code-doc-system-north-star.md`

### trustworthy-sim (1 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`

### tt-dispatch (2 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`
- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`

### tui (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### type-traits (1 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`

### user-profile (6 files)

- `memory/user_adhd_deferred_reward_discipline.md`
- `memory/user_deep_design_work_intrinsic_value.md`
- `memory/user_mvp_to_professional_transition.md`
- `memory/user_owns_core_design_meet_confidence_genuinely.md`
- `memory/user_public_work_attracts_hedge_funds.md`
- `memory/user_structure_is_correctness_risk_control_for_capital.md`

### userspace-networking (1 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`

### userspace-tls (1 files)

- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### verification (4 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`

### versioning (1 files)

- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`

### viewer (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### viewer-split (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### websocket (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`

### wire-format (19 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/framework-patterns/calibration-log-column-registry.md`
- `DESIGN_SPECS/framework-patterns/persisted-struct-with-ephemeral-field-coexistence-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/pre-post-cfg-registry-split-for-emit-order-preservation.md`
- `DESIGN_SPECS/wire-format-patterns/prng-choice-for-replay-determinism.md`
- `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-canonical-body-invariants-helper.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `memory/feedback_defer_to_source_authority_for_external_semantics.md`
- `memory/feedback_train_serve_execution_layer_meta_gap.md`
- `memory/feedback_two_foundations_determinism_vs_correctness.md`
- `memory/feedback_wire_context_vs_cfg_file_parser_separation.md`
- `memory/project_no_live_models_dev_test_only.md`

### wontfix-rationale (1 files)

- `DESIGN_SPECS/doc-disciplines/file-size-split-discipline.md`

### workflow (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### workspace (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### workspace-hygiene (1 files)

- `DESIGN_SPECS/meta-disciplines/meta-anti-pattern-index.md`

### x-macro-registry (2 files)

- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`

## SURFACE tags

### accept-handoff (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### account-state (1 files)

- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`

### accounting (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### adapter-dispatch (2 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`

### adapter-implementation (2 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`
- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`

### aggregator (6 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`
- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`
- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`
- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### atomic-state (1 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`

### audit-cycle (1 files)

- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`

### audit-gate (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### audit-log (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### audit-orchestration (2 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`
- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`

### backtest (5 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/shared-helper-extract-for-train-serve-mirror-close.md`
- `DESIGN_SPECS/wire-format-patterns/prng-choice-for-replay-determinism.md`
- `claude-skills/accounting-audit/SKILL.md`

### bitmap-packed (14 files)

- `DESIGN_SPECS/data-disciplines/aggressive-memory-reduction-techniques.md`
- `DESIGN_SPECS/data-disciplines/partner-core-bitmap-pattern.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/composed-filter-mask-pattern.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/multi-bit-state-encoding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/post-parse-normalize-with-explicit-key-bitmap-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/transient-aggregation-bitmap-pattern.md`
- `claude-skills/dod-audit/SKILL.md`

### boot-sequence (5 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-action-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`
- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`

### boot-time (11 files)

- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`
- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/postloadsetup-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`
- `DESIGN_SPECS/refactor-patterns/shared-helper-extract-for-train-serve-mirror-close.md`

### bug-fix (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### build (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### build-system (1 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`

### cache (1 files)

- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`

### capital-allocation (1 files)

- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`

### capital-management (4 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`
- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### cfg-flow (28 files)

- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/framework-patterns/categorical-tag-applicability-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/sidecar-override-pattern-for-registry-auto-flows.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/refactor-patterns/cfg-flag-eligibility-criteria.md`
- `DESIGN_SPECS/refactor-patterns/cfg-scope-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-section-parser-state-machine.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/post-parse-normalize-with-explicit-key-bitmap-pattern.md`
- `DESIGN_SPECS/refactor-patterns/slow-path-cfg-resolution-cache-pattern.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `claude-skills/trace-deps/SKILL.md`

### cfg-parser (1 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`

### ci-tooling (23 files)

- `DESIGN_SPECS/audit-methodologies/audit-report-format.md`
- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`
- `DESIGN_SPECS/feature-patterns/runtime-toggleable-bench-gate-pattern.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `claude-skills/dead-code-trace/SKILL.md`
- `claude-skills/metadata-audit/SKILL.md`
- `claude-skills/ship/SKILL.md`

### cluster-state (2 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`

### command-channel (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### compliance (1 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`

### concurrency (1 files)

- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`

### config-parser (3 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`

### credentials-loading (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`

### cross-tool (2 files)

- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`

### deletion-decisions (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### deployment (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### doc-pipeline (11 files)

- `DESIGN_SPECS/doc-disciplines/file-size-split-discipline.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/doc-disciplines/module-scoped-claude-md-pattern.md`
- `DESIGN_SPECS/doc-disciplines/toolchain-semantic-versioning.md`
- `DESIGN_SPECS/framework-patterns/doc-intelligence-toolchain-architecture.md`
- `DESIGN_SPECS/framework-patterns/isolated-per-struct-layout-probe.md`
- `DESIGN_SPECS/framework-patterns/one-action-toolchain-update-orchestrator.md`
- `DESIGN_SPECS/framework-patterns/standardized-tool-io-envelope-and-payload.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `claude-skills/capture-audit/SKILL.md`

### dpdk (1 files)

- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### drift-detection (1 files)

- `DESIGN_SPECS/framework-patterns/hybrid-reconciliation-cadence-pattern.md`

### engine-binary (2 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`
- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### exchange-adapters (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`

### exchange-connection (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`

### filesystem (2 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`
- `DESIGN_SPECS/framework-patterns/hierarchical-config-with-per-node-folders.md`

### fill-path (1 files)

- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`

### fix-adapter (1 files)

- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`

### framework-genericity (1 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-implementation-contract.md`

### gui-thread (6 files)

- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md`
- `DESIGN_SPECS/framework-patterns/display-execution-invariant-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `claude-skills/strategy-template/SKILL.md`

### handoff (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### handoff-pipeline (2 files)

- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `claude-skills/accept-handoff/SKILL.md`

### header-split (1 files)

- `DESIGN_SPECS/meta-disciplines/implementation-layer-blindspot-taxonomy.md`

### helper-extraction (1 files)

- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`

### hot-path (57 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-snapshot-publish-cluster-isolation.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-ring-embedded-in-hot-struct-cluster-discipline.md`
- `DESIGN_SPECS/data-disciplines/aggressive-memory-reduction-techniques.md`
- `DESIGN_SPECS/data-disciplines/cache-layout-discipline-for-hot-side-structs.md`
- `DESIGN_SPECS/data-disciplines/cache-line-discipline.md`
- `DESIGN_SPECS/data-disciplines/decision-first-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/function-struct-alignment-for-single-mov-access.md`
- `DESIGN_SPECS/data-disciplines/hot-side-array-element-alignment-for-sparse-access.md`
- `DESIGN_SPECS/data-disciplines/partner-core-bitmap-pattern.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/data-disciplines/register-spill-discipline.md`
- `DESIGN_SPECS/feature-patterns/runtime-toggleable-bench-gate-pattern.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/display-execution-invariant-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-axis-y3-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/multi-state-dispatch-with-per-state-update-metadata.md`
- `DESIGN_SPECS/framework-patterns/per-bit-per-core-override-pattern.md`
- `DESIGN_SPECS/framework-patterns/sink-fn-pointer-for-optional-side-effect-pattern.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/slow-path-gate-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `DESIGN_SPECS/refactor-patterns/generic-ring-buffer-template-pattern.md`
- `DESIGN_SPECS/refactor-patterns/latency-vs-cache-decision-framework.md`
- `DESIGN_SPECS/refactor-patterns/loop-fusion-pattern.md`
- `DESIGN_SPECS/refactor-patterns/multi-bit-state-encoding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/transient-aggregation-bitmap-pattern.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/finding-analyzer/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/latency-track/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`

### hot-reload (1 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`

### hot-ring-fan-out (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`

### http-endpoint (1 files)

- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### ibkr-adapter (1 files)

- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`

### ibkr-protocol (1 files)

- `DESIGN_SPECS/framework-patterns/fix-session-management-pattern.md`

### inter-thread-communication (1 files)

- `DESIGN_SPECS/concurrency-patterns/spsc-vs-blackboard-selection-criteria.md`

### kernel-bypass-lite (1 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`

### kill-switch (1 files)

- `DESIGN_SPECS/framework-patterns/global-aggregator-readonly-pattern.md`

### kill-switch-latency (1 files)

- `DESIGN_SPECS/framework-patterns/event-sourced-aggregator-o1-pattern.md`

### latency-critical (2 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### live-trading (12 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/framework-patterns/single-authority-predicate-for-mode-gating.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `claude-skills/accounting-audit/SKILL.md`

### market-data-stream (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`

### memory-index (1 files)

- `claude-skills/capture-audit/SKILL.md`

### ml-inference (17 files)

- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/sliding-window-online-statistics-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/prng-choice-for-replay-determinism.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-canonical-body-invariants-helper.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/strategy-template/SKILL.md`
- `claude-skills/sync-models/SKILL.md`

### mmap (1 files)

- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### mmap-state (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### monitoring (1 files)

- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`

### multi-client-trading (1 files)

- `DESIGN_SPECS/framework-patterns/ibkr-fa-structure-pattern.md`

### multi-exchange-substrate (1 files)

- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`

### network-io (1 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### network-stack (2 files)

- `DESIGN_SPECS/concurrency-patterns/dpdk-userspace-networking-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`

### networking-init (1 files)

- `DESIGN_SPECS/framework-patterns/kernel-vs-userspace-networking-cfg-pattern.md`

### node-state (1 files)

- `DESIGN_SPECS/framework-patterns/cluster-node-hierarchy-filesystem-layout-pattern.md`

### observability (3 files)

- `DESIGN_SPECS/concurrency-patterns/structured-audit-log-pattern.md`
- `DESIGN_SPECS/framework-patterns/built-in-observability-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-format-metrics-publication-pattern.md`

### oms-drainer (25 files)

- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/phase-separated-drainer-for-safe-cross-temporal-derives.md`
- `DESIGN_SPECS/concurrency-patterns/spsc-ring-embedded-in-hot-struct-cluster-discipline.md`
- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/data-disciplines/raii-destructor-with-cluster-reorg-interaction.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `DESIGN_SPECS/refactor-patterns/template-deferred-dependency-injection.md`
- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/latency-track/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`

### paper-mode (2 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### paper-test (1 files)

- `DESIGN_SPECS/subsystem-designs/exit-chain-tp-sl-design.md`

### parser (7 files)

- `DESIGN_SPECS/framework-patterns/config-compiler-validation-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/refactor-patterns/cfg-scope-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cfg-section-parser-state-machine.md`
- `DESIGN_SPECS/refactor-patterns/post-parse-normalize-with-explicit-key-bitmap-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `claude-skills/parity-check/SKILL.md`

### per-cluster-resources (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`

### per-cluster-state (2 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### per-node-binding (1 files)

- `DESIGN_SPECS/framework-patterns/foreach-subaccount-meta-registry-pattern.md`

### per-node-io (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-io-rings-pattern.md`

### per-node-state (3 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`
- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### persistence (6 files)

- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/data-disciplines/fill-path-completeness-and-normalization-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-position-ownership-model.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`

### plan-iteration (1 files)

- `DESIGN_SPECS/meta-disciplines/iteration-spiral-signals-audit-meta-gap.md`

### plan-pipeline (6 files)

- `DESIGN_SPECS/meta-disciplines/body-content-enumeration-at-plan-time-discipline.md`
- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `DESIGN_SPECS/meta-disciplines/structural-enforcement-when-memory-insufficient.md`
- `claude-skills/capture-audit/SKILL.md`

### plan-time (1 files)

- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`

### planning (4 files)

- `DESIGN_SPECS/meta-disciplines/audit-driven-sub-sprint-trajectory-verification.md`
- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`
- `DESIGN_SPECS/meta-disciplines/plan-decomposition-and-future-aware-agent-arming.md`
- `DESIGN_SPECS/meta-disciplines/plan-hierarchy-and-sub-master-decomposition.md`

### portfolio (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### position-state (1 files)

- `DESIGN_SPECS/framework-patterns/portfolio-soa-vectorization-pattern.md`

### post-implementation (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### pre-commit (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### producer (4 files)

- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/latency-track/SKILL.md`

### producer-thread (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-producer-pattern.md`

### reconciliation (1 files)

- `DESIGN_SPECS/framework-patterns/hybrid-reconciliation-cadence-pattern.md`

### reconnect-path (1 files)

- `DESIGN_SPECS/framework-patterns/tls-session-resumption-pattern.md`

### recovery (2 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-action-policy-pattern.md`
- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`

### refactor (1 files)

- `DESIGN_SPECS/meta-disciplines/fix-toward-future-trajectory-not-static-state.md`

### refactor-scope (1 files)

- `DESIGN_SPECS/meta-disciplines/backwards-compat-not-default-concern.md`

### registry (67 files)

- `DESIGN_SPECS/audit-methodologies/audit-driven-pre-coding-gate.md`
- `DESIGN_SPECS/audit-methodologies/audit-scope-taxonomy.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-from-arity-macro-family.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-pattern-for-production-caller-class.md`
- `DESIGN_SPECS/framework-patterns/bitmap-flag-api.md`
- `DESIGN_SPECS/framework-patterns/calibration-log-column-registry.md`
- `DESIGN_SPECS/framework-patterns/categorical-tag-applicability-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/composed-filter-mask-pattern.md`
- `DESIGN_SPECS/framework-patterns/curve-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/display-execution-invariant-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/dual-axis-y3-dispatch-pattern.md`
- `DESIGN_SPECS/framework-patterns/enum-mode-flags-bitmap-lookup-pattern.md`
- `DESIGN_SPECS/framework-patterns/foreach-exchange-meta-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/heterogeneous-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/manual-fields-inventory-pattern.md`
- `DESIGN_SPECS/framework-patterns/meta-registry-pattern-for-codebase-registry-discipline.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/multi-action-registry-walker-family.md`
- `DESIGN_SPECS/framework-patterns/multi-state-dispatch-with-per-state-update-metadata.md`
- `DESIGN_SPECS/framework-patterns/per-instance-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/persisted-struct-with-ephemeral-field-coexistence-pattern.md`
- `DESIGN_SPECS/framework-patterns/postloadsetup-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/framework-patterns/registry-tuple-as-single-source-of-truth.md`
- `DESIGN_SPECS/framework-patterns/sidecar-override-pattern-for-registry-auto-flows.md`
- `DESIGN_SPECS/framework-patterns/slot-state-foreach-registry-with-storage-routing.md`
- `DESIGN_SPECS/framework-patterns/slow-path-gate-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/framework-patterns/type-erased-per-core-resource-handle-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/universal-registry-bitmap-dispatcher-pattern.md`
- `DESIGN_SPECS/framework-patterns/x-macro-registry-with-presence-dispatch.md`
- `DESIGN_SPECS/meta-disciplines/canonical-sister-extension-discipline.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/doc-frontmatter-convention.md`
- `DESIGN_SPECS/meta-disciplines/doc-tag-vocabulary.md`
- `DESIGN_SPECS/meta-disciplines/implementation-layer-blindspot-taxonomy.md`
- `DESIGN_SPECS/meta-disciplines/pattern-codification-lifecycle.md`
- `DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md`
- `DESIGN_SPECS/refactor-patterns/bitmap-overflow-protection-discipline.md`
- `DESIGN_SPECS/refactor-patterns/cross-walker-struct-field-uniqueness-discipline.md`
- `DESIGN_SPECS/refactor-patterns/orchestration-helper-with-pod-args-pattern.md`
- `DESIGN_SPECS/refactor-patterns/registry-bitmap-set-discipline.md`
- `DESIGN_SPECS/refactor-patterns/rename-cascade-enumeration-tooling.md`
- `DESIGN_SPECS/refactor-patterns/rename-ship-methodology.md`
- `DESIGN_SPECS/wire-format-patterns/pre-post-cfg-registry-split-for-emit-order-preservation.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/dead-code-trace/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/dust/SKILL.md`
- `claude-skills/finding-analyzer/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/readiness/SKILL.md`
- `claude-skills/registry-fit-audit/SKILL.md`
- `claude-skills/strategy-template/SKILL.md`
- `claude-skills/trace-deps/SKILL.md`

### registry-consumption (1 files)

- `DESIGN_SPECS/framework-patterns/exchange-adapter-tt-dispatch-pattern.md`

### replay (1 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`

### repo-structure (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### risk-isolation (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-economic-isolation-pattern.md`

### risk-limits (1 files)

- `DESIGN_SPECS/framework-patterns/capital-allocation-policy-pattern.md`

### runtime-mutation (2 files)

- `DESIGN_SPECS/framework-patterns/runtime-mutable-vs-boot-time-config-pattern.md`
- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### session-pickup (3 files)

- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `DESIGN_SPECS/meta-disciplines/session-decision-log-discipline.md`
- `claude-skills/accept-handoff/SKILL.md`

### ship-close (1 files)

- `DESIGN_SPECS/audit-methodologies/post-implementation-verification-v-class.md`

### skill-pipeline (2 files)

- `DESIGN_SPECS/meta-disciplines/sister-cohort-amendment-completeness-discipline.md`
- `claude-skills/capture-audit/SKILL.md`

### slow-path (56 files)

- `DESIGN_SPECS/audit-methodologies/adversarial-multi-agent-audit-methodology.md`
- `DESIGN_SPECS/audit-methodologies/characterization-test-discipline.md`
- `DESIGN_SPECS/audit-methodologies/static-latency-path-conformance-analysis.md`
- `DESIGN_SPECS/concurrency-patterns/concurrency-model-summary.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-multiword-read-consistency-discipline.md`
- `DESIGN_SPECS/concurrency-patterns/cross-thread-snapshot-publish-cluster-isolation.md`
- `DESIGN_SPECS/concurrency-patterns/phase-separated-drainer-for-safe-cross-temporal-derives.md`
- `DESIGN_SPECS/data-disciplines/cache-layout-discipline-for-hot-side-structs.md`
- `DESIGN_SPECS/data-disciplines/cache-line-discipline.md`
- `DESIGN_SPECS/data-disciplines/decision-first-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/function-struct-alignment-for-single-mov-access.md`
- `DESIGN_SPECS/data-disciplines/hot-side-array-element-alignment-for-sparse-access.md`
- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/data-disciplines/per-node-purity-scale-invariance.md`
- `DESIGN_SPECS/data-disciplines/per-snapshot-cluster-layout-pattern.md`
- `DESIGN_SPECS/data-disciplines/raii-destructor-with-cluster-reorg-interaction.md`
- `DESIGN_SPECS/data-disciplines/register-spill-discipline.md`
- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md`
- `DESIGN_SPECS/feature-patterns/runtime-toggleable-bench-gate-pattern.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/framework-patterns/cfg-field-categorization-discipline.md`
- `DESIGN_SPECS/framework-patterns/curve-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/sink-fn-pointer-for-optional-side-effect-pattern.md`
- `DESIGN_SPECS/framework-patterns/slow-path-gate-registry-pattern.md`
- `DESIGN_SPECS/framework-patterns/type-erased-per-core-resource-handle-pattern.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/representation-migration-completeness.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/meta-disciplines/train-serve-execution-layer-parity.md`
- `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`
- `DESIGN_SPECS/refactor-patterns/branchless-math-kernel-pattern.md`
- `DESIGN_SPECS/refactor-patterns/critical-moment-determinism-over-average-latency.md`
- `DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md`
- `DESIGN_SPECS/refactor-patterns/failure-attribution-buffer-pattern.md`
- `DESIGN_SPECS/refactor-patterns/generic-ring-buffer-template-pattern.md`
- `DESIGN_SPECS/refactor-patterns/latency-vs-cache-decision-framework.md`
- `DESIGN_SPECS/refactor-patterns/loop-fusion-pattern.md`
- `DESIGN_SPECS/refactor-patterns/shared-helper-extract-for-train-serve-mirror-close.md`
- `DESIGN_SPECS/refactor-patterns/sliding-window-online-statistics-pattern.md`
- `DESIGN_SPECS/refactor-patterns/slow-path-cfg-resolution-cache-pattern.md`
- `DESIGN_SPECS/refactor-patterns/template-deferred-dependency-injection.md`
- `DESIGN_SPECS/refactor-patterns/transient-aggregation-bitmap-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/anti-spaghetti/SKILL.md`
- `claude-skills/bug-check/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dependency-chain-trace/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/finding-analyzer/SKILL.md`
- `claude-skills/hft-audit/SKILL.md`
- `claude-skills/latency-track/SKILL.md`
- `claude-skills/merge-scan/SKILL.md`
- `claude-skills/patch-planner/SKILL.md`
- `claude-skills/post-ship-audit/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`

### slow-path-entry (1 files)

- `DESIGN_SPECS/framework-patterns/kill-switch-hierarchical-pattern.md`

### state-persistence (1 files)

- `DESIGN_SPECS/framework-patterns/crash-recovery-via-mmap-state-pattern.md`

### state-publication (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### statistics (1 files)

- `DESIGN_SPECS/data-disciplines/running-aggregate-vs-cycle-recompute-discipline.md`

### strategy-development (2 files)

- `DESIGN_SPECS/meta-disciplines/adversarial-pessimistic-simulation-discipline.md`
- `DESIGN_SPECS/meta-disciplines/backtest-paper-live-convergence-discipline.md`

### strategy-dispatch (1 files)

- `DESIGN_SPECS/framework-patterns/strategy-hot-reload-via-dlopen-pattern.md`

### strategy-loading (1 files)

- `DESIGN_SPECS/framework-patterns/dynamic-library-strategy-loading-pattern.md`

### subagent-delegation (1 files)

- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`

### submit-dispatch (1 files)

- `DESIGN_SPECS/framework-patterns/per-node-paper-mode-flag-pattern.md`

### submit-path (1 files)

- `DESIGN_SPECS/framework-patterns/per-exchange-submit-protocol-selection.md`

### symbol-parsing (1 files)

- `DESIGN_SPECS/framework-patterns/multi-asset-class-symbol-pattern.md`

### syscall-overhead (1 files)

- `DESIGN_SPECS/concurrency-patterns/io-uring-kernel-bypass-pattern.md`

### terminal-ui (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### test-infrastructure (10 files)

- `DESIGN_SPECS/audit-methodologies/audit-report-format.md`
- `DESIGN_SPECS/data-disciplines/cpp17-inline-variable-for-header-shared-state.md`
- `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md`
- `DESIGN_SPECS/framework-patterns/registry-coverage-ci-check-pattern.md`
- `DESIGN_SPECS/meta-disciplines/acceptance-oracle-totality-before-delegation.md`
- `DESIGN_SPECS/meta-disciplines/calibration-corpus-non-vacuity-discipline.md`
- `DESIGN_SPECS/meta-disciplines/definition-of-done-and-armed-scout-verification.md`
- `claude-skills/dead-code-trace/SKILL.md`
- `claude-skills/strategy-template/SKILL.md`
- `claude-skills/test-strength-audit/SKILL.md`

### thread-architecture (1 files)

- `DESIGN_SPECS/framework-patterns/per-cluster-shared-resource-pattern.md`

### thread-pinning (1 files)

- `DESIGN_SPECS/framework-patterns/dev-vs-production-thread-topology-pattern.md`

### tls-connection (1 files)

- `DESIGN_SPECS/framework-patterns/tls-session-resumption-pattern.md`

### tls-encryption (2 files)

- `DESIGN_SPECS/concurrency-patterns/ktls-kernel-tls-pattern.md`
- `DESIGN_SPECS/concurrency-patterns/userspace-tls-pattern.md`

### tooling (1 files)

- `DESIGN_SPECS/meta-disciplines/public-private-boundary-and-ecosystem-discipline.md`

### tools (1 files)

- `DESIGN_SPECS/meta-disciplines/handoff-active-state-machine.md`

### training (6 files)

- `DESIGN_SPECS/feature-patterns/per-horizon-barrier-blending-with-shadow-mode.md`
- `DESIGN_SPECS/feature-patterns/shadow-load-state-transition-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/prng-choice-for-replay-determinism.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/sync-models/SKILL.md`

### viewer (1 files)

- `DESIGN_SPECS/framework-patterns/native-tui-via-mmap-readonly-pattern.md`

### viewer-binaries (1 files)

- `DESIGN_SPECS/meta-disciplines/headless-engine-viewer-split-pattern.md`

### viewer-layer (1 files)

- `DESIGN_SPECS/meta-disciplines/gui-deprecation-decision-rationale.md`

### wire-format (29 files)

- `DESIGN_SPECS/data-disciplines/locale-determinism-discipline.md`
- `DESIGN_SPECS/framework-patterns/autopopulate-pattern-for-production-caller-class.md`
- `DESIGN_SPECS/framework-patterns/calibration-log-column-registry.md`
- `DESIGN_SPECS/framework-patterns/cfg-derived-consumer-framework.md`
- `DESIGN_SPECS/framework-patterns/framework-composition-overview.md`
- `DESIGN_SPECS/framework-patterns/metadata-bit-driven-derived-filter-framework.md`
- `DESIGN_SPECS/framework-patterns/persisted-struct-with-ephemeral-field-coexistence-pattern.md`
- `DESIGN_SPECS/framework-patterns/stamp-vs-runtime-drift-detection-registry.md`
- `DESIGN_SPECS/framework-patterns/type-trait-dispatch-via-tt-namespace.md`
- `DESIGN_SPECS/framework-patterns/universal-cfg-field-registry-pattern.md`
- `DESIGN_SPECS/meta-disciplines/dead-code-and-identifier-retirement-discipline.md`
- `DESIGN_SPECS/meta-disciplines/mechanical-verification-of-derived-code-facts.md`
- `DESIGN_SPECS/meta-disciplines/struct-change-cascade-impact-tooling.md`
- `DESIGN_SPECS/refactor-patterns/cross-walker-struct-field-uniqueness-discipline.md`
- `DESIGN_SPECS/refactor-patterns/framework-driven-cli-binary-pattern.md`
- `DESIGN_SPECS/refactor-patterns/orchestration-helper-with-pod-args-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/avx512-byte-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/pre-post-cfg-registry-split-for-emit-order-preservation.md`
- `DESIGN_SPECS/wire-format-patterns/struct-padding-determinism-pattern.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-byte-preservation-discipline.md`
- `DESIGN_SPECS/wire-format-patterns/wire-format-canonical-body-invariants-helper.md`
- `claude-skills/accounting-audit/SKILL.md`
- `claude-skills/blindspot-scan/SKILL.md`
- `claude-skills/decision-check/SKILL.md`
- `claude-skills/dod-audit/SKILL.md`
- `claude-skills/ml-audit/SKILL.md`
- `claude-skills/parity-check/SKILL.md`
- `claude-skills/precoding-audit-gate/SKILL.md`
- `claude-skills/readiness/SKILL.md`

### ws-api (1 files)

- `DESIGN_SPECS/concurrency-patterns/persistent-ws-connection-management-pattern.md`
