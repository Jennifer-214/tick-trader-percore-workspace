...
**Post-census instances:**
- *(2026-08-20, E.1.2.C tail — sub-shape A′ false-comment form, FOUND + FIXED same session, engine `753fbed`)* — TWO comments asserted a live relationship that was dead: `CoreFrameworks/HotSwap.hpp:54` included the legacy `EnsembleHotSwap.hpp` as *"kept as thin wrapper"* (nothing inside called it — the "wrapper" wrapped nothing), and `CoreFrameworks/EngineSharded/Run.hpp` claimed *"the call site is in EngineSharded_Run below"* (the call was deleted by the v5.15.4 shadow-load cutover; the comment survived it). Both were the load-bearing reason the dead header LOOKED alive to a reader — the same false-comment-is-why-the-sibling-lived mechanism as `StampHelper.hpp:227-229` (D-426). Fixed with the deletion; the stale `ModelValidation.hpp` CALLERS list (4 claimed, 3 real, 3-of-4 names drifted) corrected in the same commit.
...
| 12 | **A′** | **HIGH — LIVE, in a signed body** | **`inference_cfg_bandit_blend_ratio` — the `fees` twin, three lines above where `fees` was deleted.** `StampHelper.hpp:249-251` sets the row's own standalone bit and **never assigns its value**, so `StampInferenceCfgInputs inf = {}` zero-inits and the emitted line is permanently `inference_cfg_bandit_blend_ratio=0` — in the same HMAC-signed model-identity document as the truthful cfg-derived `bandit_blend_ratio=<real>`. Two contradictory claims about one quantity. **Measured by compiled probe**, not inferred: two byte counts, two signatures. Same migration-tail cause as `fees` (`.B.3` moved the producer to the cfg-derived half, the `STAMP_SET` stayed), and **armed** by the `bandit_enabled` 0→1 default flip at `5d45ecc`. **Why the sweep that killed `fees` missed it:** `StampHelper.hpp:227-229`, written *during* that sweep, asserts the row emits `=X`. There is no `=X`. A false comment is the whole reason the sibling lived — the A′ signature working exactly as documented, one day late. Found 2026-08-17, D-426, by two independent a-class agents. |
| 13 | **C** | HIGH | **The `.B.3` migration has no parse→handle leg.** `ModelHandle` DECLARES ~30 cfg-derived fields (`STAMP_RESULT_DERIVED_FIELDS_AUTO_GEN`) and `FOREACH_CFG_DRIFT_CHECK` READS them, but nothing WRITES them — verified: zero writers tree-wide; the only `memcpy`s into a handle are char arrays. Two `REFUSE_STRICT` rows therefore compare a permanent `0` against a cfg default of `1.0`, behind `COHORT_GATE_BANDIT_ENABLED` which reads **cfg only, not `STAMP_HAS`** — so unlike #3 and #9 these rows are genuinely REACHABLE and DO fire. **Discharged by compiled probe with a passing positive control** (`reports/2026-08-17-stamp-emit-gate-audit/orchestrator-drift-probe.md`). The inverse symptom of #3 at the same root cause: #3 is a gate that can never fire, this is a gate that always fires falsely. Severity is not "the engine won't boot" — at the shipped default it returns 0; the harm is that the drift channel is unusable in BOTH positions (off: saturated with guaranteed-false lines so real drift is indistinguishable; on: every model refuses, so the rational response is to disable the gate). PARITY-042. |

**#3 VERIFIED 2026-08-17.** The founding census recorded #3 on a walker whose "36 fields" figure the
2026-08-16 MASTER banner explicitly flagged as *not yet verified — do not inherit it*. A D-426
i-class discharged it: the vacuity is REAL and the cohort is ~30 `STAMP_BOUND_CFG_DERIVED` rows plus
the ML/gate-flag walkers, **and it is worse than recorded** — the same bit also gates 4 rows of a
SECOND registry (`CfgDriftCheckRegistry.hpp:257/:261/:266/:332`), three of them `REFUSE_STRICT`. Use
the PARITY-042 chain, not the census line, when working this.
