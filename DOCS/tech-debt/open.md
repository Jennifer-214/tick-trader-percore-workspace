...
- **Trigger:** a dedicated **post-paper-test ML-completion ship** (revive the `.F.4c.2/.4` + `.F.5` scope) OR fold into a future `.E` ML ship. Operator decides timing (2026-06-12: "address soon OR include in a future plan"). NOT `.E.1` (Core→Node rename) scope. Recommended: bundle A11/A12 + Thompson + dual-shadow into one ML-completion ship; write the bandit/ML-calibration **subsystem-design catalogue entry first** (document the intended producer→consumer wiring so the orphans are visible against intent).
