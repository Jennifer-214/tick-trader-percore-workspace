...
| `framework-driven-cli-binary-pattern.md` | Thin C++ CLI binary that calls engine framework API directly (no bash mirror); X-macro auto-gen of CLI flag table (longopts[] + value-receiver struct + parse dispatch via tt::cfg_parse_field from FOREACH_*_CFG_FIELD walkers) + extensibility test pattern (X-macro walker validates all flagged rows round-trip byte-identical). Structurally eliminates cross-tool wire-format mirror seam where bash scripts mirror engine wire emit (e.g., `tools/stamp_model.sh` tracked 6+ cross-tool sync events). Sister discipline to `wire-format-byte-preservation-discipline.md` Layer 7 (Layer 7 codifies cross-tool sync DISCIPLINE; this pattern STRUCTURALLY ELIMINATES at framework-driven surfaces). Stage 3 first canonical: `tools/stamp_model_cli.cpp` at v5.15.5.F.4d.1.B.3 Phase L (replaces `tools/stamp_model.sh`). Closes Class 18/19/21/22 at WIRE EMIT layer + CLI INTERFACE layer + TEST layer. | **NEW (v5.15.5.F.4d.1.B.3 Phase L 2026-05-18)** Stage 2 DRAFT v1.1 (v1.1 amendment expanded with X-macro auto-gen + extensibility test + Audit detection sections per /precoding-audit-gate 6-audit fire findings) | structural-fix, framework-discipline, cross-tool, cli-binary, wire-format, registry-driven, future-easier; closes Class 18+19+21+22 at WIRE EMIT + CLI INTERFACE + TEST layers; sister to Layer 7 (codifies DISCIPLINE; this pattern provides STRUCTURAL ELIMINATION at framework-driven surfaces); Stage 2 (DRAFT) — Stage 3 first canonical lands at `.B.3` Phase L |
