...
- **Disposition (per flag — operator-facing; wire ADDS behavior / tombstone REMOVES a knob):** A36 → **WIRE** (the real risk gap); A14 → tombstone/alias (H21); A13/A35 → wire-into-`Strategy_BuildParameters` OR tombstone; A37 → fold into A24. Full per-flag detail = the SSoT in the register § "#9/#10" rows (`:325-326` / `:392-394`) — NOT duplicated here.
- **Tracked (3 live homes):** the register (SSoT) + the active handoff TaskList row 6b + MECHANICALLY by `scan_class_44_cfg_orphan.py`'s `KNOWN_COHORT` (flags them KNOWN-PENDING every `/bug-check`; `--strict` red-builds a NEW orphan). This ledger entry = the durable debt home so it survives if `.E.0.10` defers a flag.
