...
### `.E.1` INTERNAL DECOMPOSITION (N=7, locked 2026-06-15 — D-225)

`.E.1` was too big for one ship → decomposed into **7 sub-ship leaves** (the plan-tree sub-master = `subplans/2026-05-28-…E.1-foundation.md` § DECOMPOSITION). Linear chain — each leaf's OUTBOUND seam = the next's INBOUND:

```
E.1.0 → E.1.1 → E.1.2 → E.1.3 ★ → E.1.4 → E.1.5 ★ → E.1.6
guards   rename   layout   aggregator   fill    purity   framework
(§4d)    +clean-  (SoA/    +torn-read   (SPINE   (SPINE   (FOREACH_
         break    Money)   +absorption  -1)      -3)      EXCHANGE)
                  FINAL    +conc-5
                           (SPINE-2)
```
- mechanical foundation **by-layer** (E.1.0/1/2) → capital superstructure **by-spine** (E.1.3 coherence / E.1.4 fill / E.1.5 purity) → framework last (E.1.6).
- ★ **HARD live-enable = E.1.3 ∧ E.1.5** (torn-read closure + per-cluster purity; live OFF till all `.E`/v5.16 per D-168).
- The downstream `.E.2..8` impacts (above) are satisfied by the SET — the substrate is delivered across the 7 leaves, not one ship. Per-leaf seams + the residual E.1.3→E.1.4 HOLE: D-225 + the sub-master.
