...
**~66 patterns total** (57 catalog + 4 NEW DRAFT pending .F.4d + 1 NEW DRAFT pending .F.4c.2 + 4 NEW DRAFT pending .F.4c.3). Adding new patterns: write the doc, add a row above with tags, cross-link from related docs. (Process meta-tip: follow `pattern-codification-lifecycle.md` — the meta-pattern that captures HOW to fully codify a new architectural discipline end-to-end.)
