...
### TECH_DEBT-182 — `Portfolio_Init`/`ClearPositions`/`AddPositionWithExits` zero a SUBSET of Position fields (original_tp/sl/pair_index/entry_ts survive reuse — A19 bug class)

- **id:** TECH_DEBT-182 · **severity:** medium (latent; capital-adjacent on slot reuse) · **opened:** 2026-06-12 · **status:** open · **surface_tags:** [portfolio, position-reset, stale-state, class-44, a19-sister, capital]
- **What (register A28):** `Portfolio_Init` (`Portfolio.hpp:216-225`) + `Portfolio_ClearPositions` (`:414-423`) zero only `quantity`/`entry_price`/`entry_fee`/`take_profit_price`/`stop_loss_price` — they leave `original_tp@80`/`original_sl@96`/`pair_index@120`/`entry_timestamp_us@112` STALE. `Portfolio_AddPositionWithExits` (`:257-271`) sets live TP/SL but never `original_*` (relies on a follow-up patch in the legacy caller). On a zero-init struct it's harmless; after a slot has been occupied and Init/Clear runs (paper-reset re-init), the stale `original_*` survives → the sharded trail readers gate `if (!Money_IsZero(orig_tp) && !Money_Gt(price, orig_tp)) continue;` on a PRIOR trade's anchor, and a stale `pair_index` can mis-pair partial-exit legs. **This is the A19 bug class one field over** (A19 = `ratchet_tp` never cleared).
- **Why (structural fix):** the bug is "reset zeroes a SUBSET"; the fix is reset the WHOLE struct (memset/zero-all in Init+ClearPositions) + set `original_*` inside `Portfolio_AddPositionWithExits` (self-consistent like `Portfolio_OpenSlot`). Closes the class for `original_*` + `pair_index` + `entry_timestamp_us` together. Durable-now candidate (fold into the A25 `Portfolio` touch).
- **Cross-ref:** register A28 + A19 (sister, `ratchet_tp` clear); the A25 set-site fix touches `Portfolio_OpenSlot` in the same file (natural co-fix moment); `Portfolio.hpp:216-225/414-423/257-271`.
...
- **id:** TECH_DEBT-186 · **severity:** medium (recurring capital-correctness class; A1 + A25 are canonical instances) · **opened:** 2026-06-12 · **status:** open · **surface_tags:** [anti-pattern-codification, single-source, restore-replay, class-candidate, design-spec, harvest-obligation]
- **What (register meta-lesson #1, harvest obligation):** a restore/reconstruct/replay path re-derives a value the FORWARD path computes, but reads a DIFFERENT source field → the two silently diverge. **A1** (warm-restart read GLOBAL `take_profit_pct`; entry read the per-strategy override) + **A25** (post-fix, event-log replay reconstructs `original_tp` from expected-entry `e.tp` while live sets `fill×(1+tp_pct)`) are the canonical instances. Distinct from Class 43 (D-190: same field, different FORMULA) and Class 18 (parallel emit/parse mirror); sibling to AR-7 (structural-pattern false-completeness).
- **Why:** structural fix = single-source the derivation BOTH paths call (the `ResolvePerFillTpPct` SSoT pattern A1 established). Promised in register meta-lesson #1 + the A1 TECH_DEBT but authored NOWHERE (prose-only). **Trigger/action:** author a new `Class` in `DOCS/RECURRING_BUG_PATTERNS.md` (auto-picked by `/bug-check`) + a `forward-vs-reconstruct-single-source.md` DESIGN_SPEC; A1/A25 = the 2 canonical applications (Stage-3-ready).
- **Cross-ref:** register meta-lesson #1 + the A25-impl note #5; A1 (closed) + A25; Class 43 / Class 18 (distinct); AR-7 (sibling).
