...
### TECH_DEBT-231 — clang layout-probe depends on a clang-clean flag path; the g++ compile_commands.json carries flags clang rejects
- **id:** TECH_DEBT-231 · **severity:** LOW · **opened:** 2026-07-05 · **status:** open · **surface_tags:** [ci-tooling, doc-pipeline, cache-layout-gate, plugin, clang, gpp, E.1.2.A]
- **What:** the E.1.2.A cache-layout gate (D-320) + the fox-symdeps HUD (`recordlayout`/`sizeprobe`) compute LAYOUT facts (size/align/offsets/straddle) via clang `-Xclang -fdump-record-layouts` (clang-only flag; Itanium-ABI-identical to the shipped g++ — D-321). But the engine's `compile_commands.json` is g++-oriented (compiler `/usr/bin/c++`; carries `-flto` / `-funroll-loops` + g++ diagnostics clang rejects), so a naive clang invocation over a TU errors. Worked around by filtering to the portable flag subset (`-I` / `-isystem` / `-D` / `-std`) — exactly what the plugin's `sizeprobe._filter_flags` already does. The dump still succeeds (555 records off `main.cpp`) but the filter is a band-aid, and the flag-set is a SHARED HUD+CI dependency: if the two filters drift, the HUD and the CI gate disagree on layout.
- **Severity rationale:** LOW — the layout dump WORKS today via the portable-flag filter (no correctness gap; ABI-identical); this is a robustness / SSoT concern (one flag-filter shared by HUD + CI, not two that drift) + a friction point (a stray non-portable flag breaks the probe). Not capital / wire / determinism.
- **Disposition / trigger:** at the cache-layout gate build (D-320) — reuse the plugin's `sizeprobe._filter_flags` as the SINGLE flag-producer for both HUD and CI (do NOT fork a Python filter — Class-18 mirror discipline). Optionally emit a clang-targeted `compile_commands` (or a `.clang` flags file) so the layout probe has a first-class clean flag path. Close when the gate lands with the shared filter wired.
- **Cross-ref:** D-320 (cache-layout gate) · D-321 (clang=LAYOUT / g++=CODEGEN toolchain split) · `fox-symdeps.nvim/lua/fox-symdeps/sizeprobe.lua` (`_filter_flags` / `_flags_for`) · `recordlayout.lua` (`M.straddlers` :122/:135) · `check_struct_alignment.py` (the CI alignas / size-pin guard) · `DESIGN_SPECS/doc-disciplines/in-code-documentation-schema.md` (§ CI-enforcement item 3, toolchain split) · `feedback_single_source_of_truth_discipline` · `feedback_no_unhomed_debt_code_smell`.
