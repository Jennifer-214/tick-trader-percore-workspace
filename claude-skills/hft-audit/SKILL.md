...
loads_dynamically: [DOCS/DESIGN_PHILOSOPHY.md, DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md, DESIGN_SPECS/refactor-patterns/decision-time-data-binding-pattern.md, DESIGN_SPECS/meta-disciplines/skill-knowledge-consultation-and-auto-routing.md]
skill_kind: judgment
...
- **Branchless dispatch opportunity scan:** Look for if/else if chains + switch statements on runtime enum values in SP/HP/drainer code that should be converted to fn pointer table (Pattern 1) or 2D state×type dispatch table (Pattern 2) per `DESIGN_SPECS/refactor-patterns/branchless-dispatch-discipline.md`. Flag candidates as Class 28 (per `DOCS/RECURRING_BUG_PATTERNS.md`). Detection patterns:
...
- **Codegen verification (intent-vs-emitted) — named hot loops only:** for the hottest inner loops (`BG_Evaluate` / `SG_Evaluate` / `ExecutionCore_Tick`, the AVX-512 kernels), verify the `-O3 -march=native` build ACTUALLY emitted what the source intends — branchless (no surprise conditional jump where a `cmov`/mask was expected), the AVX-512 kernel actually vectorized (not scalar-fallback'd), no register-spill under pressure. Inspect via `objdump -d` / godbolt, or the existing **codegen A/B oracle** pattern (the FPN-flip `check_fp_determinism` precedent — emit pre/post asm + diff). The gap this closes: branchless *source* can still compile to a *branch* under the wrong flags/inlining — intent-without-verification. NOT a blanket sweep; the named hot loops only. **MECHANIZED (`.E.1.0`): RUN `tools/check_latency_path_conformance.py` instead of hand-`objdump`** — it disassembles the named hot/slow fns on the PRODUCTION build (no `LATENCY_PROFILING`) and gates instruction-count + **branch-classification {loop / rare-cold / data-dependent-warm}** + no-scalar-float incl. AVX/FMA (H4) + no-div + no-malloc/free/lock + no-indirect/vtable (H2) + no-stack-spills, asserting its OWN non-vacuity (else it's the Class-51 guard it codifies against). The **data-dependent-warm** branch list (with source lines) IS the H7/H20 finding set; reducing it → 0 is the optimization leaf (D-234/D-235/D-236). Sister to `check_struct_size_budget.py`.
- **Instruction-level parallelism / dependency chains (secondary, hot loops):** flag long SERIAL dependency chains the CPU can't overlap — a reduction `acc += f(x[i])` where each iteration waits on the prior `acc`. The compiler SCHEDULES instructions but CANNOT break an *algorithmic* dependency; restructuring (multiple accumulators; reassociation where determinism permits) is the audit's call, not the compiler's. Overlaps the branchless + cache lenses — a worthwhile addition for inner loops, not a primary pillar. **NB — hand-written LEA / strength-reduction / micro-instruction-tuning is PROFILING-GATED** (the compiler does LEA + scheduling well at `-O3`; audit only on a *measured* stall): TECH_DEBT-200.
