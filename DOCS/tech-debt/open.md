...
- **PROGRESS (2026-08-12, half 1 LANDED — plugin `9d6c98b`):** the data-enumeration half is in — `toolio_kinds.lua` (consumed-kind registry: grammar/1→nodemodel, defining_site/1+cited_path/1→docview; findings/1 exempt per D-384) + a `:checkhealth` parity leg against `toolio_schemas.json` (tri-state: unreadable=refusal per Class 57) + suite tooth `test_toolio_kinds.lua` (planted missing-consumer/ghost/stale-exempt all FIRE; 43/43). A new producer kind with no plugin surface is now VISIBLE at :checkhealth. **Remaining (the closure bar):** the repo-FLOOR tooth (a `check_session_docs.sh`-family assert, NOT `parity_check.sh` — that harness PARKED at D-415; the fix-shape's home re-points to the live floor) + deriving the plugin's views kind-keyed off the registry rather than per-file wiring.
