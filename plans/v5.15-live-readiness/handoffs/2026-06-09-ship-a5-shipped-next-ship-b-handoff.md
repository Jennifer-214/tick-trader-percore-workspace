...
4. **OPERATOR TRIAGE PENDING (pre-existing; surfaced by the A.5 independent close review):** `Strategies/private/EmaCross.hpp` header says **"PRIVATE — do not publish"** but the file is TRACKED in the PUBLIC repo since ~v5.4.0. Two readings: stale header (ema_cross is in the public per-node strategy set — likely intentional-since; fix = delete the comment line) vs. real boundary leak (public a year; decide if its current form stays). Caramel decides; do NOT act unilaterally. Evidence: `plan_checks/2026-06-09-a5-close-independent-review.md`.
