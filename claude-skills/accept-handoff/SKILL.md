...
- **Tool index (ALWAYS consult — the always-CONSULT tier):** `DOCS/TOOLS.md` (every `tools/*` + disposition + invoker) — consult on EVERY pickup, do NOT wait to "need" a tool: knowing the inventory is what stops a session hand-rolling what a tool already does (and seeds the Stage-3.7 kit's mechanical-toolchain row). Load it (it's small), and **SEARCH it against the directive BEFORE reaching for grep** — if a tool covers what you're about to grep for, RUN the tool (the authority; catches the compiler-blind / transitive / ledger surfaces grep misses); grep is the fallback for open-ended discovery only. This is the general rule — search the index first, don't rely on recalling which tools exist — and it applies to the session AND every spawned agent (the `SUBAGENT_ARMING.md` §3 discipline, passed down at arm-time).
