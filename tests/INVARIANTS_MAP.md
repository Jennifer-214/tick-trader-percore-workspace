...
| 28 | **Capital-Control Latch Re-Arm** (every latch gating a capital control is cleared by EVERY resume path, or the survivor is justified) | D-421 close block `D-421 step 2: paper reset re-arms the drift auto-kill` — 4 checks: latch cleared, BREACHED edge cleared, ring emptied, plus a non-vacuity control proving the pre-reset node genuinely held a set latch and a non-empty ring | **COVERED (paper-reset path)** / **DISCIPLINE (manual kill-reset path)** — the manual path's clear lives behind `#ifdef USE_IMGUI_GUI`, so `./build.sh test` does not compile it; verified by `./build.sh gui` RC=0 and untested at runtime (v-class H-2). The asymmetry between the two paths is deliberate and commented at both sites. |
