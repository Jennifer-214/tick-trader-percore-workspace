...
| `check_struct_alignment.py` | STANDING-CI | pre-commit (Check K) | — | struct-alignment guard (Knight/H21-class) — every `alignas(>16)` struct (struct- or member-level) must NOT be allocated via bare malloc/calloc/realloc (those give 16B → misaligned = UB + broken aligned-SIMD/false-sharing); `new`/aligned_alloc/posix_memalign/arena-with-align OK. Surfaced by ubsan's first run during `.E` #11 Ship-A (RollingStats + EventLoopState); TECH_DEBT-157. Advisory (b): flags alignas types lacking a `static_assert(alignof==N)` lock. `FOXML_ENGINE` env override for the self-test |
| `test_check_struct_alignment.py` | TEST-HARNESS | test runner; manual | — | NEGATIVE self-test (teeth) — proves the alignment guard goes RED + names the type on a bare-malloc-of-`alignas(64)` violation, and GREEN on `aligned_alloc` (a guard that's only ever GREEN could be silently broken) |
