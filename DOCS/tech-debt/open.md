...
### TECH_DEBT-167 — F-096: partial-exit per-leg ENTRY qty split in `double` (H4 sizing violation + leg non-conservation)

- **id:** TECH_DEBT-167 · **severity:** medium · **opened:** 2026-06-11 · **status:** open · **surface_tags:** [accounting, H4, partial-exit, sizing, money, .E.1]
- **Title:** Partial-exit per-leg ENTRY order quantity is split in `double` — an H4 violation on the accounting/sizing path — AND leg-A + leg-B qty does NOT conserve `intended_qty`.
- **Created:** 2026-06-11 — surfaced by the `.E.0.10` Net-1 MED-tier OMS/money re-triage; the `/close-session` Stage 5.5 independent review caught the OWED ledger entry never being written (the auto-write contract was missed at fold-time; the obligation lived only in the register + the `.E.1` plan + task list).
- **Why DEFERRED (merit, not effort):** the fix is SUBSUMED by the `.E.1` Decision-K Portfolio/Position SoA re-layout (the qty field is re-laid-out there → make it `Money` not `double` + assert `legA.qty+legB.qty==intended_qty`); fixing it in the to-be-reworked `double` model = do-it-twice. NOT a Net-1 characterization (freezing a buggy `double`-path is wrong).
- **Mitigation in place:** HARD-gated at `.E.1` Decision K + acceptance "CLOSED bug classes" ("`.E.1` cannot close with a `double` money field surviving") — partially netted; this entry is the queryable durable row the gate references.
- **Hunt-validated site + magnitude (2026-06-12 A25 deep-harvest — discharges the register's "this hunt validates the ledger entry" forward-promise):** precise split site = `CoreFrameworks/EngineSharded/Async.hpp:827-837` (the leg-A + leg-B entry-qty split done in `double`); magnitude = `legA.qty + legB.qty ≠ intended_qty` (leg non-conservation, not just the H4 type violation).
...
### TECH_DEBT-196 — Per-core snapshot save/load block lacks a field-count-lock (silent wire-desync class)

- **id:** TECH_DEBT-196 · **severity:** medium (silent persistence desync; paper-mode continuity) · **opened:** 2026-06-14 · **status:** open · **surface_tags:** [persistence, wire-format, ci-guard, count-lock, H9, H21, .E.1]
- **What:** the per-core `CoreContext<F>` snapshot save/load block is a HAND-WRITTEN sequential `fwrite`/`fread` (`ShardedSnapshotPersist.hpp:176-260` save / `:441-487` read) with NO compile-time count-lock — unlike the OMS block, guarded by `static_assert(FOREACH_OMS_FIELD_PERSIST_COUNT == 10)` (`OmsFieldRegistry.hpp:380`). A field added to the save loop but forgotten in the read loop (or a reorder) is a SILENT wire-desync at recovery, NOT a compile error. Surfaced 2026-06-14 by the PERSIST A-class (the structural sibling of the PERSIST characterization gap — the test freezes the round-trip, only a count-lock/registry closes the desync CLASS).
- **Why / fix:** either (a) drive the per-core save/load from a `FOREACH_CORE_PERSIST_FIELD` X-macro registry (like OMS — single-source the field set → save/load/count auto-agree; + H15 meta-registry enrollment), or (b) the minimal `static_assert` field-count-lock + a paired save/read count check. (a) is the structural close; (b) is the floor.
- **Severity bound:** paper-mode continuity (live never restores, `Run.hpp:982`) → not live-capital-bearing, but a silent recovery corruption.
- **Trigger:** `.E.1` Decision-K Portfolio/CoreContext SoA re-layout (the exact re-pack that would trip a non-count-locked block — the highest-risk moment to have the guard).
- **Cross-ref:** register § Net-1 REMAINING-TARGETS (PERSIST row, Stage-2 structural find); `ShardedSnapshotPersist.hpp:176-260`; `OmsFieldRegistry.hpp:380` (the OMS count-lock to mirror); H9/H21; the PERSIST characterization (companion — test freezes the round-trip, this closes the class).
- **CLOSED 2026-08-15 via fix option (a), the STRUCTURAL close — verified in code, not asserted.** The hand-written sequential block is gone: the per-node save/read/commit are now driven by `FOREACH_NODE_PERSIST_FIELD` through its three projections (`ShardedSnapshotPersist.hpp:205` SAVE · `:414` READ · `:477` COMMIT), so the field set is single-sourced and save/load/count agree BY CONSTRUCTION. The count-lock this entry asked for exists as `static_assert(FOREACH_NODE_PERSIST_FIELD_COUNT == 29)` (`MemHeaders/NodeCtxPersistRegistry.hpp:134`), and H15 enrolment is live (69/69, `check_meta_registry` Checks 1-3 PASS). Landed at **D-305 (engine `63364a7`, 2026-08-14)** — i.e. this entry has been closable for a day and nothing flipped it, which is the WH-7 stale-disposition shape (`feedback_tag_disposition_at_fix_time`) and the reason it surfaced only when the pre-commit tech-debt advisory was actually read rather than skimmed.
- **Closed ABOVE its own bar.** The entry asked for save↔load agreement; D-421 step 2 additionally added the COMPLEMENT half — `check_node_ctx_partition.py` (pre-commit **Check Q**) proves every `NodeContext<F>` member is persisted OR declared-exempt, which is the question a count-lock structurally cannot ask (a count-lock is vacuous against both a count-neutral swap and a member that was never counted). The founding instance of THAT gap — `node_gross_wins` — is instance #1 of the newly-codified **Class 58**.
