...
# ----------------------------------------------------------------------------
# Ship-B P1c (2026-06-10, D-100): Money_FromString + cast references + vectors.
# Parse spec: flags {MALFORMED=1, OVERFLOW=2, EXCESS_DP=4}; consumers REFUSE on
# MALFORMED per the per-site table => malformed rows assert FLAGS ONLY (value
# is dont-care; the engine's quirk-preserving scan value is NOT modeled here —
# oracle independence). Clean rows: value == Decimal exact (half-even at >8dp).
# Cast specs: to_binary = round_he(m*2^64/10^8) (saturation unreachable);
# from_binary = round_he(b*10^8/2^64) closure-saturated + OVERFLOW flag.
# ----------------------------------------------------------------------------
def _parse_spec_flags(s):
    LIM = (1 << 63) - 1
    body = s[1:] if s[:1] in '+-' else s
    digits = sum(c.isdigit() for c in body)
    bad = any(c not in '0123456789.' for c in body) or (s[:1] not in '+-' and s[:1] == '.' and False)
    dots = body.count('.')
    malformed = 1 if (bad or dots > 1 or digits == 0) else 0
    flags = malformed
    if not malformed:
        frac = body.split('.')[1] if '.' in body else ''
        if len(frac) > 8:
            flags |= 4
        with localcontext() as ctx:
            ctx.prec = 80
            v = int((Decimal(s) * SCALE).to_integral_value(rounding=ROUND_HALF_EVEN))
        if abs(v) > LIM:
            flags |= 2
            v = max(-LIM, min(LIM, v))
        return v, flags
...
def emit_money_parse_cast_vectors(path_parse, path_cast):
    LIM = (1 << 63) - 1
    P64 = 1 << 64

    strings = ["0", "1", "-1", "+1", "0.0", "-0.00000000", "50000.50", "0.00006150",
               "12345678.12345678", "92233720368.54775807", "92233720368.54775808",
               "99999999999999.99999999", "-92233720368.54775807", "0.00000001",
               "-0.00000001", "70000.12345678", "3.14159265", "0.1", "0.2", "0.3",
               "0.123456789", "0.000000005", "0.000000015", "0.0000000049999",
               "-0.123456785", "1.000000005",
               "", "abc", "1.2.3", "1e5", "+", "-", ".", "12a3", "--1", "1..2",
               "123456789012345678901234.5"]
    prows = []
    for s in strings:
        v, fl = _parse_spec_flags(s)
        prows.append((s, v, fl))

    def to_binary_spec(m):
        with localcontext() as ctx:
            ctx.prec = 80
            return int((Decimal(m) * P64 / SCALE).to_integral_value(rounding=ROUND_HALF_EVEN))

    def from_binary_spec(b):
        with localcontext() as ctx:
            ctx.prec = 80
            v = int((Decimal(b) * SCALE / P64).to_integral_value(rounding=ROUND_HALF_EVEN))
        ovf = 1 if abs(v) > LIM else 0
        return max(-LIM, min(LIM, v)), ovf

    money_edges = [0, 1, -1, SCALE, -SCALE, SCALE // 2, 12345678, LIM, -LIM, 70000 * SCALE]
    tb_rows = [(m, to_binary_spec(m)) for m in money_edges]
    for _ in range(150):
        m = random.randrange(-LIM, LIM + 1) >> random.randrange(0, 50)
        tb_rows.append((m, to_binary_spec(m)))
    BINMAX = (1 << 127) - 1
    bin_edges = [0, 1, -1, P64, -P64, P64 // 2, (1 << 63), 3 * P64, BINMAX, -BINMAX,
                 (LIM * P64) // SCALE, (LIM * P64) // SCALE + P64]
    fb_rows = [(b, *from_binary_spec(b)) for b in bin_edges]
    for _ in range(150):
        b = random.randrange(-BINMAX, BINMAX + 1) >> random.randrange(0, 100)
        fb_rows.append((b, *from_binary_spec(b)))

    def i128_split(v):
        u = v & ((1 << 128) - 1)
        return u & 0xFFFFFFFFFFFFFFFF, (u >> 64) & 0xFFFFFFFFFFFFFFFF

    with open(path_parse, "w") as f:
        f.write("// GENERATED by decimal_oracle.py emit_money_parse_cast_vectors() — Ship-B P1c D-100 fixture.\n")
        f.write("// MALFORMED rows assert FLAGS ONLY (value dont-care — sites refuse; oracle stays independent).\n")
        f.write("#pragma once\n#include <cstdint>\n#include <cstddef>\n\n")
        f.write("struct MoneyParseVec { const char* s; int64_t expected; uint32_t flags; };\n")
        f.write("static const size_t MONEY_PARSE_VECTOR_COUNT = %d;\n" % len(prows))
        f.write("static const MoneyParseVec MONEY_PARSE_VECTORS[] = {\n")
        for s, v, fl in prows:
            f.write('    { "%s", %dLL, %du },\n' % (s.replace('\\', '\\\\').replace('"', '\\"'), v, fl))
        f.write("};\n")
    with open(path_cast, "w") as f:
        f.write("// GENERATED by decimal_oracle.py emit_money_parse_cast_vectors() — Ship-B P1c D-100 fixture.\n")
        f.write("// i128 values carried as (lo,hi) u64 pairs of the two's-complement bit pattern.\n")
        f.write("#pragma once\n#include <cstdint>\n#include <cstddef>\n\n")
        f.write("struct MoneyToBinVec { int64_t m; uint64_t b_lo, b_hi; };\n")
        f.write("static const size_t MONEY_TOBIN_VECTOR_COUNT = %d;\n" % len(tb_rows))
        f.write("static const MoneyToBinVec MONEY_TOBIN_VECTORS[] = {\n")
        for m, b in tb_rows:
            lo, hi = i128_split(b)
            f.write("    { %dLL, %duLL, %duLL },\n" % (m, lo, hi))
        f.write("};\n")
        f.write("struct MoneyFromBinVec { uint64_t b_lo, b_hi; int64_t expected; int ovf; };\n")
        f.write("static const size_t MONEY_FROMBIN_VECTOR_COUNT = %d;\n" % len(fb_rows))
        f.write("static const MoneyFromBinVec MONEY_FROMBIN_VECTORS[] = {\n")
        for b, e, o in fb_rows:
            lo, hi = i128_split(b)
            f.write("    { %duLL, %duLL, %dLL, %d },\n" % (lo, hi, e, o))
        f.write("};\n")
    print("[oracle] wrote %d parse + %d to_binary + %d from_binary vectors" % (len(prows), len(tb_rows), len(fb_rows)))
