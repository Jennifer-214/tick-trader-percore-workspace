...
- **id:** TECH_DEBT-212 · **severity:** MED (capital-safety for the capital subset; latent-until-live) · **opened:** 2026-06-22 · **status:** in-flight (capital subset closing at ③/D-253) · **surface_tags:** [cfg-validation, capital, vacuous-guard, Class-51]
...
- **Disposition:** ③/D-253 CLOSES the CAPITAL subset (the `CAPITAL_BOUND_LOSS/GAIN` fields → ERROR via the barrier constants, replacing the vacuous clamp). The non-capital percent reconciliation (~36 remaining fields) → E.2 value-sweep. The vacuous-clamp PATTERN → RBP anti-pattern at ③ ship-close.
- **Cross-ref:** D-253 (capital-subset fix) · IA-cascade SSoT · E.2 (non-capital sweep) · the `uniform-rule-over-heterogeneous-cohort` anti-pattern (to codify).
