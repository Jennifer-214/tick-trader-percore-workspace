...
### TECH_DEBT-157 — struct-alignment guard tool (alloc-site + alignof-assert CI check)

```yaml
id: TECH_DEBT-157
title: A static CI tool — every alignas(>16) type's allocation sites honor its alignment, and every alignment-critical struct carries a locked static_assert(alignof==N)
severity: medium
surface_tags: [ci-tooling, capital-safety, data-oriented-design, alignment, structural-enforcement]
trigger: dedicated tooling/cleanup pass post-Ship-A; motivated by the RollingStats malloc-fallback misalignment ubsan caught during Ship-A acceptance
status: open
opened: 2026-06-08
related_specs: [meta-disciplines/structural-enforcement-when-memory-insufficient.md, data-disciplines/cache-line-discipline.md]
```

- **Created:** 2026-06-08 — operator: "is there a tool we can make to check struct alignment across the codebase?" Surfaced when ubsan (run for the first time during Ship-A acceptance) caught a `RollingStats` (alignas(64)) stored from `alloc_or_arena`'s malloc fallback, which dropped the alignment arg (`std::malloc` → 16-aligned). A convention-only invariant (alloc honors alignment) with no mechanical guard — the Knight/H21 shape.
- **What it checks:** (a) every `alignas(N>16)` type → find its alloc sites (arena/pool/malloc/aligned_alloc/new) + flag any that drop the alignment for such a type (the bare-`malloc(bytes)` class); (b) every alignment-critical struct carries a co-located `static_assert(alignof(X)==N)` so a future layout change that breaks it fails compile (generalizes the R1 `offsetof` asserts to `alignof` + alloc-site coverage).
- **Why MEDIUM:** capital-path — a misaligned alignas(64) struct breaks false-sharing isolation + faults aligned-SIMD; runtime sanitizers catch it only if a test hits the path AND asan/ubsan aren't in CI. A static guard closes the whole class at CI time.
- **Trigger:** dedicated pass post-Ship-A (NOT bolted onto the flip — adjacent deliverable per features-first-then-cleanup). The Ship-A malloc-align fix (`aligned_alloc` in `alloc_or_arena`) is the first instance it would have caught.
- **Status:** OPEN (operator-floated 2026-06-08).
- **Cross-ref:** Ship-A acceptance ubsan finding (RollingStats malloc-fallback); `structural-enforcement-when-memory-insufficient.md` (M7); `feedback_guards_compound_enforcement_is_leverage`.

### TECH_DEBT-158 — pre-existing sanitizer findings (asan AVX-512 SEGV + ubsan timing-test) — deferred from Ship-A

```yaml
id: TECH_DEBT-158
title: Pre-existing asan/ubsan findings surfaced by the first-ever sanitizer run during Ship-A — asan AVX-512-masked-op SEGV + ubsan timing-test flakiness under instrumentation; NOT flip-related
severity: low
surface_tags: [ci-tooling, sanitizers, avx512, ml-bandit, testing]
trigger: dedicated sanitizer-cleanup pass (composes with TECH_DEBT-157 wiring asan/ubsan into CI)
status: open
opened: 2026-06-08
related_specs: [avx512-byte-determinism-pattern.md]
```

- **Created:** 2026-06-08 — Ship-A acceptance ran asan/ubsan for the first time (neither is in CI). After fixing the 2 flip-adjacent misalignment alloc sites (RollingStats arena-fallback + EventLoopState test malloc → `aligned_alloc`), two PRE-EXISTING, non-flip findings remain in flip-untouched code:
  1. **asan SEGV in `_mm512_mask_storeu_pd`** (`BanditLearning.hpp:226` `Bandit_GetProbabilities`; sibling `RidgeBlender` sites). Buffers are correctly sized (`weights[8]`/`probs[8]`, `BANDIT_MAX_ARMS=8`) → NOT an OOB; asan can't instrument AVX-512 masked/wide ops cleanly. Standard fix: `__attribute__((no_sanitize("address")))` on the SIMD kernels (correct-by-construction; verified by the scalar-reference byte-determinism tests).
  2. **ubsan `v5.11.3.B: reader observed >= 100 snapshots`** — a seqlock threading test whose fixed-window snapshot-count threshold is too tight under the instrumented `-O1` build. Fix: sanitizer-aware threshold (lower the count / skip under `__SANITIZE_*__`).
- **Why LOW + deferred:** both pre-existing (flip-untouched code), NOT flip regressions. The flip is value-correct (3246/0) + own-UB clean (ubsan no runtime errors; INT_MIN probe passes). Deferred per operator at Ship-A (scoped to flip-clean + no-new-failures); fixed in a dedicated sanitizer-cleanup pass.
- **Status:** OPEN (deferred at Ship-A 2026-06-08).
- **Cross-ref:** TECH_DEBT-157 (alignment-guard tool + wiring sanitizers into CI); `avx512-byte-determinism-pattern.md`.
