...
- **🔑 CONCURRENCY CLUSTER (the disciplined-investigation payoff):** `persist-8` (the robust hot-path quiesce), `persist-dod-1` (torn periodic save), the known kill-switch `oms->balance` torn-read (`Async.hpp:434`, tracked + a code comment), and `conc-5` (CRITICAL submit-queue multi-producer race) ALL share ONE root — **unsynchronized cross-thread reads of drainer-written money state** (no seqlock / consistent-copy on the producer/save side). ONE fix-shape (a seqlock or a snapshot-copy for the cross-thread money reads) addresses the whole cluster. → fix them TOGETHER in a focused concurrency session (with the `conc-5` tsan pass), NOT piecemeal — and not in this giant, money-math-saturated context that degrades race-sensitive work.
