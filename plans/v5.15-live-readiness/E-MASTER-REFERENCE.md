...
**▶ UPDATE (2026-06-15) — `.E.0.10` (Net-1) SHIPPED** (tag `v5.15.5.F.4d.1.E.0.10`): the pre-`.E.1` correctness net + the capital-backlog burn-down — A6 capital guard (egress+ingress) + the 4 durable-now fixes (NEW-1 single capital-authority predicate / A20 seed-don't-replay / A21 venue-net detect-only / A17) + TD-202 OMS-UAF fix (RBP Class 50) + Phase-D blanket live-gate (live refused until `.E`; TECH_DEBT-203). **Both pre-`.E.1` gates are now MET** — Net-1 characterization ✅ + guard-coverage-matrix no-HOLE ✅ (§4d). **`.E.1` is unblocked to dive**; its FIRST-order work = close the recorded H7/H10/H14/H20 hot-path/determinism HOLE cluster BEFORE the rename. Live SSoT = `MASTER.md` latest ▶ UPDATE banner + the current `status: active` handoff in `handoffs/` (categorical pointer — singleton-enforced; do NOT hardcode a handoff filename here, it goes stale — WH-2). As of 2026-06-28: `.E.1.1` is mid-dive (rename LANDED; ③ config-compiler arc CLOSED — item-2 + the clean-break + N1 + GAP-1 LANDED `be5d803`; **item-4 step-1 = B's merge** [legacy capital arrays → `nodes[c]` via `FOREACH_PER_NODE_ARRAY_OVERRIDE`] LANDED `9c93d1d` 3680/0; item-4 CORE next [LOSS/GAIN post-resolve sweep + explicit dollar floors]; the `metadata_flags` uint32 widening + the `*_mult`/FEE(venue-sourced)/DOLLAR/partial_exit cap-bits + the `[section]` parser all deferred → E.1.6/E.2 per D-275/D-276).
